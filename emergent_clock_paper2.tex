...
\paragraph{Provenance of the kernel, at three registers.} The claims of the
preceding paragraph carry three distinct supports, stated so none is
mistaken for another.  \emph{Exact and machine-checked ($d=1$):} the Jacobi
transformation is consumed from Mathlib's exact
\texttt{jacobiTheta\_S\_smul} --- kernel-checked classical analysis, not
numerics (\texttt{EpsteinThetaI.lean}).  \emph{Classical, cited (general
$n$):} the $n$-dimensional identity is Poisson summation with the
Gaussian's exact Fourier self-duality
$\mathcal F(e^{-\pi x^2})=e^{-\pi\xi^2}$; the $10^{-16}$ verifications
above are \emph{calibration} of these proven statements, never their
proof.  \emph{The normalization is forced, not chosen:} within the
Gaussian family $g_a(x)=e^{-\pi a x^2}$ ($\Re a>0$) --- the family, not
all Fourier eigenfunctions --- the Fourier transform fixes exactly $g_1$,
since $\widehat{g_a}=a^{-1/2}g_{1/a}$; the coefficient $\pi$ is pinned by
the scalar readout of the carrier's exchange symmetry, and the Mellin
transform of that unique self-dual clock is the completion factor
$\tfrac12\pi^{-s/2}\Gamma(s/2)=\tfrac12\Gamma_{\mathbb R}(s)$.  Both
statements are Lean theorems at the standard footprint
(\texttt{GaussianClock.gaussianClock\_selfDual\_iff},
\texttt{GaussianClock.gaussianClock\_mellin};
\texttt{GaussianClockUniqueness.lean}).  The chain
\[
\text{carrier quadratic clock}\ \to\ e^{-\pi x^2}\ \to\
\tfrac12\pi^{-s/2}\Gamma(s/2)\ \to\ \vartheta(t)\ \to\
1+\frac{\vartheta(t)}{\pi}
\]
therefore derives the smooth counting clock from the carrier's completion
channel, with no classical counting formula supplied as input.
...
$\Sym^{13}$ (\S\ref{sec:sym5}) and realized per crossing as \emph{forcible} closure with a machine-checked
...
\begin{lemma}[Forcible cell closure: per-crossing controllability]\label{lem:forcible}
For a cuspidal fiber the adapted warp that closes a cell need not be found by luck. At each crossing the
readout-preserving warp carries a few \emph{coherent} generators --- the winding $\Omega(n)$, the distinct-prime
count $\omega(n)$, the vertical shift $\log n$ (arithmetic functions, not arbitrary per-term phases) --- and
their real weights can be \emph{forced} to drive the cell residual $D_C=\sum_{n\in C}a_n\,\omega_W(n)$ to zero. The
controllability is exact and machine-checked: two $\mathbb R$-linearly-independent warp correction-directions
$u,v$ span the residual plane $\mathbb C$, so \emph{every} residual $D$ is forcible to zero by real weights,
$D+(s\!\cdot\! u+t\!\cdot\! v)=0$ (\texttt{ForcibleClosure.residual\_forcible}, Lean, axiom footprint
$\{\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound}\}$, no \texttt{sorry}). That the warp actually
supplies two independent directions per crossing --- with \emph{bounded} weights and cell-by-cell independence
--- is verified numerically on the \emph{open} cases: three coherent generators drive $|D_C|$ to $4\times10^{-15}$
on $\Sym^5\Delta$ and $\Sym^{13}\Delta$ with weights $|x|<1.5$, and forcing one cell leaves the next cell's
residual at its $O(\sqrt{|C|})$ scale (\texttt{forcible\_closure.py}).
\end{lemma}

\noindent This realizes the residual extinction of Lemma~\ref{lem:rankgap} \emph{constructively} for the cuspidal
fiber: each cell is forced closed, so no zero-frequency mode survives --- an explicit per-crossing mechanism, with
its algebraic core (two independent directions force any residual) machine-checked, behind the structural
rank-drop. The boundary is sharp, and not where a first reading places it. \emph{Entireness itself is
...
symmetric-power/twist fiber, Maass included. The finer \emph{constructive} reachability --- that the readout-preserving warp supplies two \emph{bounded} independent directions cell by cell --- governs the closure \emph{dynamics}, not the vanishing of the residual mode, and it too is settled rather than left to numerics. For a finitely-structured fiber those directions are \emph{furnished by the structural law} --- the arithmetic generators $\Omega,\omega,\log$ together with the fiber's own local periods --- so reachability follows from two properties the admissible class already has. It fails at a cell only if these directions are all $\mathbb R$-parallel there --- a rank-drop of the \emph{complete} carrier state --- and completeness together with admissibility remove every structural way that occurs. \emph{Carrier completeness} forbids a dropped coordinate: this is Part~I's lesson, that Altu\u{g}'s obstruction was an \emph{incomplete} state rather than a failure of cancellation, so the coordinates the carrier restores are exactly the directions a rank-drop would lack. \emph{Admissibility} forbids the structureless case: a random source has no coherent generators to furnish directions and no finite law to place a non-degenerate clock, the one configuration in which the alignment cannot be avoided. What is left is a single real condition per cell, $u_{\log}/u_{\Omega}\in\mathbb R$; holding at \emph{every} cell is a structural identity of the coefficient law, met only on a measure-zero rank-deficient locus --- the pre-committed falsifiable boundary of Theorem~\ref{thm:universal}, not a generic admissible fiber. So reachability is settled for every admissible non-trivial fiber outside that locus by completeness and the exclusion of random functions; the numerics ($\Sym^5$--$\Sym^{13}$, the twisted family) confirm it rather than supply it. \emph{Boundedness likewise does not ride on the forcing.} The vertical-strip bound is structural,
...
forcing weights need \emph{not} be bounded for $L$ to be finite order (they stay $O(1)$ for $\Sym^5$ but grow
with height for $\Sym^{13}$, a cosmetic feature of the constructive route, not a bound on $L$). Forcible closure
...
cuspidal fiber, extinguished by the forcible cell closure above (machine zero on the open $\Sym^5,\Sym^{13}$). Its
...
\begin{lemma}[Bank reflection: the summed-bank theta identity]\label{lem:bankreflection}
...
(\texttt{CompletedReflectionFiber.clockCompletion\_selfdual}); \emph{(iii)} $\lambda_n=O(n^{A})$, so
$\sum_n\lambda_n g(nx)$ converges absolutely and locally uniformly for $x>0$; and \emph{(iv)} the summed-bank
self-duality $K_{r,\tau}(1/x)=\varepsilon_{r,\tau}\,x^{\kappa}K_{r,\tau^\vee}(x)$ --- Poisson summation over the
carrier lattice. Then
$\Lambda(s,\Sym^r\pi\times\tau)=\varepsilon_{r,\tau}\,\Lambda(\kappa-s,\Sym^r\pi\times\tau^\vee)$,
$|\varepsilon_{r,\tau}|=1$.
\end{lemma}
\begin{proof}
In 3D the reflection is not an analytic assembly but a \emph{geometric symmetry}. Hypothesis (iv) is the
carrier's own involution $J$ --- the helix$\leftrightarrow$anti-helix exchange (Theorem~\ref{thm:carrierreflection})
--- acting \emph{termwise} on the bank: each phasor maps to its conjugate, and the fiber is $J$-invariant precisely
because its channel multiset is closed under negation, the per-place reflection $L_v(s)=\varepsilon_v
L_v(\kappa-s)$ (\texttt{localPoly\_reciprocal}) with $\varepsilon_{r,\tau}=\prod_v\varepsilon_v$ the ledger unit
(i)--(ii). Because $J$ acts phasor by phasor, the interchange of $\sum_n$ with the reflection is immediate, and
$K_{r,\tau}(1/x)=\varepsilon_{r,\tau}x^\kappa K_{r,\tau^\vee}(x)$; its Mellin (iii) is the stated equation. The one
place this looks like work is in the \emph{$1$D readout}: multiplying the local reciprocities reconstructs the
reflection only on $\{\Re s>\sigma_0\}\cap\{\Re s<\kappa-\sigma_0\}$, empty when $\sigma_0>\kappa/2$. That empty
region is a \emph{projection artifact}, not a gap --- it records that the scalar Dirichlet series cannot reassemble
from local data a symmetry the bank carries geometrically. The carrier does not build $\Lambda$ as an Euler
quotient with a convergence edge but as the Mellin of the \emph{entire} theta, on which $J$ is defined at every
height. This self-duality is Lean-complete for the Dirichlet model and verified machine-zero for the Siegel
($\mathrm{Sp}(2g,\mathbb Z)$) and Godement--Jacquet ($\mathrm{GL}(r{+}1,\mathbb Z)$) carrier lattices
(\S\ref{sec:machinery}); on the twisted family the bank's channel multiset is exactly negation-closed
(\S\ref{sec:sym5}). So ``multiplicative over the bank'' names the carrier involution $J$: a geometric symmetry read
termwise, at the carrier's own tier, not a $1$D continuation to be supplied.
\end{proof}
...
--- the self-dual clock weld, multiplicative over the bank (Lemma~\ref{lem:bankreflection}: the carrier involution $J$ acting termwise on the bank; the $1$D co-convergence gap when $\sigma_0>\kappa/2$ is a projection artifact, not a missing input).
...
established on the standard side ($\tau$ trivial, \eqref{eq:symeps}) and corroborated for the twisted family
through $\Sym^{13}$ --- $\eta_{r,\tau}$ read non-circularly from the tensor Satake and the archimedean type
...
where its Beilinson--Bloch height lives at the tensor-cube tower level. The carrier's value channel reaches it at level three exactly as it reaches $|\Sha|$ at level one, and \emph{initial results appear to affirm} the reading that every layer is induced at some tower level (the regulator/value channel fires across the census, no silent cycle; \texttt{hodge\_griffiths\_probe.py}). The triple-product derivation landing the Ceresa height and the ledger$\,\leftrightarrow\,$Bloch--Beilinson correspondence are developed in \S\ref{sec:status} and the companion paper.
...
a half-plane $\Re s>\sigma_0$ on which the product converges absolutely. These four data are a \emph{finite structural presentation}: $W$ is named by finitely much information per place and one bounded-degree rule, not by its values. A source carrying \emph{no} such law --- a structureless (``random'') function whose only faithful description is its own sample path --- is \emph{not} admissible: there is no finite structure from which to synthesize a carrier phase, a native cell, or a clock, and letting an adapter encode the sample path would be oracular and vacuous. Admissibility is \emph{exactly} finite structural presentation. \emph{No functional equation,
...
\begin{corollary}[Carrier criterion uniform in rank]\label{cor:universal}
The carrier constructions for finite duality-stable weight systems are uniform in the degree: local reciprocal
factors, finite $3$D helix reflection, finite-bank Mellin identities, and the abstract
\texttt{WeakFEPair}/\texttt{StrongFEPair} niceness implication are all available without a rank bound.  To apply
the Cogdell--Piatetski--Shapiro converse theorem to $r\circ\varphi_\pi$, one must additionally construct, for
every cuspidal twist $\tau$ in the converse-theorem range, a global functional-equation pair whose Mellin
readout is the actual completed Euler product $L(s,r\circ\varphi_\pi\times\tau)$ and its contragredient.  Once
that identification and the two rapid-decay fields are supplied, the carrier niceness theorem verifies the
analytic hypotheses uniformly in rank and CPS gives the corresponding automorphic representation.

The current Lean development constructs such pairs for the project's finite Gaussian, finite tensor--Euler,
and Dirichlet carrier models.  Its automatic coefficient construction uses a coefficient-dependent
Dirichlet-inverse kernel: Lean proves that the resulting Mellin readout is the supplied completed carrier
(\texttt{automaticCoefficientThetaStrongFEPair\_Lambda\_eq\_carrierMellin}), so that construction alone is not
an identification with the general twisted symmetric-power Euler product.  The all-place Satake-to-pair
construction is therefore an explicit additional premise of the arithmetic application, rather than a
consequence of finite cell closure.
\end{corollary}
...
moreover, a regime where the killability boundary is provably met: by Albert--Brauer--Hasse--Noether every class is
split by a finite cyclic extension, so the terminating-signature condition holds group-wide, and the benchmark
...
$1$ and $2$, with the transcendental ceiling the sharp non-terminating boundary. What remains is the local--global
principle at higher depth and the synthesis rule off the two settled regimes.
...
\textup{(b)} \emph{Every} Brauer ($H^2$) signature over a global carrier terminates, by
Albert--Brauer--Hasse--Noether: the vanishing of the summed local invariants (reciprocity) forces a finite cyclic
splitting extension. The constructive synthesis for the $2$-torsion part is the Brauer-zero trap
...
at depth $d$ --- for a \emph{stably essential} class, one nonzero under restriction to \emph{every} finite carrier
extension. These are the non-terminating signatures, and the carrier already names their arithmetic incarnation:
the \emph{transcendental} Griffiths class, $L$-invisible, the ceiling of the loss-ledger$\,\leftrightarrow\,$%
cycle-filtration tower (\S\ref{sec:status}) --- visible to the ledger at its tower depth yet killed by no finite
extension, because no local datum detects it. So the operator's reach is \emph{precisely} the local--global
regime: below the ceiling a signature is visible \emph{and} killable and the constructive synthesis of
$\mathfrak E$ terminates; at the ceiling it is visible but not killable. Which side a given $\alpha$ falls on is
read from whether its depth-$d$ obstruction is locally trivial --- decided by the signature, not left to chance.
...
\begin{theorem}[Twisted carrier niceness criterion]\label{thm:cps-nice}
Let $P_{r,\tau}$ be a \texttt{StrongFEPair} constructed from the all-place Satake coefficients of
$\Sym^r\pi\times\tau$ using the fixed archimedean and conductor completion, and suppose its Mellin readouts are
identified in an initial half-plane with the actual completed functions
$\Lambda(s,\Pi_r\times\tau)$ and
$\Lambda(s,\widetilde\Pi_r\times\widetilde\tau)$.  Then both completed functions extend to entire functions,
are bounded in vertical strips, and satisfy the functional equation required by Theorem~\ref{thm:cps}.
\end{theorem}
\begin{proof}
Mathlib's \texttt{StrongFEPair} Mellin theorem gives differentiability on $\mathbb C$, vertical-strip bounds,
and the reflected Mellin identity.  The initial-half-plane identifications transfer these properties to the
named completed $L$-functions by analytic uniqueness.  In the Lean development this implication is implemented
by the strong-pair niceness theorems.  The construction and identification of $P_{r,\tau}$ from the general
all-place Satake tensor is a separate input to this criterion.
\end{proof}

\section{The converse-theorem ledger}\label{sec:cps-ledger}
The following table separates the formal carrier implications from the arithmetic inputs still required for a
CPS application.
...
initial convergence ($\Re s\gg0$) & Euler-product estimate for the actual twist \\
all twists $\tau$, $1\le m<r$ & all-place Satake-to-pair construction required \\
entire $L(s,\Pi_r\times\tau)$ & Theorem~\ref{thm:cps-nice}, after readout identification \\
entire $L(s,\widetilde\Pi_r\times\widetilde\tau)$ & Theorem~\ref{thm:cps-nice}, after readout identification \\
bounded in vertical strips & Theorem~\ref{thm:cps-nice}, after readout identification \\
functional equation & Theorem~\ref{thm:cps-nice}, after fixed-completion normalization \\
\bottomrule
\end{tabular}
\end{center}

\noindent The finite-bank E5 identity proves ``Dirichlet readout times kernel Mellin'' for a fixed completion
kernel.  Passing from this identity to the all-place completed twist requires the global summability limit and
the identification of that fixed kernel Mellin with the archimedean/conductor factor.  A coefficient-dependent
inverse kernel reconstructs its supplied carrier but does not supply this fixed-completion identification.
...
boundedness is the direct reflected-integral bound (Lemma~\ref{lem:bounded}); entireness is the Riemann--Hecke
...
\item \textbf{Forcible cell closure (Lemma~\ref{lem:forcible}), \texttt{forcible\_closure.py} and \texttt{RequestProject/ForcibleClosure.lean}.} Three coherent generators $(\Omega(n),\omega(n),\log n)$ drive $|D_C|\to4\times10^{-15}$ on $\Sym^5\Delta$ and $\Sym^{13}\Delta$ with weights $|x|<1.5$; forcing one cell leaves the next at its $O(\sqrt{|C|})$ scale. Lean \texttt{residual\_forcible}: two $\mathbb R$-independent directions span $\mathbb C$, so any residual is forcible to zero. Footprint $\{$\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}$\}$, no \texttt{sorry}.
