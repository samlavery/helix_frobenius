...
properties exists it coincides with $\Fcar$. On every finite normalized
...
no nonzero finite extension state is silent at every level. At depth one the carrier's leading
...
\texttt{sorry} (the filtration dictionary, the finite-extension retention theorem, and the
...
(\S\ref{sec:harmonic}). [Frame calibrated against Lefschetz $(1,1)$ truth on
...
\item \textbf{Nothing is exotic.} Every class in every census is a frozen
...
law pre-registered, unfalsified.]
...
The scope is exact: a homologically trivial cycle must still be supplied with a
typed realization into this finite jet stack, and its arithmetic
derivative/height channels must be identified with the paired tower. Once that
landing is proved, retention transports automatically; semisimplification by
itself only sees layer zero.
...
m\equiv1,2\pmod4,
\]
and hence $\varepsilon(\psi^m)=-1\iff m\equiv0,3\pmod4$.
The period four belongs to the \emph{global} local-factor product.  Its
archimedean component has phase $i^{m+1}$, while the ramified finite
component supplies the compensating Gauss phase; in the standard
normalization the quadratic character modulo $7$ has
$\tau(\eta_7)/\sqrt7=i$.  Neither factor alone is the global sign.
...
At every measured grade with $\varepsilon(\psi^m)=-1$, the functional
equation forces a central zero and the computed derivative is nonzero.
Thus every forced zero in the tested range $1\leq m\leq12$ is simple: no
silent grade occurs in that range.  This is empirical evidence for
Retention along the ladder, not an all-grade nonvanishing theorem.  The
bad-prime coefficient at the ramified place is the primitive-local value
$\lambda_7=i^m$ (so $a_7=(\sqrt{-7})^m$; at grade two $a_7=-7$, the
classical coefficient of $7.3.b.a$), and the sign follows from the
carrier's $\varepsilon$-factor independently of it.  The residual left by
the split-vs-direct certificate at level $7$ is a projection-side artifact
of a one-dimensional device at the ramified place --- the sign is fixed on
the carrier and does not rest on it --- not an obstruction on the carrier.
...
not unnamed assumptions in the conclusion.
...
(\texttt{weil\_rails.py}, App.~\ref{app:repro}).
...
\textsc{SourceExhaustion}. On every finite normalized extension dial, retention
is already a theorem (\texttt{generalExtension\_retention} consumes the
layerwise Vandermonde separation). The new grade-split source packages prove
recognition, and
\texttt{generalExtension\_sourceExhaustion\_of\_gradeSplit\_sourceConstruction}
assembles both into unrestricted \textsc{SourceExhaustion} on the complete
finite-extension dial. Its scope is exact: arithmetic applications instantiate
the typed source packages and cycle-to-jet realization; no further logical
assembly remains.
...
identify its extension coordinates with that stack.
...
admissible frame, not a fresh construction. Step~three should then \emph{fall out}
...
one universal recognition. That, not a diffuse frontier, is where the work lives.
...
depth-three first visibility, and analytic-to-cycle source landing),
\texttt{RequestProject/HodgeSourceExhaustionAssembly.lean} (unrestricted source
exhaustion on the complete finite-extension dial),
...
\bibitem{Markman} E.~Markman, the algebraicity of Weil-type Hodge classes
on abelian fourfolds via hyper-K\"ahler geometry and hyperholomorphic
sheaves (2019--2024): the discriminant-one case and its extensions to all
fields and discriminants. [Exact reference chain per the campaign notes,
\texttt{tmp/weil\_scout\_notes.md}; to be pinned at final bibliography
pass.]
...
\bibitem{PatelZhang} D.~Patel and Z.~Zhang, \emph{On Weil classes and
cyclic covers}, arXiv:2506.13729 (2025).
