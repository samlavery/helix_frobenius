...
\medskip
\noindent\emph{The completed five-channel deep curve, and the burial
collapse.}  The full $30$-digit spectra at $N=120$ give per-channel deep
yields ($10^{-20}$ floor) of $10,9,7,6,4$ for $q=3,4,5,7,8$.  Assigning
each channel its effective depth $20-\lambda_1\log_{10}C_q$ decades
($\lambda_1=\tfrac{11}{2}$, $C_q=q^3$: effective depths
$12.1,\,10.1,\,8.5,\,6.1,\,5.1$), the yields collapse onto a single line
of $\approx0.85$ directions per effective decade, constant across all five
channels: within measurement resolution the decline is \emph{entirely}
burial --- no intrinsic channel overlap is visible.  Projected forward,
only conductors with $\lambda_1\cdot3\log_{10}q<20$ (i.e.\ $q\lesssim16$)
contribute at this depth at all, for a total quadratic-twist ceiling of
$\approx98$ of $120$ directions: at rank $6$, quadratic
$\mathrm{GL}(1)$ twisting cannot reach rigidity at any fixed finite depth
--- while at rank $3$, where $\lambda_1=1$ (the light zero-channel
parameter), the same throttle is negligible to astronomical conductors,
consistent with $\mathrm{GL}(3)$'s $\mathrm{GL}(1)$-only converse theorem.
The projection is an extrapolation of the measured collapse, not a
theorem.
...
escalate with $d$.  Fourth, the mechanism suggests \emph{why} higher-rank
twists are what converse theorems demand: a $\mathrm{GL}(m)$ twist with
archimedean parameters $\mu$ changes the tensor stack itself and can insert
\emph{light} channels (small $|\lambda_j-\mu|$-combinations) whose burial
rate is near zero, opening unthrottled information channels that no
$\mathrm{GL}(1)$ twist --- which shifts conductors but never lightens the
stack --- can provide.  On the carrier: a rail can only be read where its
phasors have matured, and only a higher-rank twist can lower a rail's entry
exponent.  This is interpretation, testable by re-measuring with mock
$\mathrm{GL}(2)$ Satake twists of tunable spectral parameter.
