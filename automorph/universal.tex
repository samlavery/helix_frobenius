...
Lem.~\ref{lem:bounded}, Lem.~\ref{lem:rankgap}; \texttt{FiniteWeightFiber.symTensorCompleted\_FE},
\texttt{TransferContinuation.transfer\_analytic},
\texttt{HarmonicCell.harmonic\_residual\_no\_independent\_mode}). [theorem, on Lean
carrier inputs]
...
\texttt{CarrierReachability.reachable\_independent\_stage}); the resulting nice $L$-function, fed to
the converse theorem (Thm.~\ref{thm:cps}), lands the fiber as automorphic, the symmetric-power tower
(item~1) being the fully-executed instance. \emph{Stated honestly}: that closure always \emph{exists}
...
theorem (\texttt{vonNeumannOp\_isSymmetric}), thirty-four machine-checked evidence facts
organized by four named engines (\emph{Feynman's Quiver}, \emph{Deligne's Pairs}, the
\emph{Shannon Cascade} with its projection dominance, \emph{Riemann's Fold}), GRH at
...
(\texttt{HarmonicCell.harmonic\_residual\_no\_independent\_mode}, channel-agnostic). The bridge is the identification of the
...
one. 
\subsection{Cycle-filtration faithfulness: the semisimple theorem and the Ceresa frontier}\label{sec:hodge-companion}
...
arithmetic-sensitive step is the \emph{inverse-warp transfer} of \S\ref{sec:construct} (machine-checked in
full by \texttt{TransferContinuation.transfer\_analytic}, with every admissible fiber closed in closed form
by \texttt{ForcibleClosure.residual\_forcible}), and the functional-equation bill is paid by
