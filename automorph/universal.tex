...
of automorphy lifting: Cor.~\ref{cor:satotate}. Its native carrier geometry and moment law are
...
Schur's lemma gives
\texttt{NonSelfDual.tateClass\_zero\_of\_simple\_not\_unit}.  The rank inequality is never used for
an ordinary vector-space embedding.
...
\texttt{unitaryPrimePhase\_towerCeiling} proves its two-sided power ceiling and
\texttt{unitaryPrimePhase\_strandBlock\_isometry} proves the associated transverse phase block is
an isometry. This does not insert unit modulus into the arithmetic strand
...
\texttt{integral\_empiricalPrimeAngleMeasure} identifies the resulting literal prime averages with the
probability-measure integrals, and the continuous mapping theorem then moves this already-derived
arithmetic convergence to the native 3D carrier. This case is out of reach of automorphy lifting --- a Maass form carries no
...
\texttt{NonSelfDual.tateClass\_zero\_of\_simple\_not\_unit}).  Thus $W_{r,\tau}$ is complete and the twist is entire. The exceptional/reducible $\pi$ carry a genuine
...
1D-zero/3D-zero/eigenvector.  This event-space assembly does not by itself
...
multiplicities.  Completing the analytic-coverage route requires proving the
