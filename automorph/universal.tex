...
(Prop.~\ref{prop:localid}), so the entire carrier readout is \emph{itself the continuation} of
...
consumed field, exactly as local Langlands is consumed by Prop.~\ref{prop:localid}); the
...
(\texttt{ConeProjection.record\_bijective}).  So Proposition~\ref{prop:localid} is closed on the
...
and root number at every place (Prop.~\ref{prop:localid}). \emph{Hypothesis-free on the completed
...
classical-$L$ identification (Prop.~\ref{prop:localid} with \textup{(E5)}) is closed on the carrier;
...
factors included, is \emph{not} a remaining scope item: Prop.~\ref{prop:localid} closes it on the
...
(Prop.~\ref{prop:localid} with \textup{(E5)}) is closed on the carrier and reproduced numerically
