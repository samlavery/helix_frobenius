...
\begin{proof}
Each law is machine-checked in \texttt{CarrierFunctoriality} with footprint
...
With the registered counts of Definition~\ref{def:registered-count}:
\begin{enumerate}
\item[\textup{(i)}] \(N_{\pi/3}(e^t)=N(t)\): the native count is the
arithmetic event count over the height ray;
\item[\textup{(ii)}] \(N_1(e^t)=1+\vartheta(t)/\pi\): the unit count is the DC
residue plus the archimedean clock, with \(\vartheta\) the Riemann--Siegel
theta, derived from the gauge, not inserted;
\item[\textup{(iii)}] consequently
\[
\boxed{\,N_{\pi/3}(e^t)-N_1(e^t)=S(t)\,},\qquad
S(t)=N(t)-1-\frac{\vartheta(t)}{\pi}.
\]
\end{enumerate}
Both \(N_{\pi/3}\) and \(N_1\) are derived as cumulative carrier crossing
counts; in particular \(N_1\) is not defined to be the theta term.  Every step
is machine-checked in \texttt{RequestProject/CarrierScaleCompensation.lean}
(no \texttt{sorry}; axiom footprint
\(\{\texttt{propext},\ \texttt{Classical.choice},\ \texttt{Quot.sound}\}\)):
...
\begin{definition}[Scale-indexed registered count]\label{def:registered-count}
...
In Lean the scale-indexed cumulative count functor is installed as
\texttt{registeredCount}: DC base at the common origin, the chart's continuous
clock procession in \(\pi\)-cells, and the event count weighted by the induced
per-event contribution of the scale's closure geometry
(\texttt{eventContribution}; derivation in the discharged-arrows subsection
below) --- every input pinned by a theorem.  The closure predicate is
...
\(N(t)\) is the zero-counting procession over the 3D representation's
...
\(t\): \(N_{\pi/3}(e^t)=N(t)\) (\texttt{native\_identification'}).  No
...
event channel gives \(N_1(e^t)=1+\vartheta(t)/\pi\)
(\texttt{unit\_identification}): the theta term is a conclusion, not a
definition.
\end{proof}
...
With parts (i) and (ii) supplied, the compensation identity is formal.  The
classical continued-argument comparison away from zero ordinates is
\[
...
The structural consequence --- \(S(t)\) has no native oscillatory carrier
state --- is \texttt{S\_has\_no\_native\_carrier} with
\texttt{no\_native\_oscillation}: despinning the unit readout by the clock
leaves a real value at every height.  \(S(t)\) exists only as the compensation
between the two carrier registrations of one arithmetic procession.
\end{remark}

\subsection{The arrows, discharged}
Both arrows previously recorded here as unsupplied --- the scale-indexed
cumulative count functor \(\mathsf X_{\zeta,H}\longrightarrow(z\mapsto N_H(z))\)
and the unit-\(1\) derivation --- are now proven in
\texttt{RequestProject/CarrierScaleCompensation.lean}, with no functor input
inserted by hand:
\begin{itemize}
...
\end{itemize}
The fundamental gap behind the theorem is prior to any analytic content: the
...
(\texttt{unit\_cell\_no\_antipodal}).  \(S(t)\) is the accumulated readout of
that incommensurability; crossings are only the units of the native
bookkeeping.

\subsection{The general registration gap \texorpdfstring{\(S_{H,K}\)}{S(H,K)} and the harmonic family}
The comparison generalizes to arbitrary scale pairs.  With \(N_H\) the
canonical scale-\(H\) chart count (\texttt{NH}; by \texttt{NH\_dichotomy} the
family contains exactly the two proven chart types, decided by the arc
geometry), define
\[
S_{H,K}(t):=N_H(e^t)-N_K(e^t)
\qquad(\texttt{Sgap}).
\]
...
\(N_H\) (\texttt{Sgap\_refl}, \texttt{Sgap\_antisymm},
\texttt{Sgap\_cocycle}).
\end{proof}
...
registers identically, and every member reads the \emph{same} classical
\(S(t)\) against the unit chart:
\[
S_{\pi/m,1}(t)=S(t),\qquad\text{i.e.}\qquad
N_{\pi/m}(e^t)=N_1(e^t)+S(t)\quad\text{for every }m\ge1.
\]
\end{theorem}
...
\(S_{\pi/m,1}=S_{\pi/m,\pi/3}+S_{\pi/3,1}=0+S\)
...
prescribed, and \(S_{\pi/6,1}=S\) (\texttt{Sgap\_pi6\_one}).
\end{proof}
...
machine-checked in \texttt{RequestProject/ResidueJump.lean} (no \texttt{sorry};
axiom footprint
\(\{\texttt{propext},\ \texttt{Classical.choice},\ \texttt{Quot.sound}\}\)).
...
% [MOVED to new Part~II immediately after Part~I: the converse-theorem
% checklist part formerly sat here, between "The universal transport" and
% "Significant Results".]
