...
\emph{is} $L(s,\Pi_r\times\tau)$: on the initial half-plane the two agree by the coefficient
identification (Prop.~\ref{prop:localid}), and both continue to entire functions, so they coincide
everywhere by the identity theorem
(\texttt{CarrierTargetIdentification.carrier\_FE\_transfers\_to\_target}, universal core
\texttt{entire\_eq\_of\_eqOn\_isOpen}). Every hypothesis of Theorem~\ref{thm:cps} is met (the ledger above), so $\Pi_r$ is a
...
artifacts made it uninstantiable by any operator.)  This separation
