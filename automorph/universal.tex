...
\begin{center}
\fbox{\parbox{0.94\textwidth}{\textbf{Status of equidistribution: exact arithmetic-to-3D
identification, with the analytic prime-cancellation theorem cited.} The full passage from the
literal arithmetic symmetric-power prime traces to three-dimensional equidistribution is
machine-checked in Lean~4 at the standard footprint
...
\emph{single} cited analytic input is the cancellation of the literal arithmetic prime traces
...
\S\ref{sec:aristotle-rs} --- discharging it \emph{is} the Sato--Tate conclusion. Given the
symmetric-power functoriality of item~1 (each $\Sym^r\pi$ automorphic) it reduces to the classical
non-vanishing of $L(\Sym^r\pi,s)$ on $\Re s=1$ (Jacquet--Shalika) followed by Wiener--Ikehara ---
both cited.}}
\end{center}
...
same two cited inputs (per-rung ceiling; per-rank cancellation).  Both
hypothesis-side items are now at their honest strength.  The ceiling enters
...
conclusion.  Toward discharging it, the Rankin--Selberg positivity that powers
both classical engines is extracted at the surface (\texttt{RSPositivity}): the
...
=(1+2\,\mathrm{Re}\,\alpha)^2$.  The Tauberian half exists in Lean elsewhere
(the \texttt{PrimeNumberTheoremAnd} project's \texttt{Wiener.lean}; its
supporting decay lemmas were still \texttt{sorry}d at the time of writing) and
its one-line theory covers $\zeta$ and Dirichlet $L$ only; the per-rank
one-line theory of $L(\Sym^r)$ for a Maass form is the open frontier itself ---
beyond $r=4$ even meromorphic continuation is classically unavailable, which is
precisely where the carrier tower's compiled all-rank structure is the
program's leverage.
...
\texttt{arithmeticSatakeCancellation\_iff\_carrierEquidistribution} therefore has the actual
