...
Lem.~\ref{lem:bounded}, Lem.~\ref{lem:rankgap}; \texttt{symTensorCompleted\_FE},
\texttt{TransferContinuation.transfer\_analytic},
\texttt{FocalResidualGeneral.harmonic\_residual\_no\_independent\_mode}). [theorem, on Lean
carrier inputs]
\item \textbf{The Ramanujan--Petersson conjecture for Maass forms} --- open; holomorphic forms
are Deligne's theorem, and the best Maass bound is $7/64$ (Kim--Sarnak \cite{KimSarnak}).
Established as the tower's radial limit: the strand radius is exactly one at every unramified
...
\texttt{strandBlock\_unitary\_of\_tower\_ceiling}). [theorem; limit step Lean]
\item \textbf{The Selberg eigenvalue conjecture} $\lambda\ge\tfrac14$ --- open since 1965. The
same limit at the archimedean place: zero radial drift, the strand clock unit-modulus at every
...
step Lean]
\item \textbf{Sato--Tate for Maass forms} --- open; the holomorphic case is
Barnet-Lamb--Geraghty--Harris--Taylor. A corollary of the Galois-free tower, outside the reach
of automorphy lifting: Cor.~\ref{cor:satotate}. [theorem]
\item \textbf{GRH/RH} --- two conditional proofs, each resting on exactly one named definitional
reading (what the Hilbert--P\'olya program requires of its spectrum; what a \emph{real}
nontrivial zero names), with the entire supporting spine unconditional: self-adjointness a
theorem (\texttt{vonNeumannOp\_isSymmetric}), thirty-four machine-checked evidence facts, GRH at
every modulus and character under either reading
(\texttt{grh\_of\_resolvant\_trace\_3D\_real},
\texttt{grhComplete\_of\_resolvant\_trace\_1D\_correlation}), and Riemann Hypothesis corollaries
at the trivial character (Part~\ref{part:grh}). [Lean; the two readings decided]
...
\item \textbf{The $S(t)$ mechanism} --- the classical oscillatory term, resistant to
interpretation since Riemann--von Mangoldt. Identified unconditionally:
$N_{\pi/3}(e^{t})-N_{1}(e^{t})=S(t)$, the registration gap between the harmonic carrier scale
and the unit-$1$ chart (Thm.~\ref{thm:carrier-scale-S}; \texttt{CarrierScaleCompensation}).
[Lean]
\end{enumerate}
...
every Satake parameter of every isobaric constituent by $p^{1/2}$ --- the same $\sqrt{\,\cdot\,}$
law at every rung, the $1$D chart's reading of the carrier's area-law envelope
(\texttt{scaleBalanced\_iff}). A radius exceeding one compounds past any fixed ceiling; a det-one
...
where no temperedness is used) --- and then closes it.
\end{proof}
...
coordinate $\to$ exact deterministic gauge $\to$ bounded real harmonic $\to$ restored readout. These are properties of the construction, not obligations to be met: the warp is unit-modulus and
cell-exact by the deterministic clock/fiber coupling (closed form, not asymptotic;
\texttt{ForcibleClosure.residual\_forcible}, \texttt{CarrierReachability.reachable\_independent\_stage});
the warped primitive is bounded by the cell identity; the warped series agrees with the Euler datum
on $\Re s>1$ term by term; and the tensor $W_1\otimes W_2$ is again a fiber with the det-one link
preserved (\texttt{tensorFiber}, \texttt{tensorFiber\_det\_one}). The change of coordinate is a
change of \emph{scale} --- the cell closes at the fiber's own harmonic scale --- so the inverse-warp
readout preserves the value as registration bookkeeping (\S\ref{sec:carrier-S}). The transfer of the
continuation, together with its analyticity, is machine-checked in full
(\texttt{TransferContinuation.transfer\_analytic}); the Dirichlet family is the fully machine-checked
end-to-end instance.
...
\texttt{CompletedReflectionFiber.clockCompletion\_selfdual}, \texttt{ConeProjection.midpoint\_to\_critical};
...
\texttt{CompletedReflectionFiber.symTensorCompleted\_FE} (every $r$, every duality-stable twist).
...
$\dim\tau<\dim\Sym^r$ (\texttt{TrivialChannel.no\_nonzero\_hom\_of\_finrank\_lt}); and the carrier builds the readout as the Mellin of an
...
(\texttt{RequestProject/FocalResidualGeneral.lean}, Lemmas~\ref{lem:bridge}, \ref{lem:rankgap})
...
% DEDUP 2026-07-12: duplicate of sec:cps-satotate (the full corollary + proof
% immediately below); commented out non-destructively.
\iffalse
