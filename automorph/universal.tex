...
\begin{center}
\fbox{\parbox{0.94\textwidth}{\textbf{Status of equidistribution: everything but one Tauberian
character-average is proved.} The full passage from the symmetric-power character averages to the
three-dimensional equidistribution is machine-checked in Lean~4 at the standard footprint
...
\emph{single} cited input is the symmetric-power character prime-average cancellation
$(N+1)^{-1}\sum_{j\le N}U_r(\cos\theta_{p_j})\to 0$ for each $r\ge1$
(\texttt{SymmetricPowerCharacterPrimeZeroInput}) --- a genuine Wiener--Ikehara Tauberian that is
\emph{provably on the equidistribution critical path}: the machine-checked biconditional
\texttt{characterAverageZero\_iff\_equidistribution} (standard footprint, no \texttt{sorry}) proves
this cancellation is not merely sufficient but \emph{equivalent} to weak convergence of the empirical
...
both cited, neither of RH/GRH strength. Everything but that one Tauberian input is proved.}}
\end{center}
