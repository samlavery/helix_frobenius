...
\gptGFour{G4 discharge: the finite structural presentation constructs the fiber, cells, scaling, clock, and readout; the synthesis map is total on finite structure and non-oracular.}
...
\(\mathcal L_*\), every native cell a disjoint union of refined cells.  On
\(\mathcal L_*\) each source's complete-cell closure holds coordinatewise
(Corollary~\ref{cor:g5-refined-coordinate-closure}), so normalizing every source
to the shared harmonic scale places all \(m\) fibers on one carrier without
disturbing any native closure.  Each realized fiber retains its represented
...
\gptGTen{G10 discharge: a detected cell residual is forced to zero by two independent carrier lane directions at a reachable stage, preserving identity and ledger.}
\gptDatum{D10}{Detected residual, reachable independent stage, real correction weights, and adapted state.}
...
D+s\,u+t\,v=0
\]
exist, extinguishing the residual.  The adaptation is readout-preserving, so it
retains the represented identity, ledger, unaffected fibers, and carrier
compatibility, and the adapted obstruction is zero,
\(\mathfrak o(\mathcal A(\mathsf X_W))=0\).
\end{theorem}
...
forcing there extinguishes the residual.  Since the correction is a
readout-preserving carrier adjustment, the represented identity
(Theorem~\ref{thm:g3-g4-identity-synthesis}), the ledger, the unaffected fibers,
and carrier compatibility are retained; hence the adapted obstruction is zero.
Both Lean facts carry the footprint
...
\texttt{FocalResidualVanishes.focal\_residual\_zero\_iff\_L\_zero}).
\end{theorem}
