...
constituent by the dimension count $\dim\varphi_\tau<\dim\Sym^r\varphi_\pi$
(\texttt{no\_nonzero\_hom\_of\_finrank\_lt}) --- so $R_{r,\tau}=0$ and no constant mode survives to carry a
...
completeness argument of Lemma~\ref{lem:completeness}; continuation to the axis neighbourhood is continuation at the
...
whenever $\dim\varphi_\tau<\dim\Sym^r\varphi_\pi$
(\texttt{TrivialChannel.no\_nonzero\_hom\_of\_finrank\_lt}, a dimension count --- no automorphy of $\Sym^r\pi$
used), so $W_{r,\tau}$ is complete and the twist is entire. The exceptional/reducible $\pi$ carry a genuine
...
$\dim\varphi_\tau<\dim\Sym^r\varphi_\pi$ that $\operatorname{Hom}$-space is zero
(\texttt{no\_nonzero\_hom\_of\_finrank\_lt}), so there is no trivial channel, $R_{r,\tau}=0$, and $\Lambda$ is
...
companion paper \cite{HodgeCompanion} --- not proved here. \emph{Initial results appear to affirm} this
...
full correspondence: a
from-scratch triple-product central derivative landing the Ceresa height, the exact coupling that induces
each higher Griffiths layer, and the ledger$\,\leftrightarrow\,$Bloch--Beilinson-graded bijection. (Status
at this draft: the depth dictionary and semisimple theorem are machine-checked there; the Ceresa
computation's $L$-side is on record --- the Klein quartic's Ceresa channel carries forced sign $-1$ and
central derivative $0.8299\ne0$, matching the known non-torsion of the cycle --- with the height side
open; and the first unnamed rung, grade four, is mapped and crossed on the carrier.) A
...
paper's target \cite{HodgeCompanion}. \\
