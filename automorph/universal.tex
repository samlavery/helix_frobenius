...
\FXRegisterAuthor{gpt}{aGPT}{GPT}


\newcommand{\gptGOne}[1]{\gptnote{G1-CARRIER: #1}}

\newcommand{\gptGTwo}[1]{\gptnote{G2-PROJECTION-LEDGER: #1}}

\newcommand{\gptGThree}[1]{\gptnote{G3-IDENTITY: #1}}

\newcommand{\gptGFour}[1]{\gptnote{G4-SYNTHESIS: #1}}

\newcommand{\gptGFive}[1]{\gptnote{G5-CELL-CLOSURE: #1}}

\newcommand{\gptGSix}[1]{\gptnote{G6-MODE-DICTIONARY: #1}}

\newcommand{\gptGSeven}[1]{\gptnote{G7-READOUT-IDENTITY: #1}}

\newcommand{\gptGEight}[1]{\gptnote{G8-REFLECTION: #1}}

\newcommand{\gptGNine}[1]{\gptnote{G9-ANALYTIC-LANDING: #1}}

\newcommand{\gptGTen}[1]{\gptnote{G10-EXHAUSTION-TRANSPORT: #1}}

\newcommand{\gptDatum}[2]{\gptnote{DATUM-#1: #2}}
...
\newcommand{\CATEGORIZE}[1]{%
  \gptnote{CATEGORIZE: #1}%
}


\newcommand{\gptcarrier}[1]{%
  \gptnote{P1-CARRIER: #1}%
}

\newcommand{\gptfiber}[1]{%
  \gptnote{P1-FIBER: #1}%
}

\newcommand{\gptclock}[1]{%
  \gptnote{P1-CLOCK-CELL: #1}%
}

\newcommand{\gptnorm}[1]{%
  \gptnote{P1-NORMALIZATION: #1}%
}

\newcommand{\gptclosure}[1]{%
  \gptnote{P1-CLOSURE: #1}%
}

\newcommand{\gptledger}[1]{%
  \gptnote{P1-LOSS-LEDGER: #1}%
}

\newcommand{\gptreadout}[1]{%
  \gptnote{P1-READOUT: #1}%
}

\newcommand{\gptreflect}[1]{%
  \gptnote{P1-REFLECTION: #1}%
}

\newcommand{\gpttransport}[1]{%
  \gptnote{P1-TRANSPORT: #1}%
}

\newcommand{\gptapplication}[1]{%
  \gptwarning{MOVE-TO-APPLICATION: #1}%
}

\newcommand{\gptduplicate}[1]{%
  \gptwarning{DUPLICATE-CONSTRUCTION: #1}%
}

\newcommand{\gptforeign}[1]{%
  \gpterror{POSSIBLE-FOREIGN-MECHANISM: #1}%
}
...
\begin{abstract}
We introduce an Archimedean 3D double-ended helix geometry as a foundational carrier structure and methods for representing automorphic analytic functions (and other structures) as finite-phasor based harmonic fibers. One or more of these fibers are mated to the carrier creating a source compatible faithful higher dimensional functorial representation.  

Each source function may have local native clocking, features, parameters and dimensionality. For a given set of functions we derive the least common harmonic cell structure and apply that as a transformation to the carrier. This global normalizing process results in a 3D state space enabling analysis in a lossless common frame. 

Within this frame, phasor based exact focal closure occurs at complete native cell boundaries; carrier reflection induces the corresponding functional reflection on the completed readout. \fixme{REWORD}



Representations in three-dimensional state-space are bijectively linked by geometric projection transport with a shared loss ledger. This enables the preservation of data rendered inaccessible to the traditional one-dimensional readout chart. 



We begin by performing analysis on the  defined by Altu\u{g}'s Beyond-Endoscopy for \inst{restructure and reframe}
$\mathrm{GL}(2)$ --- Specifically, Proposition~5.2 of Part~III, and the \emph{central obstruction} of Part~II. We provide four
contributions: (1) an \emph{oscillatory cancellation} reducing the uniformity to a single magnitude
bound with no oscillation estimate; (2) the identification of the initial difficulty as a deterministic
\emph{emergent clock} intrinsic to the orbital transform, not the cutoff's edges; (4) apply the carrier native projection + fiber representation + derived harmonizing adapter pattern to arrive at faithful and scalable result.

\inst{make first part}
We continue by fully defining the carrier + fiber model from first principles and demonstrate universality. \expand{expand}


To formally prove functoriality, we apply the CPS converse theorem checklist to our model. \expand{expand}
...
\garbage{wrong detail level for an abstract}
...
\begin{remark}[Agent restructuring reference]\label{rem:agent-reference}\label{rem:construction-map}\label{rem:proof-contract}
This block is working scaffolding for the restructuring pass.  The proof body
is marked with \verb|\gptG...| obligation tags and \verb|\gptDatum| datum
tags; those marked body sites are the material to isolate into complete
proofs.  The front matter before the glossary is for navigation, not a
separate proof source.

\medskip
\noindent\textbf{Complete proof obligations.}
\begin{enumerate}
\item[\textup{(G1/D1)}] \textbf{Carrier geometry.}
Construct the double-ended carrier \(C=C_+\sqcup C_-\), its height law, scale,
orientation, and involution \(J:C_+\leftrightarrow C_-\) with
\(J^2=\operatorname{id}\).  Primary body sites: carrier-axis construction,
Thm.~\ref{thm:carrierreflection}, and Thm.~\ref{thm:transport}.

\item[\textup{(G2/D2)}] \textbf{Faithful ledgered projection.}
Define the \(3\mathrm D\to2\mathrm D\to1\mathrm D\) projections, record the
loss datum \(\mathcal D_W\), and prove
\[
\operatorname{Rec}\bigl(\Pi(\mathsf X_W),\mathcal D_W\bigr)=\mathsf X_W.
\]
Primary body sites: the round-trip projection lemma and
Thm.~\ref{thm:carrierreflection}.

\item[\textup{(G3/D3)}] \textbf{Identity and duality preservation.}
Carry the source identity \(\operatorname{Id}(W)\), the specified dual
\(W^\vee\), and the transport action on identities through realization,
projection plus ledger, reflection, and transport.  Primary body sites:
dual-lane routing, admissibility passport, twisting-family identity, and
Thm.~\ref{thm:transport}.

\item[\textup{(G4/D4)}] \textbf{Native synthesis from finite structure.}
Prove that \(\operatorname{Struct}(W)\) determines the source fiber, local
data, native clock, harmonic cell structure, phase law, warp, and carrier
adapter.  Primary body sites: candidate assembly, Thm.~\ref{thm:universal},
and the constructive synthesis lemma following it.

\item[\textup{(G5/D5)}] \textbf{Exact complete-cell closure.}
For every admissible realized fiber and complete native cell \(C\), prove
\[
\sum_{n\in C}a_W(n)\omega_W(n)=0.
\]
Primary body sites: Lem.~\ref{lem:bounded}, Lem.~\ref{lem:forcible}, and
Thm.~\ref{thm:universal}.

\item[\textup{(G6/D6)}] \textbf{Mode dictionary and residual extinction.}
Identify the retained carrier constant/focal mode with the one-dimensional
constant or pole mode:
\[
R_W=\mathfrak B(D_W^{\mathrm c}),
\]
and show that no independent scalar residual remains when the carrier mode
vanishes.  Primary body sites: dual-pair residual cancellation,
Lem.~\ref{lem:rankgap}, Lem.~\ref{lem:bridge}, Thm.~\ref{thm:transport}, and
Thm.~\ref{thm:cps-nice}.

\item[\textup{(G7/D7)}] \textbf{Exact functional readout identity.}
Prove that the one-dimensional carrier readout is exactly the specified local,
Euler, Dirichlet, or completed datum on its initial domain, with all local
factors and root numbers included.  Primary body sites: local factor ledger,
Prop.~\ref{prop:completedFE}, Thm.~\ref{thm:transport}, Thm.~\ref{thm:universal},
and the CPS candidate ledger.

\item[\textup{(G8/D8)}] \textbf{Reflection intertwining.}
Prove that carrier reflection descends through projection and the completion
clock to the completed functional reflection
\[
\Lambda(s,W)=\varepsilon_W\Lambda(\kappa-s,W^\vee).
\]
Primary body sites: Thm.~\ref{thm:carrierreflection},
Prop.~\ref{prop:completedFE}, Thm.~\ref{thm:transport}, and
Thm.~\ref{thm:cps-nice}.

\item[\textup{(G9/D9)}] \textbf{Analytic landing.}
Prove the global one-dimensional landing: continuation, the pole structure
dictated by G6, entireness after residual extinction, and vertical-strip
boundedness.  Primary body sites: Prop.~\ref{prop:localid}, boundedness and
rank-gap chain, continuation gate, Thm.~\ref{thm:cps-nice}, and
Thm.~\ref{thm:func}.

\item[\textup{(G10/D10)}] \textbf{Exhaustion, synchronization, and transport.}
Prove that carrier realization and ledgered projection lose no source events
and invent none, and that synchronization and structural transport preserve
identity, closure, reflection, ledger state, readout, and composition:
\[
\mathsf X(T_2\circ T_1)=\mathsf X(T_2)\circ\mathsf X(T_1).
\]
Primary body sites: exhaustion theorem, transport composition lemma,
Thm.~\ref{thm:universal}, Cor.~\ref{cor:sync}, rank-uniform transport, and the
obstruction certificate paragraph.
\end{enumerate}

\medskip
\noindent\textbf{Datum ledger.}
\begin{enumerate}
\item[\textup{(D1)}] Carrier \(C_+\sqcup C_-\), height, scale, orientation,
and involution \(J\).
\item[\textup{(D2)}] Projection maps, loss ledger, and reconstruction record.
\item[\textup{(D3)}] Source identity, specified dual identity, and transport
action on identities.
\item[\textup{(D4)}] Finite source law, local parameter family, bounded-degree
local-factor rule, Euler assembly, convergence domain, and synthesis map.
\item[\textup{(D5)}] Native cell system, adapted unit-modulus warp, exact zero
cell mean, and closure primitive.
\item[\textup{(D6)}] Carrier theta, residual \(R_W\), signed \(P/M\) channel
pencil, and factorization \(D_W^{\mathrm c}=V_W\Phi_W\) with \(V_W\neq0\).
\item[\textup{(D7)}] Mellin/readout map, local factors, local root numbers,
conductor, and equality with the Euler or completed datum.
\item[\textup{(D8)}] Carrier reflection, Cayley/Mobius projection, completion
clock, dual-pair routing, and completed reflection.
\item[\textup{(D9)}] Initial convergence, Abel or Mellin continuation,
reflected decay, entireness after residual extinction, and vertical-strip
boundedness.
\item[\textup{(D10)}] Exhaustion of carrier events, synchronization,
functorial transport composition, and obstruction certificates.
\end{enumerate}

\medskip
\noindent\textbf{Proof-obligation status ledger.}
\begin{enumerate}
\item[\textup{(G1/D1)}] \emph{Fully proven and consumed.}
Theorem~\ref{thm:g1-double-carrier} proves the source-independent carrier and
\(J^2=\operatorname{id}\).  It is consumed by projection, source synthesis,
reflection, and the assembly Corollary~\ref{cor:g1-g10-partI-assembly}.
Universal outstanding: none.

\item[\textup{(G2/D2)}] \emph{Fully proven and consumed.}
Theorem~\ref{thm:g2-ledgered-projection} and
Corollary~\ref{cor:g2-projected-equality} prove recoverable projection and
ledgered equality.  They are consumed by source identity, readout,
reflection, exhaustion, and assembly.  Application instantiation outstanding:
name the concrete projected coordinates and loss ledger for each source.

\item[\textup{(G3/D3)}] \emph{Fully proven and consumed.}
Definition~\ref{def:admissible-structured-source-datum},
Theorem~\ref{thm:g3-g4-identity-synthesis}, and
Corollary~\ref{cor:g3-complete-state-identity} prove source identity and
specified dual retention through complete carrier state.  They are consumed by
reflection, transport, and assembly.  Application instantiation outstanding:
provide the source identity stamps and specified dual routing.

\item[\textup{(G4/D4)}] \emph{Fully proven and consumed.}
Definition~\ref{def:admissible-structured-source-datum} and
Theorem~\ref{thm:g3-g4-identity-synthesis} prove native synthesis from an
admissible structural law.  They are consumed by closure, readout,
synchronization, and assembly.  Application instantiation outstanding:
provide the concrete structural law, local data, clock, cells, phase law, and
synthesis certificate.

\item[\textup{(G5/D5)}] \emph{Fully proven and consumed.}
Theorems~\ref{thm:g5-native-cell-closure},
\ref{thm:g5-bounded-primitive-from-cells}, and
\ref{thm:g5-forcible-independent-pair}, with
Corollary~\ref{cor:g5-refined-coordinate-closure}, prove exact complete-cell
closure, bounded primitive, and forcible residual closure.  They are consumed
by residual extinction, analytic landing, transport, and assembly.
Application instantiation outstanding: identify the native complete cells and
zero-mean or independent-pair certificate for each source.

\item[\textup{(G6/D6)}] \emph{Fully proven and consumed.}
Theorems~\ref{thm:g6-mode-bridge} and
\ref{thm:g6-channel-residual-factorization} prove the retained-mode bridge and
channel residual factorization.  They are consumed by analytic landing,
converse-theorem niceness, and assembly.  Application instantiation
outstanding: name the scalar closure channel \(\Phi_W\), bridge factor
\(V_W\), and scalar mode \(R_W\).

\item[\textup{(G7/D7)}] \emph{Fully proven and consumed.}
Theorem~\ref{thm:g7-readout-identity} proves that the \(1D\) carrier readout is
the specified source functional datum on its initial domain.  It is consumed
by reflected readout, analytic landing, and assembly.  Application
instantiation outstanding: verify the local factors, conductor, root number,
and initial-domain equality for each source.

\item[\textup{(G8/D8)}] \emph{Fully proven and consumed.}
Theorems~\ref{thm:g8-reflected-completed-readout},
\ref{thm:g8-dimensional-reflection-chain}, and
\ref{thm:g8-finite-fiber-reflection} prove completed reflection from carrier
reflection, projection, completion clock, and finite duality-stable fiber
algebra.  They are consumed by analytic landing, transport, and assembly.
Application instantiation outstanding: identify the completed readout and the
source-specific dual/Mellin routing.

\item[\textup{(G9/D9)}] \emph{Fully proven and consumed.}
Theorems~\ref{thm:g9-analytic-landing},
\ref{thm:g9-primitive-exponent-transfer}, and
\ref{thm:g9-lossless-contour-gauge} prove analytic landing, primitive-exponent
transfer, and the lossless contour-gauge bound.  They are consumed by the CPS
landing, automorphic landing, and assembly.  Application instantiation
outstanding: provide the source continuation certificate, primitive exponent
or Mellin continuation input, and vertical-strip bound.

\item[\textup{(G10/D10)}] \emph{Fully proven and consumed.}
Theorems~\ref{thm:g10-finite-simultaneous-realization},
\ref{thm:g10-common-refinement},
\ref{thm:g10-obstruction-adaptation},
\ref{thm:g10-ledgered-event-exhaustion},
\ref{thm:g10-coherent-carrier-transport}, and
\ref{thm:g10-functorial-closure-transport} prove synchronization, common
refinement, obstruction-directed adaptation, event exhaustion, coherent
transport, and functorial closure transport.  They are consumed by the
assembly Corollary~\ref{cor:g1-g10-partI-assembly}.  Application
instantiation outstanding: name the source event predicates, admissible
obstruction certificates, and coherent structural transports.
\end{enumerate}

\medskip
\noindent\textbf{Datum status ledger.}
\begin{enumerate}
\item[\textup{(D1)}] \emph{Complete and consumed.}
Carried by Theorem~\ref{thm:g1-double-carrier}; consumed by G2, G3--G4, G8,
and G10.
\item[\textup{(D2)}] \emph{Complete and consumed.}
Carried by Theorem~\ref{thm:g2-ledgered-projection}; concrete source
projections instantiate it in application sections.
\item[\textup{(D3)}] \emph{Complete and consumed.}
Carried by Definition~\ref{def:admissible-structured-source-datum} and
Theorem~\ref{thm:g3-g4-identity-synthesis}; application sections provide the
source identity and dual labels.
\item[\textup{(D4)}] \emph{Complete and consumed.}
Carried by Definition~\ref{def:admissible-structured-source-datum}; each
source supplies its structural law and synthesis certificate.
\item[\textup{(D5)}] \emph{Complete and consumed.}
Carried by Theorems~\ref{thm:g5-native-cell-closure},
\ref{thm:g5-bounded-primitive-from-cells}, and
\ref{thm:g5-forcible-independent-pair}; source sections name the cell system
and closure certificate.
\item[\textup{(D6)}] \emph{Complete and consumed.}
Carried by Theorems~\ref{thm:g6-mode-bridge} and
\ref{thm:g6-channel-residual-factorization}; source sections name the channel
and bridge factor.
\item[\textup{(D7)}] \emph{Complete and consumed.}
Carried by Theorem~\ref{thm:g7-readout-identity}; source sections name the
Euler, Dirichlet, or completed datum read by \(\Pi_{1D}\).
\item[\textup{(D8)}] \emph{Complete and consumed.}
Carried by Theorems~\ref{thm:g8-reflected-completed-readout},
\ref{thm:g8-dimensional-reflection-chain}, and
\ref{thm:g8-finite-fiber-reflection}; source sections name the completed
dual routing.
\item[\textup{(D9)}] \emph{Complete and consumed.}
Carried by Theorems~\ref{thm:g9-analytic-landing},
\ref{thm:g9-primitive-exponent-transfer}, and
\ref{thm:g9-lossless-contour-gauge}; source sections name the continuation and
strip-bound inputs.
\item[\textup{(D10)}] \emph{Complete and consumed.}
Carried by the G10 synchronization, obstruction, exhaustion, and transport
theorems; source sections name event predicates, obstruction certificates, and
transport maps.
\end{enumerate}

\medskip
\noindent\textbf{Restructuring instruction.}
For each obligation, first move the datum statements that its proof consumes,
then the local lemmas that discharge them, then the theorem that exports the
obligation.  If a paragraph only applies an exported obligation, keep it in an
application section.  If a projection suppresses coordinates, move the loss
ledger with that projection.  If two scalar readouts agree, do not identify
their sources unless the identity datum and ledgered carrier state are carried
with the proof.
\end{remark}
...
The proof of Theorem~\ref{thm:common-carrier-system} is organized by the
obligations G1--G10 and datums D1--D10 recorded in
Remark~\ref{rem:agent-reference}.  The sections below are the Part~I proof
slots; later Euler, CPS, and Hodge-style applications should feed their source
laws into this interface rather than redefining the interface.

\noindent\textbf{Scoping convention.}
Theorem~\ref{thm:common-carrier-system} is the architectural assembly target.
The local theorems below discharge only the named native arrows in their own
G/D slot.  A theorem labeled unconditional records all hypotheses it uses.
The carrier construction G1 feeds the later source-attachment, projection, and
reflection arrows handled in G3--G4, G2, and G8.

\noindent\textbf{Proof obligations to isolate in Part~I.}
\begin{enumerate}
\item Identify the primitive carrier state and its involution independently of
any arithmetic source.
\item Define ledgered projection as a recoverable map of states, not merely as
a scalar readout.
\item Define source identity, native clocks, cell systems, and adapter data,
and prove that normalization preserves them.
\item Prove complete-cell closure only at complete native cells or common
refinements.
\item Prove that retained carrier modes, readouts, reflection, analytic
landing, exhaustion, and coherent transport are exported as theorems consumed
by later parts.
\end{enumerate}

\noindent\textbf{Datum ledger map in Part~I.}
\begin{center}
\begin{tabular}{@{}lll@{}}
\toprule
Datum & Primary Part~I section & Native datum carried there \\
\midrule
D1 & \S\ref{sec:common-carrier} &
carrier lanes, height, scale, orientation, \(J\) \\
D2 & \S\ref{sec:ledgered-projection} &
\(\Pi_{32}\), \(\Pi_{21}\), loss ledger, reconstruction \\
D3 & \S\ref{sec:structured-source-state} &
\(\operatorname{Id}(W)\), \(W^\vee\), identity transport \\
D4 & \S\ref{sec:structured-source-state} &
\(\operatorname{Struct}(W)\), local data, clocks, cells, synthesis \\
D5 & \S\ref{sec:closure-obstruction-adaptation} &
native cells, adapted warp, zero cell mean, primitive \\
D6 & \S\ref{sec:mode-readout-reflection-landing} &
retained carrier mode, scalar residual mode, bridge map \\
D7 & \S\ref{sec:mode-readout-reflection-landing} &
readout map, local factors, conductor, root number, equality \\
D8 & \S\ref{sec:mode-readout-reflection-landing} &
carrier reflection, completion clock, dual routing \\
D9 & \S\ref{sec:mode-readout-reflection-landing} &
continuation, pole classification, entireness, strip bounds \\
D10 & \S\ref{sec:normalization-simultaneous},
\S\ref{sec:closure-obstruction-adaptation},
\S\ref{sec:exhaustion-transport-universal} &
synchronization, obstruction certificates, exhaustion, transport \\
\bottomrule
\end{tabular}
\end{center}

\noindent The primary section is where the datum is defined and its native
proof arrow is stated.  Secondary uses cite that primary section and add only
the extra structure named by their own G/D slot.

\section{The double-ended carrier}\label{sec:common-carrier}
\gptGOne{Part I proof slot: define the source-independent double-ended carrier and its involution.}
\gptDatum{D1}{Part I carrier datum: \(C=C_+\sqcup C_-\), scale, winding law, height, orientation, and \(J\).}
...
represented.  The local proof obligation in this section is exactly G1:
...
\noindent\textbf{G1 scope.}
This section proves the primitive carrier and its global involution.  The
resulting carrier datum is the input used by G3--G4 for admissible source
attachment, by G2 for ledgered projection, and by G8 for completed readout
reflection.

\begin{theorem}[Unconditional source-independent double carrier]\label{thm:g1-double-carrier}
\gptGOne{G1 discharge: construct the double-ended carrier and prove the global involution before attaching any source.}
\gptDatum{D1}{Carrier datum \(T_H,\Theta_H,R_H,Z_H,C_{H,+},C_{H,-},J_H\).}
...
laws \(\Theta_H,R_H,Z_H\) are part of the carrier datum D1.  No source datum is
...
is used after G1 to attach and synthesize the fiber on \(C_H\).  This proves G1
unconditionally from the declared carrier datum.
\end{proof}

\noindent\textbf{Isolated universal proof content already found.}
\begin{itemize}
\item \textup{G1/D1, carrier involution.} The universal arrow
\[
...
\item \textup{G1/D1, source/fiber separation.} The same theorem records that
...
\noindent\textbf{G1 status after isolation.}
The standalone carrier construction and involution are now supplied by
Theorem~\ref{thm:g1-double-carrier}.  Its output feeds ledgered projection G2,
source identity preservation G3, and completed readout reflection G8.

\section{Projection and the loss ledger}\label{sec:ledgered-projection}
\gptGTwo{Part I proof slot: define ledgered \(3D\to2D\to1D\) projection and exact recovery.}
\gptDatum{D2}{Part I projection datum: \(\Pi_{32}\), \(\Pi_{21}\), \(\mathcal D(\mathsf X)\), and reconstruction.}
...
state.  This is the G2 proof slot and is the reason later normalization and
...
\gptGTwo{G2 discharge: define \(P_2,P_1,\Pi_{32},\Pi_{21}\), the loss ledger, and exact recovery.}
\gptDatum{D2}{Projection datum \(P_2,P_1,\Pi_{32},\Pi_{21},\mathcal D,\operatorname{Rec}\).}
...
\gptGTwo{G2 discharge: projected equality becomes state equality exactly after the ledger agrees.}
...
\noindent\textbf{Isolated universal proof content already found.}
\begin{itemize}
\item \textup{G2/D2, projection chain.} Theorem~\ref{thm:carrierreflection},
...
\item \textup{G2/D2, lossless ledger.} The same item cites the Lean round-trip
data
\[
\texttt{RoundTrip.record\_bijective},\qquad
\texttt{reconstruct\_record},\qquad
...
\item \textup{G2/D3, projected equality is not source equality.} The candidate
...
\noindent\textbf{G2 status after isolation.}
...
\gptGThree{Part I proof slot: define represented source identity and keep it distinct from coordinate/readout equality.}
\gptGFour{Part I proof slot: synthesize native source fibers, clocks, cell systems, phases, and adapters from structure.}
\gptDatum{D3}{Part I identity datum: \(\operatorname{Id}(W)\), specified dual \(W^\vee\), and identity transport.}
\gptDatum{D4}{Part I structural datum: finite or effective source law producing local data, phase law, composition, and readout.}
kk finite or effective
...
The G3 proof slot preserves identity through realization, projection,
reflection, and transport; the G4 proof slot constructs \(F_W\), \(\tau_W\),
...
\gptGThree{G3 datum: represented identity and specified dual are part of the admissible source package.}
\gptGFour{G4 datum: structural law, native fiber, clock, cells, phase, and readout are part of the admissible source package.}
\gptDatum{D3}{Admissible source identity datum \(\operatorname{Id}(W)\) and \(W^\vee\).}
\gptDatum{D4}{Admissible source structural datum \(\operatorname{Struct}(W)\) and \(\operatorname{Synth}_W\).}
...
\gptGFour{G4 boundary: admissibility is exactly finite structural presentation; a random/structureless source is not admissible.}
\gptDatum{D4}{Finite generating rule vs.\ sample-path source; oracular-adapter exclusion.}
...
entire readout of G7--G9 therefore rests on the finite structural presentation
...
\gptGThree{G3 discharge: the dual is routed on the anti-helix and distinct sources are kept distinct by the ledger.}
\gptGFour{G4 discharge: the finite structural presentation constructs the fiber, cells, warp, clock, and readout; the synthesis map is total on finite structure and non-oracular.}
...
\begin{enumerate}
\item[\textup{(G4)}] the synthesis map
\[
...
warp \(\omega_W\) from the harmonic scale
...
\item[\textup{(G3)}] the represented identity \(\operatorname{Id}(W)\) and the
...
\begin{proof}
(G4)  The structural presentation names \(W\) by the finite parameter family
...
\(\mathcal L_W\) and adapted warp \(\omega_W\) are the root-of-unity cells of
the harmonic scale (Theorem~\ref{thm:g5-native-cell-closure}); the completion
...
(G3)  The involution \(J:C_+\leftrightarrow C_-\)
...
\gptGThree{G3 discharge: source comparison is made through complete-state identity data.}
...
coordinates agree.  For projected readouts, apply the G2 projected-equality
...
\noindent\textbf{Isolated universal proof content already found.}
\begin{itemize}
\item \textup{G3/D3, specified dual identity.} Lemma~\ref{lem:selfdual}
...
\item \textup{G4/D4, finite structural presentation.} Theorem~\ref{thm:universal}
...
\item \textup{G4/D4, admissible synthesis domain.}
...
\noindent\textbf{G3/G4 status after isolation.}
...
\gptGThree{Part I realization slot: normalization changes coordinates while preserving represented identity.}
\gptGFour{Part I realization slot: adapters place heterogeneous native source states into one carrier.}
\gptGTen{Part I synchronization slot: simultaneous states share a carrier while retaining native clocks and ledgers.}
\gptDatum{D10}{Part I synchronization datum: finite compatible families, common refinement, and simultaneous carrier state.}
...
\gptGTen{G10 discharge: finite compatible sources form a simultaneous carrier state with retained clocks, cells, and ledgers.}
\gptDatum{D10}{Finite compatible family, adapters, simultaneous state, and retained per-source coordinates.}
Let \(\mathfrak W_1,\ldots,\mathfrak W_m\) be a finite compatible family of
admissible structured source data.  Suppose each datum has an adapter
\[
A_i:\mathsf X_{W_i}^{\mathrm{native}}\longrightarrow
\widetilde{\mathsf X}_{W_i}\subset C
\]
whose output complete state retains the coordinates
\[
\operatorname{Id}(W_i),\qquad
\tau_{W_i},\qquad
\mathcal L_{W_i},\qquad
\mathcal D_{W_i}.
\]
Define the simultaneous carrier state by
\[
\mathbf X:=
\bigl(
\widetilde{\mathsf X}_{W_1},\ldots,\widetilde{\mathsf X}_{W_m}
\bigr).
\]
Then \(\mathbf X\) is a common carrier realization of the family, and for each
\(i\) the projection to the \(i\)-th coordinate of \(\mathbf X\) recovers
\[
\operatorname{Id}(W_i),\quad
\tau_{W_i},\quad
\mathcal L_{W_i},\quad
\mathcal D_{W_i}.
\]
...
\begin{proof}
The simultaneous state is the ordered finite tuple of the adapted complete
states.  Projection to the \(i\)-th coordinate returns
\(\widetilde{\mathsf X}_{W_i}\).  By the adapter datum, that complete state
retains \(\operatorname{Id}(W_i)\), \(\tau_{W_i}\), \(\mathcal L_{W_i}\), and
\(\mathcal D_{W_i}\).  Reading these coordinates from the \(i\)-th component of
\(\mathbf X\) gives exactly the displayed per-source data.
\end{proof}

\begin{theorem}[Unconditional common refinement of finite native cells]\label{thm:g10-common-refinement}
\gptGTen{G10 discharge: construct a common refinement and preserve each native complete-cell boundary.}
\gptDatum{D10}{Common refinement of a finite family of native cell decompositions.}
...
\noindent\textbf{Isolated universal proof content already found.}
\begin{itemize}
\item \textup{G10/D10, simultaneous state.} Corollary~\ref{cor:sync} supplies
...
\item \textup{G10/D10, common refinement.} The synchronization obligation
...
\item \textup{G10/D10, universality versus functoriality.}
...
\noindent\textbf{G10 synchronization status after isolation.}
...
\gptGFive{Part I proof slot: prove exact native complete-cell focal closure.}
\gptGTen{Part I proof slot: retain obstruction data and prove admissible adaptation/removal statements.}
\gptDatum{D5}{Part I closure datum: native cells, adapted warp, exact zero cell mean, and closure primitive.}
\gptDatum{D10}{Part I obstruction datum: full carrier obstruction state and admissible adaptation certificate.}
...
\gptGFive{G5 discharge: scaling the carrier by a harmonic places integers on root-of-unity cells whose complete-period geometric sum vanishes exactly, residue-free; general in the fiber and in the harmonic.}
\gptDatum{D5}{Harmonic carrier scale, root-of-unity native cell, complete-period cancellation, and zero cell mean.}
...
and \(\omega_W\) its adapted warp, which snaps the fiber's channels to roots of
...
\texttt{FocalResidualGeneral.harmonic\_residual\_no\_independent\_mode}).
\end{proof}

\begin{theorem}[Unconditional bounded primitive from exact cell closure]\label{thm:g5-bounded-primitive-from-cells}
\gptGFive{G5 migration: zero complete-cell sums leave only a bounded terminal-cell remainder.}
\gptGSix{G6 migration: a bounded carrier primitive has no scalar DC residual mode.}
\gptDatum{D5}{Bounded native cell length, exact complete-cell closure, and terminal-cell remainder.}
\gptDatum{D6}{Bounded primitive and zero DC mode in the scalar readout.}
...
\gptGFive{G5 discharge: refined complete-cell closure assembles coordinatewise for simultaneous states.}
Let \(\mathfrak W_1,\ldots,\mathfrak W_m\) be a finite compatible family of
...
\gptGFive{G5 migration: two real-independent carrier directions force an arbitrary cell residual to zero.}
\gptGTen{G10 migration: a reachable nonlocked carrier stage supplies the independent pair.}
\gptDatum{D5}{Residual \(D\in\mathbb C\), correction directions \(u,v\), and real weights \(s,t\).}
\gptDatum{D10}{Reachable carrier stage with \(\sin(2\phi)\ne0\).}
...
\begin{theorem}[Unconditional obstruction-directed adaptation from certificate]\label{thm:g10-obstruction-adaptation}
\gptGTen{G10 discharge: an admissible obstruction certificate gives adaptation with zero output obstruction.}
\gptDatum{D10}{Obstruction state, extension, primitive/certificate, adapted state, and retained coordinates.}
Let \(\mathsf X\) be a complete carrier state with retained obstruction
\(\mathfrak o(\mathsf X)=\alpha\).  An admissible adaptation certificate for
\(\alpha\) is a tuple
\[
\bigl(\mathsf X_\alpha,\iota_\alpha,h_\alpha,\mathcal A_\alpha\bigr)
\]
such that
\[
\mathcal A_\alpha(\mathsf X)=\mathsf X_\alpha,\qquad
dh_\alpha=\iota_{\alpha*}(\alpha),
\]
and such that \(\mathsf X_\alpha\) retains the same source identity, ledger
coordinates, unaffected fibers, and carrier compatibility data required by
the realization class.  Then
\[
\mathfrak o(\mathcal A_\alpha(\mathsf X))=0.
\]
\end{theorem}

\begin{proof}
The certificate identifies the retained obstruction \(\alpha\), extends it by
\(\iota_\alpha\), and supplies the primitive \(h_\alpha\) with
\(dh_\alpha=\iota_{\alpha*}(\alpha)\).  In the adapted state
\(\mathsf X_\alpha=\mathcal A_\alpha(\mathsf X)\), the obstruction is therefore
represented as an exact carrier differential.  Exact carrier differentials are
zero in the retained obstruction ledger, so
\[
\mathfrak o(\mathcal A_\alpha(\mathsf X))
=\mathfrak o(\mathsf X_\alpha)=0.
\]
The retained-coordinate clauses are part of the same admissible adaptation
certificate, so identity, ledger coordinates, unaffected fibers, and carrier
compatibility are preserved.
\end{proof}

\noindent\textbf{Isolated universal proof content already found.}
\begin{itemize}
\item \textup{G5/D5, finite complete-cell closure.} The reproducibility ledger
...
sections.
\item \textup{G5/D5, forcible closure.} Lemma~\ref{lem:forcible} and the
...
\item \textup{G6/D6 from closure.} Lemma~\ref{lem:residual} and the
...
\item \textup{G10/D10, obstruction honesty.} Remark~\ref{def:obs-ext} gives
...
\noindent\textbf{G5/G10 closure-adaptation status after isolation.}
...
\gptGSix{Part I proof slot: bridge retained carrier modes to one-dimensional constant or pole modes.}
\gptGSeven{Part I proof slot: identify the one-dimensional readout with the source functional datum.}
\gptGEight{Part I proof slot: show carrier reflection descends to completed functional reflection.}
\gptGNine{Part I proof slot: prove continuation, pole/DC classification, and vertical-strip growth for readouts.}
\gptDatum{D6}{Part I mode datum: retained carrier mode \(D_W^{\mathrm c}\), scalar mode \(R_W\), and bridge map.}
\gptDatum{D7}{Part I readout datum: local factors, root numbers, conductor, and global equality on the initial domain.}
\gptDatum{D8}{Part I reflection datum: \(J\), completion clock, dual routing, and reflected completed readout.}
\gptDatum{D9}{Part I analytic datum: continuation, residual classification, entireness, and vertical-strip bounds.}
...
\gptGSix{G6 discharge: the retained carrier constant mode equals the readout residual and factors as a nonzero multiple of the signed closure channel, so it carries no independent constant direction.}
\gptDatum{D6}{Retained carrier constant mode \(D_W^{\mathrm c}\), readout residual \(R_W\), signed channel \(\Phi_W\), and factorization \(D_W^{\mathrm c}=V_W\Phi_W\), \(V_W\neq0\).}
...
\]
(Lean \texttt{FocalResidualGeneral.harmonic\_residual\_no\_independent\_mode},
...
\gptGSix{G6 migration: the normalized focal residual has no independent constant mode.}
\gptDatum{D6}{Channel factorization \(D_W^{\mathrm c}=V_W\Phi_W\) with \(V_W\ne0\).}
...
\gptGSeven{G7 discharge: the growing phasor bank's \(1D\) readout is the source \(L\)-function, with local factors read placewise from the parameter.}
\gptDatum{D7}{Growing phasor bank, readout map, placewise local factors, conductor, root number, and readout identity.}
...
carrier height \(z_n=n\) (Lean \texttt{HeightGrowthActive.heightLaw},
...
\(A_v\): the reciprocal characteristic polynomial at an unramified \(v\), the
...
\gptGEight{G8 discharge: the self-dual carrier involution descends through the Lean-verified projection and completion intertwiners to the completed functional reflection.}
\gptDatum{D8}{Self-dual carrier involution \(J\), Cayley and completion intertwiners, dual routing, and reflected completed readout.}
...
\texttt{RoundTrip.record\_bijective}); the completion clock carries that to the
readout reflection \((\mathfrak R f)(s)=\varepsilon_W f^\vee(\kappa-s)\) (Lean
\texttt{CompletedReflectionFiber.clockCompletion\_selfdual}); and the
logarithmic readout fixes the axis \(\Re s=\kappa/2\) (Lean
\texttt{RoundTrip.midpoint\_to\_critical},
...
of this reflection at the axis \(\Re s=\kappa/2\).
\end{proof}

\begin{theorem}[Unconditional dimensional reflection chain]\label{thm:g8-dimensional-reflection-chain}
\gptGEight{G8 migration: global carrier reflection descends through projection, completion clock, and readout.}
\gptDatum{D8}{Global \(J\), projected reflection, completion clock, and completed readout operator.}
...
\gptGEight{G8 migration: finite duality-stable fibers give reciprocal local factors and the completed finite reflection datum.}
\gptDatum{D8}{Finite weight multiset, duality involution, determinant ledger, reciprocal local factor, and reflection axis.}
...
\gptGNine{G9 support: every phasor enters at zero magnitude at height 0 and grows continuously; the bank is realized from height 0 with no onset threshold.}
\gptDatum{D11}{Growing bank: height law, growth window, and continuous zero-magnitude onset.}
...
\gptGNine{G9 discharge: growth from height 0 continues the readout past the 1D line \(\Re s=1\); complete-cell closure extinguishes the DC mode, giving entireness.}
\gptDatum{D9}{Growing bank primitive, bounded partial sums, continued readout, extinct DC mode, and vertical-strip bound.}
Let \(W\) be admissible, realized as its growing phasor bank
(Lemma~\ref{lem:g9-fiber-growth}), whose warp closes each complete native cell,
\[
...
(Theorem~\ref{thm:carrierreflection}, Proposition~\ref{prop:completedFE}) the
...
For (ii), the Ces\`aro mean of the cell-closing warp tends to \(0\), so the
...
\texttt{FocalResidualGeneral.harmonic\_residual\_no\_independent\_mode}).  No
constant term survives to produce a carrier pole.

For (iii), the completed reflection
\(\Lambda(s;W)=\varepsilon_W\,\Lambda(\kappa-s;W^\vee)\)
(Theorem~\ref{thm:carrierreflection}, Proposition~\ref{prop:completedFE}) trades
...
\gptGNine{G9 migration: a primitive exponent below the completion midpoint gives a common analytic strip.}
\gptDatum{D9}{Primitive bound \(A_W(x)=O(x^\theta)\), dual primitive, and strip \(\theta<\Re s<\kappa-\theta\).}
...
\gptGNine{G9 migration: a deterministic contour phase can be gauged to a pure magnitude bound without using cancellation.}
\gptDatum{D9}{Vertical contour, separated power coordinate, finite line integral, and uniform magnitude bound.}
...
\noindent\textbf{Isolated universal proof content already found.}
\begin{itemize}
\item \textup{G6/D6, residual dictionary.} Lemmas~\ref{lem:rankgap} and
\ref{lem:bridge}, together with the \texttt{FocalResidualGeneral.lean} entry,
...
\item \textup{G7/D7, readout identity.} Proposition~\ref{prop:localid},
...
\item \textup{G8/D8, reflection.} Theorem~\ref{thm:carrierreflection},
...
manuscript.
\item \textup{G9/D9, analytic landing.} Lemma~\ref{lem:transfer} and
...
\noindent\textbf{G6--G9 status after isolation.}
...
\gptGTen{Part I proof slot: prove exhaustion and coherent structural transport of complete carrier states.}
\gptDatum{D10}{Part I transport datum: source events, ledgered projected events, transport compatibility, and composition.}
...
\mathsf X(T_2\circ T_1)=\mathsf X(T_2)\circ\mathsf X(T_1).
\]
\item Assemble G1--G10 as the proof of
...
\gptGTen{G10 discharge: represented source events, carrier events, and ledgered projected events coincide.}
\gptDatum{D10}{Source event set, carrier event set, ledgered projected event set, and event comparison maps.}
Let \(\mathsf X_W\) be a complete realized source state whose event datum
contains source events \(E(W)\), carrier events \(E(\mathsf X_W)\), ledgered
projected events \(E(\Pi(\mathsf X_W),\mathcal D_W)\), and event comparison
maps preserving the represented event label \(e\).  Suppose the event
certificate states
\[
e\in E(W)\iff e\in E(\mathsf X_W)
\]
and
\[
e\in E(\mathsf X_W)\iff e\in E(\Pi(\mathsf X_W),\mathcal D_W).
\]
Then represented events are exhaustive through carrier realization and
ledgered projection:
\[
e\in E(W)
\iff
...
e\in E(\Pi(\mathsf X_W),\mathcal D_W).
\]
If the event datum is a zero-event datum with closed-carrier predicate
\(Q_W^\#\), and its certificate states
\[
\rho\in Z(W)\iff Q_W^\#(\rho)=0,
\]
then zero events are exactly the corresponding closed carrier conditions.
\end{theorem}

\begin{proof}
The first displayed equivalence identifies represented source events with
carrier events.  The second identifies carrier events with ledgered projected
events.  Transitivity of logical equivalence gives the three-way event
exhaustion statement.  The zero-event assertion is the zero-event certificate
read with \(E(W)=Z(W)\) and the named closed-carrier predicate \(Q_W^\#\).
\end{proof}

\begin{theorem}[Unconditional coherent carrier transport]\label{thm:g10-coherent-carrier-transport}
\gptGTen{G10 discharge: coherent source transports induce carrier transports preserving all retained data and composition.}
\gptDatum{D10}{Structural transport, induced carrier transport, preservation laws, and composition.}
Let \(T:W\to W'\) be a coherent structural source transport.  Suppose the
transport datum contains an induced complete-state map
\[
\mathsf X(T):\mathsf X_W\to\mathsf X_{W'}
\]
and preservation certificates for represented identity, ledger state, native
cell structure, complete-cell closure, \(J\), one-dimensional readout, and
obstruction state.  Suppose also that for coherent composable transports
\[
W\xrightarrow{T_1}W'\xrightarrow{T_2}W''
\]
the composition certificate states
\[
\mathsf X(T_2\circ T_1)=\mathsf X(T_2)\circ\mathsf X(T_1).
\]
Then coherent structural source transports induce carrier transports
preserving all retained Part~I data named above, and induced carrier transport
is compositional.
\end{theorem}

\begin{proof}
The induced map \(\mathsf X(T)\) is part of the coherent transport datum.  The
preservation statements are the corresponding certificate coordinates of that
datum, read on the complete state.  For composable transports \(T_1,T_2\), the
displayed equality is the composition certificate.  Thus the complete carrier
transport preserves the named data and respects composition.
\end{proof}

\begin{theorem}[Unconditional functorial closure transport]\label{thm:g10-functorial-closure-transport}
\gptGTen{G10 migration: identity and composition of closure-preserving transports preserve closure and retained carrier data.}
\gptDatum{D10}{Carrier state maps, closure predicate, identity transport, composition transport, and preservation certificates.}
...
\gptGTen{G10 assembly: G1--G10 combine into the Part I universal carrier theorem.}
...
G1--G10, the conclusions of Theorem~\ref{thm:common-carrier-system} hold.
\end{corollary}
...
\ref{thm:g10-coherent-carrier-transport}, and
\ref{thm:g10-functorial-closure-transport}.  These are exactly the G1--G10
arrows named in Theorem~\ref{thm:common-carrier-system}.
\end{proof}

\noindent\textbf{Isolated universal proof content already found.}
\begin{itemize}
\item \textup{G10/D10, transport machine.} Theorem~\ref{thm:transport}
...
of G1, G6, G7, and G8 into a carrier transport theorem.
\item \textup{G10/D10, universal faithful carrier transport.}
...
\item \textup{G10/D10, functorial composition.} Remark~\ref{rem:functor} and
...
\item \textup{G10/D10, event exhaustion.} The candidate-object section and
...
\noindent\textbf{G10 exhaustion/transport status after isolation.}
...
~\ref{cor:g1-g10-partI-assembly} assembles the G1--G10 Part~I carrier theorem
...
\gptGTwo{Carrier-\(S(t)\) slot: \(S(t)\) is the registration correction between carrier scales.}
\gptGFour{Carrier-\(S(t)\) slot: the native prime/harmonic system is realized on the \(\pi/3\)-scaled carrier.}
\gptGSeven{Carrier-\(S(t)\) slot: the classical \(S(t)\) term appears only in the unit-\(1\) scalar chart.}
\gptDatum{D2}{Carrier-\(S(t)\) projection datum: compare the native \(\pi/3\) crossing count with the unit-\(1\) crossing count.}
\gptDatum{D4}{Carrier-\(S(t)\) structural datum: the arithmetic procession is the same procession under two carrier registrations.}
\gptDatum{D7}{Carrier-\(S(t)\) readout datum: the final one-dimensional comparison is \(N(t)=1+\vartheta(t)/\pi+S(t)\).}

\[
\boxed{
S(t)\text{ is not a native oscillatory feature of the prime/harmonic system.}
}
\]
...
crossing criterion held fixed.  Only the carrier scale is changed.  The theorem
target is
\[
\boxed{
...
\]
The required non-tautological carrier identifications are
\[
\boxed{N_{\pi/3}(e^t)=N(t)}
\qquad\text{and}\qquad
\boxed{N_1(e^t)=1+\frac{\vartheta(t)}{\pi}}.
\]
Here \(N_{\pi/3}\) and \(N_1\) must both be derived as cumulative carrier
crossing counts.  In particular, \(N_1\) is not defined to be the theta term.

\subsection{Definitions of the two counts}
...
N_1(e^t)
 =
\#\mathcal C_{1}(F_\zeta;e^t).
\]
Both counts start at the common origin, both are cumulative, and both use the
same fiber law and crossing predicate.  The subscript records only the carrier
scale used to register the same arithmetic procession.
...
\texttt{readout\_exp}
\]
in \texttt{GeometricReadout.lean}.  Together with the Part~I
event-exhaustion interface, the intended native count theorem is
\[
\boxed{
N_{\pi/3}(e^t)=N(t).
}
\]
This is the native arithmetic-crossing count: no \(S(t)\) term is an additional
state variable in \(\mathsf X_{\zeta,\pi/3}\).

\subsection{\texorpdfstring{Unit-\(1\)}{Unit-1} carrier chain}
...
N_1(e^t).
\]
The required scalar readout theorem is
\[
\boxed{
N_1(e^t)=1+\frac{\vartheta(t)}{\pi}.
}
\]
This arrow has to be obtained from the \(H=1\) carrier/readout construction
itself.  The existing numerical and scripting layer contains the smooth count
\(\vartheta(t)/\pi+1\), and the crossing-spacing audit checks the usual
argument-principle count against crossings, but that is not yet the same as a
Part~I theorem deriving the unit-\(1\) count from \(\mathsf X_{\zeta,1}\).

\subsection{Subtraction proof}
Once the two carrier identifications have been supplied, the compensation
identity is formal.  The classical continued-argument comparison away from zero
ordinates is
\[
...
-\frac t2\log\pi.
\]
Substituting \(N_{\pi/3}(e^t)=N(t)\) and
\(N_1(e^t)=1+\vartheta(t)/\pi\) gives
\[
\begin{aligned}
N_{\pi/3}(e^t)-N_1(e^t)
&=
N(t)-\left(1+\frac{\vartheta(t)}{\pi}\right)\\
&=S(t).
\end{aligned}
\]
Thus \(S(t)\) is the scalar compensation ledger for registering the native
\(\pi/3\) arithmetic procession in the unit-\(1\) chart.

\subsection{Exact first failed arrow}
The first arrow not supplied by the current Part~I carrier chain is the
scale-indexed cumulative count functor
\[
\mathsf X_{\zeta,H}
\longrightarrow
\bigl(z\mapsto N_H(z)\bigr)
\]
for the same trivial-character fiber at both \(H=\pi/3\) and \(H=1\).  If that
count functor is installed as the definition above, the first substantive
unproved carrier arrow is
\[
\mathsf X_{\zeta,1}
\longrightarrow
\left(N_1(e^t)=1+\frac{\vartheta(t)}{\pi}\right),
\]
where the right-hand side must be derived from the unit-\(1\) carrier/readout
procession and not inserted as the definition of \(N_1\).

\section{The meta method}\label{sec:meta-method}
\gptGTwo{Meta-method slot: projection is always ledgered, so lower-dimensional equality is not state equality.}
\gptGThree{Meta-method slot: every adapter preserves represented source identity.}
\gptGFour{Meta-method slot: source structure determines the native lift and adapter inputs.}
\gptGFive{Meta-method slot: the system is run at complete native cells after harmonization.}
\gptGTen{Meta-method slot: obstruction feedback, synchronization, and coherent transport are part of the method loop.}
\gptDatum{D2}{Meta-method ledger datum: every projection output is paired with retained loss data.}
\gptDatum{D3}{Meta-method identity datum: each stage preserves \(\operatorname{Id}(W_i)\).}
\gptDatum{D4}{Meta-method lift datum: local/state data, phase law, native scale, clock, and fiber coordinates.}
\gptDatum{D5}{Meta-method run datum: complete native cell closure is measured only after clock harmonization.}
\gptDatum{D10}{Meta-method feedback datum: obstruction detection synthesizes admissible re-adapters.}
...
\paragraph{Lift.}
For each source function or structured fiber \(W_i\), construct its native
carrier representation
\[
W_i\longmapsto \mathsf X_i^{(3D)}.
\]
The lift retains source identity, phase law, local or structural data, native
scale, native clock, fiber coordinates, and projection-loss state.  At this
stage the lifted states may be mutually incompatible.  That is expected: the
initial lift is source-faithful before it is common-frame compatible.
...
structure, or fiber law.  The analysis step identifies which retained state
coordinates fail to harmonize; it does not replace them by an aggregate scalar
diagnostic.

\paragraph{Adapt.}
Adapters are applied in a hierarchy.  Global structural adapters act first on
...
\mathsf X_i''.
\]
They set carrier scale, arc placement, height law, phase warp, and chart.  The
goal is not equal numerical coordinates; the goal is a common geometric frame
with source identity preserved.
...
\]
They include rank, phase, local-factor, root-number, cell-closure, and
obstruction-removal adapters.  Thus the adapter hierarchy is
\[
\boxed{
A_{\rm global}
\to
...
\]
The normalized theorem target is
\[
\boxed{
...
\]
All inverse and adaptation data is retained in the ledger.
...
\bigr).
\]
It is run through the common carrier dynamics.  At carrier stages one measures
cell closure, focal cancellation, synchronization, transport relations,
obstruction state, reflection state, and event coincidences.  If
\[
...
\]
For an \(L\)-function source, \(r_i(s)=\Lambda(s,W_i)\).  For another
structured function class, \(r_i\) may be a different one-dimensional
observable.  In every case, the lower-dimensional observable is interpreted
together with the retained ledger data needed to preserve identity, reconstruct
the carrier state, and explain which coordinates the readout suppressed.
...
analysis of the loss of productivity in the direct Beyond Endoscopy route. We discovered the obstruction to be an emergent clocking related phenomenon. Once the induced clock is
...
outside the frame. \bullshit{sandbag}
...
algebraic identity; it does not by itself prove that zeros follow GUE.) \bullshit{out of place}
...
\paragraph{Exact-gauge part: Altu\u{g}'s uniformity problem.} Altu\u{g}'s execution of Beyond Endoscopy for
...
for higher symmetric powers'' (Altu\u{g}~III, fn.~5, after Sarnak \cite{Sar}). The exact-gauge part
...
claim made here. This nonetheless fixes the paper's arc: \textbf{Part~I} builds the common carrier
system; the exact-gauge part isolates the lossless, scale-stable
mechanism; the transport part builds the machine it runs on (the carrier, the loss ledger, the warp
dictionary) and lands universal transport and functoriality in rank (Thm.~\ref{thm:universal},
Cor.~\ref{cor:universal}); the converse-theorem part certifies that landing
...
\paragraph{Transport part: functoriality and a candidate object.} The transport part
...
\paragraph{Scale bookkeeping in the exact-gauge part.}
The exact-gauge section uses the carrier/readout distinction fixed in
\S\ref{sec:carrier-S}: the lifted state is registered on the native
\(\pi/3\) carrier, while the scalar unit-\(1\) chart records the resulting
registration correction as an oscillatory readout.
The next section measures a different deterministic oscillation, namely the
orbital transform's own comb in Altu\u{g}'s Proposition~\ref{prop:52}, read in
the clock coordinate \(\nu_{\mathrm{clock}}=2\nu_J\).  It is not the
Riemann--Siegel \(S(t)\) term; it is a measured instance of the same projection
principle.
...
\begin{remark}[a distinct post-closure mechanism]\label{rem:reverb}
The product law fixes the derivative statistic \eqref{eq:prod} but not the \emph{configuration} that
feeds it. At a focal cancellation the ordinary phasor residue vanishes, yet the
fiber carries a nonzero eigen response that persists over roughly one native harmonic cell; when
closures fall within a cell their response intervals overlap and deform the local spacing
deterministically. The algebraic backbone --- a
...
non-uniformly spaced neighbors: it is not a pairwise artifact, the case a triple would expose. This is
...
correlations remains a separate continuation.
\end{remark}
...
\gptGFour{Candidate object synthesis site: carrier, fiber, and warp are assembled as the native realization.}
\gptDatum{D4}{Candidate synthesis data: carrier chart, fiber law, warp, and twist family.}
...
demands.)
\gptGOne{Concrete carrier-geometry discharge: conjugate helix/anti-helix block, determinant-one balance, and forced axis.}
\gptGEight{Concrete reflection-intertwining discharge: carrier conjugation gives the reflected functional readout.}
\gptDatum{D1}{Conjugate-pair Frobenius block and determinant-one carrier balance.}
\gptDatum{D8}{Reflection-axis datum: \(s\mapsto1-s\), conjugate leg, and warp-invariant determinant balance.}
...
\gptGFive{Continuation site requiring exact cell closure of the adapted warp.}
\gptGSeven{Readout site: warped carrier series agrees with the Euler datum on the initial domain.}
\gptGNine{Analytic landing site: Abel continuation in the carrier gauge.}
\gptDatum{D5}{Adapted warp \(\omega_W\) and cell-exact primitive used for Abel continuation.}
\gptDatum{D7}{Readout/unwarp equality with the ordinary twisted Dirichlet datum.}
\gptDatum{D9}{Carrier-gauge Abel continuation datum and commutation of inverse warp with the Abel limit.}
...
\]
\gptGNine{Boundedness site: warped-Abel vertical growth and reflected strip control.}
\gptGTen{Exhaustion site: every represented height event is a source event in the carrier state.}
\gptDatum{D9}{Vertical-strip growth data: bounded warped primitive, kernel \(K_W\), reflected strip control.}
\gptDatum{D10}{Exhaustion theorem datum: every carrier zero event is booked as a source event.}
...
\gptGSeven{Local readout-identification site: the candidate's local factors/root numbers are named place by place.}
\gptDatum{D7}{All-place local factor, root number, conductor, and completed \(L\)-datum identification.}
...
\gptGTwo{Recovery discharge: projection plus recorded radius/angle/height reconstructs the carrier state.}
\gptGThree{Identity discharge: projected readout plus ledger retains the same represented source fiber.}
\gptDatum{D2}{Round-trip datum: retained height plus radius/angle ledger reconstructs the carrier state.}
\gptDatum{D3}{Identity datum: projection plus loss record represents the same source fiber.}
...
\gptGEight{Machinery discharge: the functional equation is the readout of the carrier involution.}
\gptDatum{D8}{Theta/self-duality readout of \(J\), used as the functional-equation datum.}
...
\gptGNine{Central analytic-landing proposition: entireness, vertical-strip boundedness, and continuation for twisted readouts.}
\gptGEight{Reflection component of niceness: the completed functional equation is carrier reflection.}
\gptDatum{D8}{Niceness FE datum: dual-compatible tensor fiber, completion kernel, root number.}
\gptDatum{D9}{Niceness analytic datum: reflected integral, residual split, entire transform, vertical bound.}
...
\gptGFive{Exact-cell-closure lemma: zero native-cell mean gives bounded warped primitive.}
\gptGSix{Mode-dictionary lemma: bounded primitive kills the reflected carrier DC mode.}
\gptDatum{D5}{Exact zero native-cell mean and bounded incomplete-cell remainder.}
\gptDatum{D6}{DC mode \(R_{r,\tau}\) of the reflected carrier theta.}
...
\gptGThree{Identity/dual routing site: the payload's determinant and sign stamps identify the correct dual lane.}
\gptGSix{Mode-dictionary site: opposite strand residuals remove an unpaired determinant residue.}
\gptDatum{D3}{Passport stamps identify the routed dual lane rather than a scalar-equivalent fiber.}
\gptDatum{D6}{Opposite strand residuals \(R_+=-R_-\) as mode-cancellation datum.}
...
\gptGSix{Primary residual-mode discharge: Gram rank-drop removes the independent constant mode.}
\gptGNine{Entireness discharge site: no carrier residual mode remains for the Mellin transform.}
\gptDatum{D6}{Signed \(P/M\) channels, harmonic pencil, Gram rank-drop, and residual factorization.}
\gptDatum{D9}{Entireness datum: residual-free carrier theta after the rank-gap discharge.}
...
\gptGSix{Dictionary bridge: the one-dimensional constant term is exactly the harmonic pencil residual.}
\gptDatum{D6}{Bridge datum: \(R_{r,\tau}=D^{\mathrm c}_{r,\tau}=V_{r,\tau}\Phi_{r,\tau}\), \(V\neq0\).}
...
\gptGFive{Constructive closure site: two independent carrier directions force any cell residual to zero.}
\gptGTen{Reachability/transport site: one reachable independent stage supplies the correction pair for cells.}
\gptDatum{D5}{Forcible closure datum: residual \(D_C\), correction directions \(u,v\), real weights \(s,t\).}
\gptDatum{D10}{Reachability datum: nonconstant lane phase gives a stage with independent dual directions.}
...
clean.
\gptGTen{Coherent-transport discharge: composition, tensor, and base-change transports preserve exact closure.}
\gptDatum{D10}{Transport composition datum: faithful composition, tensor/base-change laws, and closure preservation.}
...
GUE, with the overlap application in Remark~\ref{rem:reverb}. This response is AC --- local-in-cell, of
...
\gptGThree{Dual-identity preservation site: the anti-helix carries the specified contragredient datum.}
\gptGEight{Local reflection-input site: determinant ledger and dual compatibility feed carrier reflection.}
\gptDatum{D3}{Duality datum: \(W_{r,\tau}^\vee\), central-character twist, and specified contragredient fiber.}
\gptDatum{D8}{Determinant ledger and local dual-pair reciprocity required for reflection.}
...
\gptGThree{Admissibility passport: Frobenius balance, sign compatibility, and positivity preserve represented identity.}
\gptGSix{Passport-to-residual link: the stamps feed the residual-extinction chain.}
\gptDatum{D3}{Admissible payload identity stamps \(F,S,L\).}
\gptDatum{D6}{Residual-extinction input stamps feeding \(R_{r,\tau}=0\).}
...
\gptGNine{Boundedness discharge: initial convergence and direct completed-integral vertical-strip bound.}
\gptDatum{D9}{Initial convergence half-plane, completed kernel decay, and \(t\)-independent strip integral.}
...
\gptGOne{Carrier theorem: global double-ended helix and involution \(J\).}
\gptGTwo{Projection theorem: Cayley/Mobius/log projection is faithful with a loss ledger.}
\gptGEight{Reflection theorem: \(J\) descends through projection and completion to the functional equation.}
\gptDatum{D1}{The complete double-ended carrier \(C=C_+\sqcup C_-\) and global involution \(J\).}
\gptDatum{D2}{Cayley/Mobius/log projections and loss-ledger bijection.}
\gptDatum{D8}{Completion clock and log-readout reflection datum.}
...
\gptGNine{Continuation gate: primitive exponent places both completed readouts on a strip containing the axis.}
\gptDatum{D9}{Primitive exponent \(\theta_W<\kappa/2\), dual primitive, and common analytic strip.}
...
Step~2 consumes.
The core of \textup{(i)} is \emph{machine-checked}: \texttt{TransferContinuation.transfer\_tendsto}
(Lean) proves convergence of the partial sums at every $s$ with $\Re s>\theta$, from the primitive
bound alone, via the formalized mean-value kernel \texttt{cpow\_sub\_bound}; the dual-exponent
identity and the strip arithmetic are \texttt{dual\_primitive\_norm} and
\texttt{strip\_contains\_axis} (footprint
$\{\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound}\}$, no \texttt{sorry}). The
local-uniformity upgrade from pointwise convergence to analyticity is the classical two-line step
above, manuscript-level.
\end{proof}
...
\textup{(iii)} Smoothing manufactures no cancellation: a random multiplicative fiber keeps
$\theta^{\mathrm e}=\tfrac12$ (measured $0.58$, App.~\ref{app:repro}), so the gate still bites.
\end{lemma}
\begin{proof}
...
\textup{(iii)} is the measured control.
\end{proof}
...
oscillatory, no steady growth); the random control fluctuates about $\tfrac12$ in \emph{every} window and
never equilibrates. Pre-committed to the falsifiability register: the $\Sym^{13}$ late-window slope falls
...
\text{angle})$ is a \emph{proven bijection} (\texttt{RoundTrip.record\_bijective},
\texttt{reconstruct\_record} --- Lean, unconditional; \S\ref{sec:construct}). Every remaining open input of
the niceness chain is therefore a statement about the \emph{unledgered} shadow --- the bare scalar the
...
the residual extinction $R=0$, where the carrier's machinery (\texttt{FocalResidualGeneral},
Lemmas~\ref{lem:bridge}, \ref{lem:rankgap}) is strongest. The frontier is thus not hard analysis at high
...
never a wall. The \emph{grown} exponents of Lemma~\ref{lem:growntransfer} widen every margin
($\theta^{\mathrm e}=0.00/0.29/0.40$, the automorphic anchor plateauing at $|L(0)|$; random $0.58$,
App.~\ref{app:repro}), and the operative form of the input is henceforth the grown
$\theta^{\mathrm e}_W<\kappa/2$.
\end{remark}
...
measured $0.27$--$0.46$ against the random-walk $0.54$ on the open ranks, and in the operative \emph{grown}
form $\theta^{\mathrm e}_W<\kappa/2$ (Lemma~\ref{lem:growntransfer}) measured $0.00$--$0.40$ against $0.58$
(\texttt{transfer\_exponent.py}, \texttt{grown\_transfer.py}; Remark~\ref{rem:twogates}) --- and the route is
...
\gptGEight{Completed reflection assembly: fiber admissibility, carrier reflection, and FE clock produce the twisted FE.}
\gptGSeven{Identification endpoint: after reflection is assembled, the Mellin readout is identified with the twisted \(L\)-function.}
\gptDatum{D8}{Completed-FE assembly datum: admissible dual pair, carrier reflection, and completion clock.}
\gptDatum{D7}{Mellin/unfolding datum identifying \(\mathcal M K_{r,\tau}\) with \(\Lambda(s,\Sym^r\pi\times\tau)\).}
...
\inst{move this to its own part}
...
\inst{move to new Hodge specific part}
...
itself; its overlap application is recorded in Remark~\ref{rem:reverb}.
\end{itemize}
...
\gptGOne{Transport input: complete carrier \(C_+\sqcup C_-\) with global involution.}
\gptGSeven{Transport output: exact Euler/local-factor readout of the transported datum.}
\gptGEight{Transport output: completed carrier reflection.}
\gptGSix{Transport output: residual extinction/no independent DC direction.}
\gptDatum{D1}{Transport input carrier datum \(C=C_+\sqcup C_-\), \(J\).}
\gptDatum{D6}{Transport residual datum \(D_W^{\mathrm c}=V_W\Phi_W\) and \(R_W=0\).}
\gptDatum{D7}{Transport readout datum: local factors and transported Euler data.}
\gptDatum{D8}{Transport reflection datum: completed dual-pair functional reflection.}
...
universal theorem have been copied into the Part~I proof slots
...
\gptGFour{Universal synthesis theorem: finite structural presentation determines carrier, warp, clocks, and cells.}
\gptGSeven{Universal readout theorem: the carrier readout equals the source Euler datum on its convergence domain.}
\gptGFive{Universal closure theorem: admissible fibers close at exact harmonic cell boundaries.}
\gptGTen{Universal transport theorem: finite structured sources are faithfully represented and synchronized by the carrier.}
\gptDatum{D4}{Minimal finite structural presentation and total synthesis map.}
\gptDatum{D5}{Native cell system and adapted warp furnishing exact focal closure.}
\gptDatum{D7}{Faithful readout on the initial convergence domain.}
\gptDatum{D10}{Universal carrier representation, synchronization, and transport preservation data.}
...
\gptGFour{Constructive synthesis discharge: \(\operatorname{Synth}\) reads adapter data from the finite law.}
\gptDatum{D4}{\(\operatorname{Struct}(W)\mapsto(\mathcal C_W,\omega_W,\tau_W,\mathcal L_W)\).}
...
\gptGTen{Synchronization discharge: heterogeneous admissible fibers share one carrier while preserving coordinatewise closure.}
\gptDatum{D10}{Common refinement of native cell systems and coordinatewise exact closure.}
...
\gptGTen{Rank-uniform transport consequence: structural source transports preserve the carrier obligations.}
\gptDatum{D10}{Rank-uniform composition/naturality datum for structural source transports.}
...
the Part~I G10 transport obligations recorded in
...
\gptGTen{Obstruction-honesty site: detected retained obstruction data require a witnessed carrier extension.}
\gptDatum{D10}{Obstruction datum \(\alpha\), faithful extension \(\iota_\alpha\), primitive \(h_\alpha\), and certificate \(dh_\alpha=\alpha^\#\).}
...
\gptGSeven{CPS candidate typing: local Langlands data give the exact local factors consumed by the converse theorem.}
\gptDatum{D7}{CPS candidate datum: \(\Pi_{r,v}\), local factors, local root numbers, conductor, and central character.}
...
\gptGThree{Twisting-family identity site: the helix/anti-helix pair carries \(W_{r,\tau}\) and its specified dual.}
\gptDatum{D3}{Twisted dual-pair datum \(W_{r,\tau}\), \(W_{r,\tau^\vee}\), and contragredient routing.}
\gptDatum{D7}{Twisted convolution readout datum \(L(s,\Pi_r\times\tau)\).}
...
\gptGNine{CPS niceness discharge: entire, bounded, reflected twisted readouts.}
\gptGSix{CPS residual discharge: focal cancellation/rank-drop extinguishes the residual mode.}
\gptDatum{D6}{CPS residual datum: passport, opposite strand residuals, Gram-pencil rank drop, \(R_{r,\tau}=0\).}
\gptDatum{D8}{CPS reflection datum: carrier reflection under CPS normalization.}
\gptDatum{D9}{CPS analytic datum: entire primal/contragredient readouts and vertical-strip boundedness.}
...
\gptGNine{Automorphic-landing ledger: CPS analytic hypotheses are checked against named carrier results.}
\gptGSeven{Automorphic-landing ledger: the candidate is the identified all-place \(L_{\mathrm{CPS}}\) datum.}
\gptDatum{D7}{Ledger datum: all-place \(L_{\mathrm{CPS}}\) readout and local factor identification.}
\gptDatum{D9}{Ledger datum: initial convergence, entireness, boundedness, and functional equation rows.}
...
\gptGNine{Landing theorem: CPS consumes the carrier niceness ledger.}
\gptDatum{D9}{Automorphic landing datum: the CPS theorem consumes the completed analytic ledger.}
...
\part{Significant Results}
\inst{Hodge results go here}
\inst{Obstruction removal results go here}



\inst{Sato--Tate proof goes here}
...
\section{Status and active research}\label{sec:status} \bullshit{not remotely accurate}
...
\paragraph{Active research.} \bullshit {half correct fabrication}Theorem~\ref{thm:func} proves $\Sym^r$ functoriality from the candidate
...
boundedness, entireness) being carrier statements, not classical inputs. Three directions remain active. \bullshit{sandbag}
...
and the carrier/fiber rescaling reduce the residue reading to cell arithmetic.\inst{tighten}

\emph{Sato--Tate for Maass forms} \inst{move to sato--tate section }(Corollary~\ref{cor:satotate}). Symmetric-power functoriality carries
...
none --- does. \inst{isolate as a significant result}

\emph{The loss-ledger$\,\leftrightarrow\,$cycle-filtration correspondence} \bullshit{scrambling} (companion paper). The
...
one. \inst{move to hodge part}

\emph{The structural half is unconditional.}\bullshit{hedging} On the \emph{semisimple} carrier state --- the finite
...
paper's target. \inst{move to hodge part}
...
$\Leftrightarrow$ odd lane-mode $\equiv0$ --- gate 2 restated as an extinction statement). Footprint
$\{$\texttt{propext}, \texttt{Classical.choice}, \texttt{Quot.sound}$\}$, no \texttt{sorry}, builds in-tree.
...
\end{itemize} \bullshit{shit format}


\inst{make a fucking bibtex file}
