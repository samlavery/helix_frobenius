% ============================================================================
% Self-contained since 2026-07-26: the GRH worked-example part was split into
% the standalone companion paper grh_companion.tex (cited as GRHCompanion);
% a condensed on-ramp remains at \ref{sec:worked-example}.
% ============================================================================
...
zero with a native event remains a separate premise; its sharpest form is a
single function-level identity --- agreement of the two charts' integrated
registrations $\int_0^T S$ at every height --- which the proven integrality
and positivity of the registration defect convert into the per-zero
identification, zeros arriving as corollary rather than premise (pointwise
coverage and fixed-operator spectral capture are its event-level and
operator-level faces).
...
\item \emph{The worked example} (\S\ref{sec:worked-example}, condensed): the
basic configuration --- the \(\pi/3\) carrier with a Dirichlet fiber ---
built from scratch, exercising every mechanism the rest of the paper
generalizes; the midpoint arrives as a geometric output (area-law scale
balance and no-drift), never as a stipulated decimal.  The full development
--- the carrier geometry evidence, the Gram lifts, the ambient operator, and
the two conditional GRH routes with their compiler-audited premises --- is
the companion paper \cite{GRHCompanion}.
...
are displayed with their exact premises in the companion paper
\cite{GRHCompanion}, and the compiler audit identifies which premise supplies
the critical-line conclusion. No claim in this paper exceeds its register.
...
shortest self-contained path should read the condensed worked example
(\S\ref{sec:worked-example}) first: it builds the basic configuration from
scratch, names the two decisions, and exercises every mechanism the rest of
the paper generalizes --- with the full development and the two conditional
GRH routes in the companion paper \cite{GRHCompanion}. A running ledger of
contributions and a glossary of the paper's vocabulary follow immediately.
...
\texttt{GlobalHelix.radialGlobalSatakeCoeff\_norm\_le}, with the finite$\to$infinite Euler limit and
its infinite-bank summability, exact reflection, and rapid decay compiled
(\texttt{GlobalHelix.symTwistEulerLimit\_eq\_div},
\texttt{GlobalHelix.summable\_fullTensorEuler\_zeroModeTerm},
\texttt{GlobalHelix.fullTensorEulerZeroModeDualTheta\_eq\_div}); a general polynomially-bounded datum
passes through the same constructor,
\texttt{CarrierTheta.automaticCoefficientThetaStrongFEPair}). [theorem, on Lean carrier inputs]
...
\texttt{ConeProjection.record\_bijective}, Thm.~\ref{prop:localid}), and the transport composes
...
(\texttt{cpsRadialSatoTateGeometry3D\_unified}). The ceilings' supply line is priced exactly and
\emph{non-circular}: the per-rung Jacquet--Shalika ceiling ($\|\alpha_p\|\le p^{1/2}$, unconditional,
no Ramanujan) consumes the tower, whose machine-checked niceness payload holds over \emph{arbitrary
nonzero radius}. The bank structure \texttt{PolynomialSatakeDualPair} carries a polynomial radial
bound $\|\alpha_p\|\le p^{e}$ --- never $\|\alpha_p\|=1$ --- and
\texttt{cpsArithmeticTwist\_radiusLive\_niceness} (via \texttt{cpsPolynomialAllTwists\_payload} over
\texttt{arithmeticCPSPolynomialTwist}) proves the reciprocal-height reflection, entirety, strip
bounds, and functional equation on it \emph{with no temperedness assumed}, for any base
$\pi:\texttt{PolynomialSatakeDualPair}\,(\mathrm{Fin}\,2)$ (holomorphic or Maass). Unitarity is not
packaged in the bank; it is the radial-limit \emph{output}
(\texttt{RamanujanLimit.strand\_radius\_one\_of\_tower\_ceiling} takes an arbitrary ceiling and
returns radius one). So the non-tempered start is \emph{not} a remaining construction: the niceness
is radius-live and machine-checked, the ceiling is Jacquet--Shalika, and $\|\alpha_p\|=1$ falls out
only at the capstone. The unit-modulus payload \texttt{cpsAllTwists\_unconditional3DAnalyticPayload}
is a specialization, not the locus of generality. [theorem; radius-live niceness and limit step Lean]
...
(\texttt{cpsRadialSatoTateGeometry3D\_unified}).  The arithmetic identification is also exact:
\texttt{ArithmeticSatakePrimeFamily.character\_eq\_satakeTrace} identifies each carrier
character with the literal symmetric-power Satake root sum, and
\texttt{primeTestAverage\_eq\_arithmetic} identifies their empirical averages term-by-term; the
prime-cancellation theorem supplied by Jacquet--Shalika plus Wiener--Ikehara remains cited.
...
\texttt{grh\_of\_selfAdjoint\_resolvent\_capture}, developed in full in the
companion paper \cite{GRHCompanion}. [Lean]
...
(\texttt{warpFiber}, \texttt{ForcibleClosure}). A warp scales by a \emph{function}; the reciprocal of a warp is again a warp, so there is no separate ``inverse warp'' --- removing a warp from a readout is the kernel-transfer operation (\texttt{WarpRemovalTransfer}). \\
\emph{carrier scale} & scaling by a harmonic \emph{constant}: the invertible constant reparametrization on the object's own integer lattice --- not a warp
(\texttt{Admissible.scale}, \texttt{CommonRepresentation}); fibers sometimes carry their own constant scale (fiber scaling). \\
...
Lemma~\ref{lem:selfdual}; rank/jet, \texttt{BSDClocks}).  The governing principle: \emph{phase and
radius may decouple dynamically, but not informationally} --- channels evolve independently; the
ledger books both. \\
...
\texttt{CarrierReachability.reachable\_independent\_stage}).  For the Riemann
carrier the hypothesis pair is instantiated at the actual clock: the lane phase
is \(\vartheta(t)=\int_0^t \operatorname{Re}\,(\log\Gamma_{\mathbb R})'(\tfrac12+iu)\,du\)
--- the Riemann--Siegel phase by its defining ODE and initial condition --- and
its continuity and nonconstancy are compiled
(\texttt{CarrierScale.theta\_continuous},
\texttt{ZetaPhaseBridge.theta\_nonconstant}, the latter from the compiled
Stirling asymptotic), so the specialized stage exists unconditionally (Lean
\texttt{ZetaPhaseBridge.zeta\_reachable\_independent\_stage}, with the closure
form \texttt{ZetaPhaseBridge.zeta\_reachable\_residual\_forcible}).  Applying the
...
(Theorem~\ref{prop:localid}).  The global \(\varepsilon\) and \(N\) are
...
\noindent The completion clock \(K\) supplied to this theorem is not
independent analytic input.  In dimension one it is furnished upstream by
the unique scale-balanced Gaussian fixed under the carrier's Fourier
exchange --- unique \emph{within the Gaussian family}
\(g_a(x)=e^{-\pi a x^2}\), \(\Re a>0\), where
\(\widehat{g_a}=a^{-1/2}g_{1/a}\) forces \(a=1\), the coefficient \(\pi\)
pinned by self-duality rather than by any zeta convention --- and its
Mellin transform is the completion factor
\(\tfrac12\pi^{-s/2}\Gamma(s/2)\).  Both statements are Lean theorems at
the standard footprint
(\texttt{GaussianClock.gaussianClock\_selfDual\_iff},
\texttt{GaussianClock.gaussianClock\_mellin};
\texttt{GaussianClockUniqueness.lean}), and the exact Jacobi
transformation the clock generates is likewise consumed from Mathlib
(\texttt{jacobiTheta\_S\_smul}, \texttt{EpsteinThetaI.lean}).  For the
basic configuration the intertwining hypothesis is thus discharged
...
\item \textup{Readout identity.} Theorem~\ref{prop:localid},
...
\]
The native system is realized on a harmonic carrier --- written below at
\(\pi/3\), the scale at which part~(i) of Theorem~\ref{thm:carrier-scale-S} is
proved, with the trivial character's own cell \(\pi/6\) giving the identical
ledger (Remark~\ref{rem:which-harmonic}).  The conventional analytic readout
uses a unit-\(1\) coordinate system.  These two carrier scalings represent the
same arithmetic procession, but they do not register the same crossings in the
...
\begin{remark}[Which harmonic scale names the box]\label{rem:which-harmonic}
The theorem is stated at \(\pi/3\) because that is where the realization is
anchored (part (i) is proved at \(\pi/3\)), \emph{not} because \(\pi/3\) is
\(\zeta\)'s aligned cell.  It is not: the fiber-scale assignment of
\S\ref{sec:carrier} sends the trivial character to \(\pi/6\) (the \(\mu_{12}\)
cell, where the eta-regulated principal reading lands,
\(\tfrac12\ln3+i\pi/6\)), and \(\pi/3\) is \(\chi_3\)'s aligned scale.  Naming
\(\pi/6\) instead changes nothing, and this is a theorem rather than an
expectation:
\[
N_{\pi/6}(e^t)-R_1(e^t)=S_{\mathrm{ev}}(t)
\qquad(\texttt{Sgap\_pi6\_one},\ \texttt{unit\_tracks\_pi6}),
\]
because the whole harmonic family registers identically,
\(S_{\pi/m,\pi/m'}=0\) (\texttt{Sgap\_pi\_div}; Theorem
\ref{thm:harmonic-family-S}).  The reason is structural: the event channel
reads the arc-set \emph{indicator}, which is \(1\) at every \(\pi/m\)
(\(m\cdot(\pi/m)=\pi\), \texttt{pi\_div\_channel\_open}) --- it does not read
the cell resolution.  At \(\pi/3\) the mark falls at \(k\equiv3\ (\mathrm{mod}\
6)\), at \(\pi/6\) at \(k\equiv6\ (\mathrm{mod}\ 12)\): twice the cells, one
mark per closed loop either way, the same count.  What is \emph{not} gauge is
the cell itself (\(\mu_6\) versus \(\mu_{12}\)) --- it governs the complete-cell
sums of Theorem~\ref{thm:g5-native-cell-closure} --- but the registered count
does not read it, so the choice of harmonic scale is a naming convention here
and carries no mathematical content.
\end{remark}
...
\begin{remark}[Register: the atomic defect ledger --- sign flips at
cancellation]\label{rem:atomic-ledger}
For a quotient $Q=\Lambda_P/\Lambda_C$ of completed $L$-functions the
registration defect is the signed measure $d\nu=dN_P-dN_C$: positive atoms
at numerator zeros, and at each denominator zero $\rho$ the site atom
$\operatorname{ord}_\rho\Lambda_P-\operatorname{ord}_\rho\Lambda_C$.  The
ledger has exactly three layers, and they must never be blended:
\begin{enumerate}
\item[\textup{(DC)}] the accumulated defect
$\nu[0,T]\sim\frac{d_P-d_C}{2\pi}\,T\log T$ is \emph{unconditional} ---
Riemann--von Mangoldt counting on the licensed factors; at the rung-$r$
instance the coefficient is the degree $r+1$ of the formal symmetric power;
\item[\textup{(AC)}] the fluctuation about it is the $S(t)$ oscillation of
the quotient, on the fluctuation ledger and nothing else;
\item[\textup{(atoms)}] the \emph{sign} of each site atom at a denominator
zero is the cancellation indicator: a successful cancellation (numerator
vanishing on the site) flips the would-be negative atom to $\geq0$; a
failed cancellation books a genuine pole atom $<0$.
\end{enumerate}
Atomic nonnegativity of $\nu$ at every denominator-zero site is therefore
\emph{equivalent} to pole-freeness of the quotient --- at the rung instance,
to entirety of the formal $\Sym^r$ completion.  Integration deliberately
forgets the atomic signs: the accumulated positivity (DC) is unconditional
and cannot by itself decide containment; the containment content lives
entirely in the sign flips.  The two attacks on that content are dual: the
warp/carry rigidity chain
(\texttt{RequestProject/ResidueLedger}, \texttt{WarpRigidity},
\texttt{GlobalCarry}, \texttt{RectResidue}, \texttt{PolarCorrection},
\texttt{PolarCorrectionJordan}) forces the atoms nonnegative top-down;
the registration ledger of this section reads the same signs bottom-up,
site by site, as the unit-chart residue left where the harmonized carrier
cancels residue-free.
\end{remark}
...
This non-cancellation statement is now quantitative.  If an enclosed zero
\(\rho\) is off the carrier, then
\[
  \operatorname{ord}_{\rho}\xi
   \le N_{\mathrm{off}}^{\mathrm{mult}}(T)
   =S_\Gamma(T)-S_{\mathrm{mult}}(T),
\]
and the right-hand side is strictly positive.  These are the compiled
theorems
\texttt{xiOrderNat\_le\_offLineStripZeroCountMult\_of\_offCarrier},
\texttt{xiOrderNat\_le\_classicalSContour\_sub\_Smult\_of\_offCarrier}, and
\texttt{classicalSContour\_sub\_Smult\_pos\_of\_offCarrier}.  Thus neither a
second zero nor the completed-reflection partner can cancel an off-carrier
contribution in the global coordinate ledger.

\begin{remark}[Relation to Turing's method]\label{rem:turing-method}
The principle that an independently continued global count, compared
against a locally assembled list, certifies the list's completeness is
classical: it is Turing's method \cite{Turing53}, in which an explicit
bound on the average of the argument (Littlewood's theorem) makes the
comparison effective at a given height; Turing's error terms were
corrected by Lehman \cite{Lehman70}, the argument-integral bounds
sharpened by Trudgian \cite{Trudgian11}, the method extended to general
$L$-functions by Booker \cite{Boo06}, and an explicit-formula variant
proved for an extended Selberg class by B\"uthe \cite{Buethe15}.  What is
claimed here is therefore not the count-comparison principle.  It is the
exact typed form: \eqref{eq:global-coordinate-defect} is an identity
between two independently constructed coordinates, holding at every good
height with no averaging, no estimate, and no error term; the defect is
multiplicity-safe --- every summand a positive analytic order, so
cancellation between zeros is impossible --- and both constructions and
their comparison are machine-checked.  Turing-style inequalities estimate
precisely the quantity this census identifies exactly: the identity is
the law those certificates instantiate, stated and verified separately
from the estimates that make it computable.
\end{remark}
...
The analytic-zero side is no longer represented only by a scalar membership
predicate.  The compiled transfer in
\path{RequestProject/ZetaZeroNative3DSourceTransfer.lean} begins with every
actual \(\rho\in\texttt{ZD.NontrivialZeros}\) and constructs
\path{PrincipalZeroAnalyticFiber3D}\((\rho)\).  Its state is the literal
eta-configured \(\texttt{Vec3}\) fiber at the original complex parameter; after
division by the nonvanishing eta factor, its plane partial sums converge to
\(\zeta(\rho)=0\).  The same object carries the event-independent ambient helix
state at ordinate \(\Im\rho\), positive physical height
\(Z=\exp(\Im\rho)\), and its nonzero eigenvector with eigenvalue \(\Im\rho\).
None of these fields places \(\rho\) on a preselected vertical line.

Before any parameter-preserving carrier-to-chart identification, the compiled
constructor \path{principalZero_focalCancellation_on_carrier} packages this
closure as a \path{PrincipalZeroNativeCarrierEvent3D}.  Its
\path{carrierState} is a typed member of the independently defined
Archimedean helix, and its stored analytic fiber closes at the same physical
height.  The theorems \path{.on_archimedeanHelix},
\path{.physicalHeight_eq_focalHeight}, and \path{.ambient_eigenvector} prove,
respectively, literal helix membership, equality of the carrier and focal
heights, and the ambient 3D eigenvector equation.  Most importantly,
\path{.noRadialDrift} is derived directly from that carrier membership by
\path{carrierPointAtHeight_noRadialDrift}.  The event type and this proof use
neither the Euclidean magnitude of the analytic fiber nor a radial-limit
hypothesis.  Only afterwards does \path{.carrierReadout_scaleBalanced} apply
the area law to the carrier readout.  Thus focal cancellation is first marked
at \(Z=\exp(\Im\rho)\) on the helix, no drift is a fact about that helix
state, and equality of the resulting carrier coordinate with the original
complex parameter remains the separate chart-identification theorem.

A source audit of this passage confirms the registration but sharpens what it
delivers, and we record both halves because the second is easy to misread.
The constructor
\path{principalZero_focalCancellation_on_carrier} carries the single
hypothesis \(\rho\in\texttt{ZD.NontrivialZeros}\); the nonvanishing of the eta
factor is \emph{derived} from the strip bound rather than assumed, and no
occurrence of \path{CarrierScaleBalanced}, \path{RidesCarrier},
\(\Re\rho=\tfrac12\), or GRH appears in its arguments. The claim above that
none of these fields places \(\rho\) on a preselected line is therefore exact,
and the reason is visible in one line of the fiber:
\path{ambientState_eq} reads
\(\texttt{ambientState}=\texttt{carrierState3D }1\,1\,(\Im\rho)\). The
registration is a function of \(\Im\rho\) alone.

That same line bounds what the registration can decide. Because \(\Re\rho\)
never enters, \emph{every} nontrivial zero --- on the line or off it ---
receives the identical carrier event at height \(e^{\Im\rho}\); the
construction is faithful and, for the same reason, blind to the question at
issue.

The absence of radial drift is an effect of the geometry, and the half is
derived rather than posited. \path{CarrierScaleBalanced} asks only that the
fiber amplitude reciprocate the arclength-derived radius to a positive finite
limit, \(n^{-\sigma}\,\texttt{carrierRadius}(n)\to L>0\); the definition
mentions no zero set, no spectral chart and no preselected midpoint. Since
\(\texttt{carrierRadius}(n)/\sqrt n\to c>0\) by the area law --- the radius
being computed from winding at integer sites, with no arithmetic input --- the
product factors as \((\texttt{carrierRadius}(n)/\sqrt n)\cdot n^{1/2-\sigma}\)
and converges to a positive limit exactly when \(\sigma=\tfrac12\)
(\path{sigma_half_is_scale_critical}). Hence
\path{exists_unique_carrierScaleBalanced}, \(\sigma_*\) is defined as the
choice of that unique abscissa, and \path{carrierAbscissa_eq_half} is a
\emph{theorem}: the critical abscissa is a consequence of the helix's
arclength growth. By \path{noRadialDrift_iff_carrierAbscissa} the condition
\(n^{x-\sigma_*}=1\) holds if and only if \(x=\sigma_*\), so ``no radial
drift'' characterises that geometrically determined line, and ``every zero has
no radial drift'' is a rendering of the Riemann hypothesis that answers
\emph{why that abscissa} without assuming it. What the geometry does not
supply is that any particular zero sits there. Applied to a carrier point,
\path{carrierPointAtHeight_noRadialDrift} follows from the simp lemma
\(({\texttt{carrierPointAtHeight}}\,Z).\mathrm{re}=\sigma_*\), so its exponent
is \(\sigma_*-\sigma_*\) and it holds for \emph{every} \(Z\): carrier points
do not drift because they are carrier points. The statement of the same shape
with \(\Re\rho\) in place of the carrier coordinate is about \(\rho\) and is
equivalent to \(\Re\rho=\sigma_*\). The characterisation is the content; the
step from the first statement to the second is the theorem. Accordingly
\path{nonempty_principalZeroNative3DSourceTransfer_iff_noRadialDrift} is
stated, correctly, as an equivalence: the source transfer exists precisely
when \(\Re\rho=\sigma_*\). It is a faithful reformulation of the conclusion,
not a step toward it, and the certificate field
\path{analyticPoint}\,:\,\(\texttt{carrierPointAtHeight}(\texttt{event}.1)
=\rho\) carries that same content as a hypothesis.

The two uses of ``rides'' are now separated at the type level.  For an
analytic zero fiber \(F_\rho\), Lean defines
\path{PrincipalZeroAnalyticFiber3D.RidesCarrier} by the state equality
\[
 F_\rho=
 \texttt{spectralFiber etaW}
   \bigl(\texttt{carrierPointAtHeight}(Z_\rho)\bigr)
 \qquad\text{in }(\mathbb N\to\texttt{Vec3}).
\]
This is the fiber-to-carrier attachment; it is stronger than sharing the
ordinate or physical height.  The carrier-to-helix map is the independent
field
\(
  \texttt{carrierState.point}
   =\texttt{Geometry.gammaY}(1,1,\texttt{carrierState.ordinate})
\).
The theorem
\path{PrincipalZeroAnalyticFiber3D.ridesCarrier_iff_carrierWeld} proves that
the first state equality is equivalent to the parameter-preserving weld
\(
 \texttt{carrierPointAtHeight}(Z_\rho)=\rho
\): its imaginary part comes from
\(\log Z_\rho=\Im\rho\), while equality of the radial exponents is extracted
from the actual three-dimensional magnitudes at the nontrivial carrier site
\(n=2\).  Composing with the area law gives
\path{.ridesCarrier_iff_scaleBalanced}.  Thus no equality of unlike objects is
used and no bare midpoint is placed in the ride definition.

Exact focal cancellation itself cannot occur off the carrier.  Lean defines
\path{CompletedFocalCancellationRepresents}$(\chi,\rho)$ by requiring one
positive physical height \(Z\) for which the completed residual
\(\texttt{Dcell}\,\chi\,Z\) vanishes and whose readout
\(\texttt{carrierPointAtHeight}\,Z\) is the same parameter \(\rho\).
The compiled theorem
\path{completedFocalCancellationRepresents_iff} proves
\[
 \texttt{CompletedFocalCancellationRepresents}(\chi,\rho)
 \quad\Longleftrightarrow\quad
 L(\rho,\chi)=0\ \text{ and }\ \Re\rho=\sigma_*.
\]
The reverse direction reconstructs \(Z=\exp(\Im\rho)\); the real coordinate
is read from the Archimedean area-law carrier.  Consequently
\path{no_offCarrier_completedFocalCancellation} proves pointwise that a
parameter with \(\Re\rho\ne\sigma_*\) cannot be represented by focal
cancellation.  This precedes every resolvent trace and every zero count, so
there is no cross-event or reflected-pair cancellation mechanism to invoke.

The second structure, \path{PrincipalZeroNative3DSourceTransfer}, records the
geometric projection of that analytic fiber into a conservative source mode
and the complete native focal/operator certificate at the same parameter.  Its
constructor
\path{principalZeroNative3DSourceTransfer_of_noRadialDrift} accepts the
area-law equation \(n^{\Re\rho-\sigma_*}=1\), derives the source coordinate
internally, and never accepts \(\Re\rho=1/2\).  Conversely the source
conservation field recovers that no-drift equation at every \(n>1\).  Hence the
compiled pointwise theorem
\path{nonempty_principalZeroNative3DSourceTransfer_iff_noRadialDrift}
identifies the exact projection step, rather than hiding it inside a statement
that a zero already equals a carrier point.  Globally,
the native direction no longer needs that radial equation as an input.
The theorem \path{carrierPointAtHeight_noRadialDrift} proves zero radial drift
for every physical-height carrier readout solely from helix membership, and
\path{completedThreeDZeroAtHeight_focal_rankDrop_eigenvector_noRadialDrift}
assembles, for one and the same event, the completed focal residual zero, the
spatial harmonic-pencil rank drop, the ambient carrier eigenvector at
\(\log Z\), and the no-drift conclusion.  At the transfer layer,
\path{PrincipalContourNative3DCertificate.analyticFiber_eq_carrierFiber}
proves that the literal \(\texttt{spectralFiber etaW }\rho\) is the carrier
fiber at the certificate's marked height, while
\path{PrincipalContourNative3DCertificate.noRadialDrift} derives its radial
equation from that identification.  Consequently
\path{principalZeroNative3DSourceTransfer_of_nativeCertificate} constructs the
full source transfer from native realization itself, with neither a midpoint
nor a no-drift proposition supplied.  The physical-height identification is
also literal, not merely an agreement of ordinates:
\path{PrincipalZeroNative3DSourceTransfer.eventHeight_eq_focalCancellationHeight}
proves that the native event height equals the analytic fiber's stored
focal-cancellation height.  After rewriting by that equality,
\path{.harmonicRankDrop_at_focalCancellationHeight} proves that the completed
spatial pencil rank-drops at precisely that height.  The combined theorem
\path{.focalCancellation_rankDrop_noRadialDrift_sameHeight} returns the fiber
readout closure, pencil rank drop, and helix no-drift law at this one shared
physical height.
Moreover every completed source transfer proves the attachment itself by
\path{PrincipalZeroNative3DSourceTransfer.fiberRidesCarrier}; it then derives
the original analytic parameter from the carrier readout by
\path{.carrierReadout_eq_analyticParameter}.  The composed theorem
\path{.fiberRidesCarrier_carrierRidesHelix} returns, in one statement, the
fiber-state attachment, literal membership of the carrier point in
\(\Gamma_Y\), and equality of the two physical focal heights.  At the global
level,
\path{globalCoordinateIdentification_iff_allCanonicalZeroFibersRideCarrier}
states the remaining identification exactly as this state-level attachment
for every canonical analytic zero fiber.  The assembly has also been closed
without passing through the contour coordinate:
\path{nonempty_principalZeroNative3DSourceTransfer_iff_fiberRidesCarrier}
proves, zero by zero, that existence of the completed native source transfer
is exactly equality of the two complete \(\mathbb N\to\texttt{Vec3}\) fiber
states.  Quantifying this statement gives
\path{allNative3DSourceTransfer_iff_allCanonicalZeroFibersRideCarrier}, and
\path{allNative3DSourceTransfer_iff_zeroLedger_eq_carrierHeight} identifies
that same family-level attachment with equality of the analytic zero-ledger
and event-independent carrier-height operators.  Thus the source, state, and
operator formulations are proved equivalent; none silently substitutes a
bare equality of ordinates for fiber attachment.

Globally,
\path{globalCoordinateIdentification_iff_upperNative3DSourceTransfer}
states that all-good-height coordinate agreement is exactly transfer of every
upper zero from its genuine three-dimensional analytic fiber to the
conservative native source.  Finally,
\path{upperNative3DSourceTransfer_of_selfAdjointXiReceiver} performs this
full transfer for the independent self-adjoint xi-receiver route.  The
conjugation-complete theorem
\texttt{globalCoordinateIdentification\_iff\_}\allowbreak
\texttt{allNative3DSourceTransfer} upgrades
the statement to every nontrivial zero in both half-planes, and
\path{allNative3DSourceTransfer_of_selfAdjointXiReceiver} performs that complete
two-sided transfer through the same receiver.
The final operator propagation is now direct and basiswise.  Given those
two-sided transfers,
\path{xiZeroLedgerOperator_eq_carrierHeight_of_allNative3DSourceTransfer}
proves
\[
  \texttt{xiZeroLedgerOperator}=\texttt{xiCarrierHeightOperator}
\]
by evaluating the two diagonal operators on each canonical analytic-zero
mode and using only that mode's compiled scale-balance theorem.  No trace sum
or cross-event cancellation appears.  The theorem
\path{globalCoordinateIdentification_of_allNative3DSourceTransfer} then feeds
this operator equality into the contour/carrier unification and returns
\(\texttt{classicalSContour}=\texttt{Smult}\) at every good height.  At the
receiver endpoint,
\path{globalCoordinateIdentification_and_zeroLedger_eq_carrierHeight_of_selfAdjointXiReceiver}
returns both conclusions in one certificate.
The state-space form is sharper still.  At every positive site \(n\),
\path{PrincipalZeroAnalyticFiber3D.radialMagnitude} computes directly from the
literal \(\texttt{Vec3}\) state
\[
  \operatorname{mag}_3(F_\rho(n))=n^{-\Re\rho}.
\]
Consequently
\path{PrincipalZeroAnalyticFiber3D.carrierScaleBalanced_iff_radialMagnitude}
identifies the additional parameter-preserving scale condition with a positive finite limit of
\(\operatorname{mag}_3(F_\rho(n))R_{\rm carrier}(n)\).  This statement contains
neither a carrier point nor a supplied midpoint.  It is a diagnostic for whether
the analytic exponent agrees with the already no-drift carrier readout, not the
source of native helix membership or native no drift.  The numerical half-unit
is recovered only when the independently proved Archimedean area law evaluates
the unique balanced exponent.

The analytic/operator identification has also been completed before any event
certificate is introduced.  In
\path{RequestProject/XiZeroLedgerResolvent3D.lean}, the affine chart identity
\[
  \left(\frac12+iz\right)-\rho
     =i\bigl(z-\operatorname{poleParam}(\rho)\bigr)
\]
transports the proved Hadamard partial fraction term by term and gives
\path{xiChannel_eq_constant_add_zeroResolventTrace}:
\[
 -\frac{\xi'}{\xi}\!\left(\frac12+iz\right)
 = C+\sum_{\rho}m_\rho\left(
      \frac{i}{z-\operatorname{poleParam}(\rho)}-\frac1\rho\right).
\]
Thus the half-unit is the affine carrier readout unit used in an independently
proved change of coordinates, not a zero-location field.  The associated
\path{xiZeroLedgerOperator} is defined on the free finitely supported Hilbert
space on analytic zeros and has
\(\operatorname{poleParam}(\rho)\) as an actual eigenvalue by
\path{xiZeroLedgerOperator_hasEigenvalue}.  Its definition mentions no
\texttt{ThreeDZero}, focal event, or equality with \texttt{carrierPoint}.
Spectral symmetry then yields the area-law balance event by event through
\path{xiZeroLedgerOperator_symmetric_imp_scaleBalanced}.  Finally the compiled
unification theorem
\path{globalCoordinateIdentification_iff_xiZeroLedgerOperator_symmetric}
states that symmetry of this independently identified resolvent operator is
exactly the all-good-height identity \(S_\Gamma=S_{\rm mult}\).  This rules out
an ``offline cancellation'' inside the comparison: analytic multiplicities are
positive, and every diagonal pole is carried by its own nonzero basis
eigenvector.
More explicitly, the compiled operator identity
\path{xiZeroLedgerOperator_eq_height_sub_I_smul_radialDrift} is
\[
 H_{\xi}=H_{\rm carrier}-iD_{\rm radial},\qquad
 D_{\rm radial}e_\rho=
   \left(\Re\rho-\frac12\right)e_\rho .
\]
Here both \(H_{\rm carrier}\) and \(D_{\rm radial}\) are independently
constructed real-diagonal symmetric operators; the half-unit in
\(D_{\rm radial}\) is the value selected by the preceding area law.  The
theorems
\path{xiRadialDriftOperator_eq_zero_iff_all_scaleBalanced} and
\path{xiZeroLedgerOperator_isSymmetric_iff_radialDrift_eq_zero} prove,
basis mode by basis mode, that symmetry of the analytic pole operator is
precisely extinction of radial drift, not cancellation of drift after summing
different zeros.  At global scale this is compiled in the two interchangeable
forms
\path{globalCoordinateIdentification_iff_xiRadialDriftOperator_eq_zero} and
\path{globalCoordinateIdentification_iff_zeroLedger_eq_carrierHeight}.
There is also a canonical projection statement, rather than a bare instruction
to discard the radial coordinate.  On the same finitely supported zero-state
space, \path{xiZeroLedgerFormalAdjoint} is defined coefficientwise and satisfies
the exact Green identity
\[
 \langle H_{\xi}f,g\rangle=\langle f,H_{\xi}^{\dagger}g\rangle
\]
by \path{xiZeroLedgerFormalAdjoint_green}.  Its Hermitian part is computed,
not postulated:
\[
 P_{\rm nd}(H_{\xi})
   :=\frac12\bigl(H_{\xi}+H_{\xi}^{\dagger}\bigr)
   =H_{\rm carrier},
\]
the theorem \path{xiZeroLedgerNoDriftProjection_eq_carrierHeight}.  The
complementary anti-Hermitian part is exactly the radial ledger,
\[
 D_{\rm radial}
   =\frac{i}{2}\bigl(H_{\xi}-H_{\xi}^{\dagger}\bigr),
\]
by \path{xiRadialDriftOperator_eq_I_half_smul_adjointDefect}.  Hence the
all-good-height identity is equivalently the fixed-point equation
\(H_{\xi}=P_{\rm nd}(H_{\xi})\), compiled as
\path{globalCoordinateIdentification_iff_zeroLedger_fixedByNoDriftProjection}.
This makes precise the phrase ``no-drift projection'': the carrier height and
the drift are the Hermitian and anti-Hermitian components of the independently
identified analytic pole operator, and separate basis modes cannot cancel one
another in either component.  Indeed
\path{xiZeroLedgerResidual_eq_zero_iff_each_basis} proves that the projection
residual vanishes as an operator if and only if it vanishes on every canonical
zero basis vector separately, and
\path{globalCoordinateIdentification_iff_eachZeroBasisProjectionResidual_eq_zero}
identifies that basiswise condition with the global coordinate equality.  The
pointwise computation is literal:
\[
 (H_\xi-P_{\rm nd}H_\xi)e_\rho
   =-i\left(\Re\rho-\frac12\right)e_\rho,
\]
proved by \path{xiZeroLedgerProjectionResidual_on_basis}.  Finally
\path{principalZeroProjectionResidual_eq_zero_iff_radialMagnitude} identifies
vanishing of this same basis residual with the positive finite area-normalized
Euclidean-magnitude limit of the actual eta-configured 3D fiber.  Thus the
operator, state-space, and area-law formulations agree at each event before any
global summation is taken.

There is also a direct receiver formulation.  For every complex sample \(z\),
\path{zeroPoleResolvent_eq_carrierBasisReadout_iff_scaleBalanced} proves
\[
 i\,(z-\operatorname{poleParam}(\rho))^{-1}
 =i\,\langle e_{\Im\rho},(z-H_{\rm carrier})^{-1}e_{\Im\rho}\rangle
 \quad\Longleftrightarrow\quad
 F_\rho\ \text{is area-law scale balanced}.
\]
Only injectivity of inversion and of multiplication by \(i\) is used, so one
sample already identifies the pole parameter and no trace-level cancellation
is available.  The global assembly
\path{globalCoordinateIdentification_iff_eachZeroPoleResolvent_eq_carrierBasisReadout}
states that \(S_\Gamma=S_{\mathrm{mult}}\) at all good heights exactly when
this equality holds separately on every analytic-zero basis mode.
The regularized trace bridge is now typed at the same resolution.  Lean defines
\path{xiZeroResolventKernel} and \path{xiCarrierHeightResolventKernel} with the
same positive analytic multiplicity and the same Hadamard counterterm, but with
pole coordinates \(\operatorname{poleParam}(\rho)\) and \(\Im\rho\), respectively.
The pointwise theorem
\path{xiZeroResolventKernel_eq_carrierHeightKernel_iff_scaleBalanced} cancels
the positive multiplicity and proves that the two kernels agree exactly at the
area-law fixed point.  Hence
\path{globalCoordinateIdentification_iff_basisResolvedCarrierResolventBridge}
identifies the all-good-height theorem with equality of these two kernels for
every \((z,\rho)\), before summation.  Only after that basiswise equality is
established does \path{xiCarrierHeightResolventTrace} take the \texttt{tsum};
\path{carrierHeightResolventTrace_of_globalCoordinateIdentification} then gives
the literal carrier formula
\[
 -\frac{\xi'}{\xi}\!\left(\frac12+iz\right)
   =C+\texttt{xiCarrierHeightResolventTrace}(z).
\]
Thus the global theorem, the operator equality, and the resolvent-trace bridge
have the same basiswise weld, and no scalar trace equality is used to infer a
zero-by-zero identification.
For the paper's stated primary-3D/shadow-1D conditional, Lean packages the
whole implication in
\path{globalCoordinateIdentification_basisResolvedBridge_and_carrierTrace_of_allNative3DSourceTransfer}.
The theorem consumes one native 3D source transfer for each analytic shadow
and returns the all-good-height coordinate identity, every basis-resolved
kernel equality, and the summed carrier-height trace together.  In particular,
the half-unit is delivered inside the native no-drift/area-law transfer and is
not repeated as an independent assumption of this assembly theorem.
The upper-half-plane census has now been joined to the complete two-sided analytic
channel in
\path{RequestProject/ZetaContourXiReceiverIdentification3D.lean}.
The theorem \path{conj_mem_nontrivialZeros} transports genuine zeros by the
real structure of the entire xi function, while
\path{nontrivialZero_im_ne_zero} consumes the independently proved zero-free
region \(0<\operatorname{Re}s<1\), \(\lvert\operatorname{Im}s\rvert<2\), to
exclude a real-axis member.  Consequently
\path{upperNoRadialDrift_iff_allNoRadialDrift} promotes the upper no-drift
...
\]
compiled as
\path{globalCoordinateIdentification_iff_xiChannel_offReal_regular}.
...
\path{globalCoordinateIdentification_and_upperNative3D_of_selfAdjointXiReceiver}
...
\path{RequestProject/CarrierUnboundedResolvent3D.lean}.  Because the carrier
...
\path{carrierThreeDShift_comp_resolvent} and
\path{carrierThreeDResolvent_comp_shift}.  Moreover,
\path{carrierBasisResolventReadout_regular_off_real} proves that every
...
\begin{remark}[The increments carry the zero statistics]\label{rem:st-statistics}
Rearranged, the compiled interval law reads
\[
N^{\mathrm{mult}}\bigl((a,b]\bigr)
=\bigl(S_{\mathrm{mult}}(b)-S_{\mathrm{mult}}(a)\bigr)
+\frac1\pi\int_a^b\operatorname{clockRate}(t)\,dt,
\]
so on any family of windows the fluctuation of the zero count about the
deterministic clock term \emph{is} the \(S_{\mathrm{mult}}\)-increment:
number variance is the variance of \(dS_{\mathrm{mult}}\)-increments, and
two-point counting statistics (pair correlation in the sense of
Montgomery \cite{Montgomery}) are autocorrelation statistics of the same
increments.  This reading is an immediate rearrangement of the compiled
\texttt{Smult\_interval\_clock\_jump}; the statistical laws themselves
(e.g.\ GUE agreement) are a separate empirical tier and are not claimed
here.
\end{remark}
...
\noindent\textbf{The audited frontier, discharged.}  The two arrows between the
windowed identity and the full operator statement are now both proven, and we
record them at exactly their strength.  (i)~The mode dictionary carries
multiplicity: the event's mode space is the local algebra
\(\mathbb C[X]/((X-\rho_\gamma)^{m_\gamma})\), whose dimension is the vanishing
order (\texttt{GradedModeDictionary.finrank\_modeSpace}), so the kernel is
graded by \(m_\rho\) rather than witnessed one vector per event; the graded
traces equal the windowed trace and the log-derivative residue sum
(\texttt{gradedWindowTrace\_eq\_windowedTrace},
\texttt{gradedResolventTrace\_eq\_residue\_sum}) --- operator traces on genuine
\(m\)-dimensional local algebras \emph{are} the \(S_{\mathrm{mult}}\) jump
ledger.  (ii)~The \(T\to\infty\) limit is taken by differencing, exactly as
anticipated: the raw sum \(\sum_\rho m_\rho/(\gamma_\rho-w)\) diverges, but the
differenced resolvent \(\sum_\rho m_\rho\bigl((\gamma_\rho-w)^{-1}
-\gamma_\rho^{-1}\bigr)\) converges absolutely, and the windowed differenced
trace converges to it (\texttt{DifferencedResolvent.windowedDiffResolvent\_%
tendsto}), the event windows being a monotone cofinal exhaustion.  Both are
unconditional; neither asserts where zeros lie --- the limit object is a
statement about the \emph{sourced} spectrum, the event ledger.

\subsection{The exact prime--zero bridge}\label{sec:st-bridge}

The global coordinate \(S_\Gamma\) of \S\ref{sec:carrier-S} admits an exact
finite-height expansion into arithmetic data on one side and individual
zero contributions on the other.

\begin{theorem}[The explicit \(S(t)\) bridge]\label{thm:st-explicit-bridge}
There is a single global constant \(A\in\mathbb C\) --- the Hadamard
constant of \(\xi'/\xi\) --- such that at every good height \(T\),
\[
\boxed{\;
\pi\,S_\Gamma(T)
=\Im\!\Bigl(i\!\int_0^T\frac{\xi'}{\xi}(2+iy)\,dy\Bigr)
-\frac32\,\Im A
-\sideset{}{'}\sum_{\rho}m_\rho\,P_T(\rho)
-\vartheta(T)-\pi,\;}
\]
where the primed sum runs over the actual nontrivial zero multiset ---
each zero \(\rho\) weighted by its analytic order \(m_\rho\), with no
on-line hypothesis --- and
\[
P_T(\rho)
=\Im\int_{1/2}^{2}\Bigl(\frac1{x+iT-\rho}+\frac1\rho\Bigr)\,dx
\]
is the zero packet: the Hadamard pair term of \(\rho\) read on the top
segment.  The packet sum converges absolutely, and the identity is
unconditional.
\end{theorem}

The Lean spine, in \texttt{RequestProject/StExplicitBridge.lean}: the
rectangle-to-argument rung
\texttt{pi\_mul\_classicalSContour\_eq\_contour\_im}; the Hadamard
substitution with dominated interchange
\texttt{horizontal\_integral\_eq\_packets} (majorized by the compiled
zero-density bound); absolute convergence
\texttt{summable\_packet\_integral}; and the capstone
\texttt{exists\_stExplicitBridge}.

\begin{remark}[The arithmetic side is \(O(1)\); the constant term
vanishes]\label{rem:st-bridge-arithmetic-side}
Both non-zero terms of the bridge are now evaluated outright.

\emph{The constant.}  \(\xi(s)=(s-1)\pi^{-s/2}\Gamma(s/2+1)\zeta(s)\) has real
Taylor coefficients, so \(A=(\xi'/\xi)(0)\) is real:
\[
 A=-1-\frac{\gamma_{\mathrm E}}{2}+\log 2+\frac12\log\pi
  =-0.023095708966121\ldots,
\]
the classical Hadamard constant.  Hence \(\Im A=0\) and the term
\(-\tfrac32\Im A\) drops identically, leaving
\[
\boxed{\;
\pi\,S_\Gamma(T)
=\Im\!\Bigl(i\!\int_0^T\frac{\xi'}{\xi}(2+iy)\,dy\Bigr)
...
\emph{The arithmetic side.}  Since \(ds=i\,dy\) on the segment, the vertical
leg is a continuous logarithm increment, \(\arg\xi(2+iT)\), which splits as
\[
 \arg(2+iT)+\arg(1{+}iT)-\frac T2\log\pi
 +\Im\log\Gamma\!\Bigl(\frac{2+iT}{2}\Bigr)+\arg\zeta(2+iT),
\]
each branch continuous from \(0\).  Only the last term is arithmetic, and on
\(\Re s=2\) the Euler product gives \(\log\zeta(s)=\sum_{p,k}(k\,p^{ks})^{-1}\)
absolutely, so
\[
 \arg\zeta(2+iT)=-\sum_{p,k}\frac{\sin(kT\log p)}{k\,p^{2k}},
\]
dominated termwise by \(1/(k p^{2k})\).  Therefore, for \emph{every} \(T\),
\[
 \bigl|\text{arithmetic part of the vertical leg}\bigr|
 \;\le\;\sum_{p,k}\frac1{k\,p^{2k}}=\log\zeta(2)=0.497700\ldots,
\]
contributing at most \(\log\zeta(2)/\pi=0.1584\) to \(S_\Gamma\).  The prime
side of the bridge is thus bounded by an absolute constant: all growth in the
vertical leg is archimedean, and the size of \(S\) lives entirely in the zero
packets.

\emph{The packets.}  With \(\rho=\beta+i\gamma\) and \(a=T-\gamma\),
\(P_T(\rho)=\operatorname{atan2}(a,2-\beta)-\operatorname{atan2}(a,\tfrac12-\beta)
-\tfrac32\Im(1/\rho)\); on the midline the middle term is a signed right angle,
\[
 P_T=\arctan\frac{2(T-\gamma)}3-\frac\pi2\operatorname{sgn}(T-\gamma)
     +\frac{3\gamma/2}{\tfrac14+\gamma^2},
\]
and summing a conjugate pair \(\{\gamma,-\gamma\}\) cancels the \(1/\rho\)
terms exactly, leaving a pair contribution of size \(3T/\gamma^2\) --- the
absolute convergence of the theorem, made quantitative.

\emph{Measured} (\texttt{tmp/att018\_vertical\_leg.py},
\texttt{tmp/att019\_packets.py}).  The von Mangoldt series reproduces
\(\arg\zeta(2+iT)\) to \(1.1\times10^{-7}\) over \(T\in[0.5,10^{6}]\); the
bound is essentially sharp, \(\max|\arg\zeta(2+iT)|=0.4278\) over \(20{,}001\)
samples in \(T\in[0,7400]\) with rms \(0.1975\); at \(T=10^{5}\) the \(\Gamma\)
term is \(4.910\times10^{5}\) against an arithmetic term of \(+0.27\).  On
\(22{,}491\) zeros to \(\gamma=20000\) (census gated by grid refinement; the
Riemann--von Mangoldt prediction is \(22{,}491.4\)), the pair decay matches
\(3T/\gamma^2\) to within \(0.8\%\), and the identity
\(\sum_\rho m_\rho P_T(\rho)=\arg\xi(2+iT)-\arg\xi(\tfrac12+iT)\) --- computed
by two disjoint routes, the left from the zeros and the closed form, the right
from \(\Gamma\) and \(\zeta\) --- verifies with residual \(9.1\times10^{-7}\)
at \(T=200\) (\(5.5\times10^{-7}\) relative), the residual growing as the
second-order density tail.  Register: \emph{measured}, an independent
reproduction of the compiled statement, never its proof.
\end{remark}

The reading: the vertical leg lives at \(\Re s=2\), inside the
Euler-product regime --- it is the arithmetic side, and its expansion into
the von Mangoldt series is finite bookkeeping on absolutely convergent
data.  That bookkeeping is performed in
Remark~\ref{rem:st-bridge-arithmetic-side}, where it is shown to be
bounded by an absolute constant.  Each zero contributes one
convergent packet; the clock \(\vartheta\) and the endpoint half-turn are
retained exactly.  Thus \(S(t)\) is an exact finite-height transport law
between the prime side and the individual zero packets --- the third
outward face of the registration object whose census face is
\eqref{eq:global-coordinate-defect} and whose measure face is the
clock--jump rule.

\medskip
\noindent\textbf{The \(S(t)\) theorem cluster.}  The compiled package of
this section and \S\ref{sec:carrier-S} presents one object in four
registers:
\[
\boxed{\ \text{phase defect}
\;=\;\text{atomic residue ledger}
\;=\;\text{spectral measure}
\;=\;\text{scale-registration cocycle},\ }
\]
with two outward-facing global laws: the census
\(S_\Gamma-S_{\mathrm{mult}}=N_{\mathrm{off}}^{\mathrm{mult}}\)
\eqref{eq:global-coordinate-defect} and the explicit prime--zero bridge
(Theorem~\ref{thm:st-explicit-bridge}).  The registers: the compensation
theorem (Theorem~\ref{thm:carrier-scale-S}) types the defect; the exact
clock--jump rule with
\(dS_{\mathrm{mult}}=\sum_{\gamma}m_\gamma\,\delta_\gamma
-\pi^{-1}\operatorname{clockRate}\,dt\) makes it an atomic measure; the
windowed trace (Theorem~\ref{thm:windowed-trace}) with the convergent
differenced resolvent makes the same atoms a spectral measure; and the
coboundary laws (Proposition~\ref{prop:sgap-coboundary}) make the defect
a cocycle of the harmonic-scale family.  Every displayed equality is
machine-checked; the statistical and von Mangoldt readings
(Remark~\ref{rem:st-statistics}, above) are corollary tiers, labeled as
such.
...
character $\omega_{\Pi_r}=\omega_\pi^{\,r(r+1)/2}$ and, by Theorem~\ref{prop:localid},
...
$\Lambda_C=N^{s/2}L_{\mathrm{CPS}}$ (Thm.~\ref{prop:localid}(E5)) it is the CPS functional equation with
...
(Thm.~\ref{prop:localid}(E5)), so the proved reflection is already the CPS functional equation, with
...
\paragraph{Compiled warp-to-CPS endpoint.}
The formal endpoint now returns the full automorphic object, rather than stopping at
pole extinction.  A degree-generic rung package
\texttt{RungCapstone.RungWarpData} supplies the three prime warps; the compiled rigidity theorem
\texttt{RungCapstone.rung\_polar\_carrier\_zero} annihilates its polar correction.  In the CPS
section, \texttt{ThreeDConverse.cpsRungFullAutomorphy3D\_ofProfileCouplings} then packages that
extinction together with
\texttt{ThreeDConverse.symmetricPowerFunctoriality3D\_ofProfileCouplings}.  Its returned
\texttt{CPSRungAutomorphicLift3D} retains simultaneously
\begin{enumerate}
\item the restricted-product representation with the literal arithmetic $\Sym^r$ local roots;
\item rational-quotient cuspidality;
\item the entire, vertical-strip-bounded functional equation for every CPS twist
$1\le m<r$; and
\item the identity $R_{r,\tau}\equiv0$ for the rung polar carrier.
\end{enumerate}
Thus the compiled CPS conclusion is a first-class cuspidal symmetric-power lift with its
pole-extinction certificate attached, exactly the object consumed by the following automorphic
landing theorem.  The axiom audit of the endpoint is
...
map is surjective, forcing $r+1\le m\le r-1$.  This is Schur's lemma for the genuine group action:
in $\mathrm{FDRep}\,k\,G$ a morphism between the simple objects $\varphi_\tau^\vee$ and
$\Sym^r\varphi_\pi$ is zero unless they are isomorphic, and the dimension mismatch $m\ne r+1$ already
forbids the isomorphism.  The exclusion is machine-checked in exactly this general form ---
\texttt{NonSelfDual.hom\_zero\_of\_finrank\_lt}, with Schur's core
\texttt{hom\_zero\_of\_simple\_not\_iso} (non-isomorphic simple $G$-representations have zero
$\operatorname{Hom}$, via \texttt{FDRep.finrank\_hom\_simple\_simple}) and the dimension step
\texttt{not\_iso\_of\_finrank\_ne}.  Dimension supplies \emph{only} the non-isomorphism; the vanishing
is Schur with the $G$-action present, never an ordinary vector-space rank bound.  This is the
representation-theoretic exclusion consumed by the residual calculation. So the entire carrier readout
\emph{is} $L(s,\Pi_r\times\tau)$, in the standard pattern of Tate and Godement--Jacquet: on the
initial half-plane the two agree \emph{as convergent series} by the coefficient identification
(Thm.~\ref{prop:localid}), so the entire carrier readout is \emph{itself the continuation} of
$L(s,\Pi_r\times\tau)$ --- exhibiting an entire function agreeing with the series on its half-plane
is what ``continues to an entire function'' means, and is exactly what the CPS hypothesis asks;
nothing about the target is presupposed. The identity theorem enters only as uniqueness
bookkeeping --- any continuation coincides with this one
(\texttt{CarrierTargetIdentification.carrier\_FE\_transfers\_to\_target}, universal core
\texttt{entire\_eq\_of\_eqOn\_isOpen}, its target-entirety hypothesis instantiated by the carrier
object itself). Every hypothesis of Theorem~\ref{thm:cps} is met (the ledger above), so $\Pi_r$ is a
...
--- and the classical reading passes through the named identification layer, Thm.~\ref{prop:localid}.)
\end{corollary}
...
That the ledger's substantive rows are genuinely substantive is itself compiled, as a
falsifiability instrument (\texttt{CPSUnconditionalityAudit3D}): an arbitrary group action does
not make an arbitrary readout invariant
(\texttt{not\_all\_readouts\_invariant\_under\_all\_group\_moves}), and an integrable unipotent
kernel need not be cuspidal (\texttt{unitConstantKernel\_not\_cuspidal}) --- the engine's
hypotheses cannot be discharged by genericity, only by genuine modularity and cuspidality
content.  The division of labor is equally explicit: on the functoriality route the
automorphic/cuspidal \emph{output} is delivered by the cited converse theorem at its published
strength (Theorem~\ref{thm:cps}), consumed exactly as local Langlands and Deligne--Tate are
consumed; the engine of this section is the internalized generator descent, whose ledger names
what a full internalization must supply --- an audit of that program, never a second,
independent proof of the citation.
...
The forward geometric assembly is now typed separately from every analytic continuation.  The
structure \texttt{ArithmeticCPSCarrierSeed3D} carries two independently defined functions on the
global carrier, one on the helix and one on the anti-helix, together with their global involution
exchange and the exact positive-height identifications of their completed logarithmic readouts with
the literal primal and contragredient coefficient/Gamma banks.  The two functions are deliberately
distinct: inverse radial Satake weights ride the contragredient lane and are not silently replaced by
complex conjugates.  The theorem \texttt{completedLogTheta\_pair\_one\_div} transports this paired
carrier exchange through logarithmic inversion and the self-dual envelope.  Consequently
\texttt{reflection\_of\_carrierSeed} derives the literal off-weld bank reflection, and
\texttt{ofCarrierSeed} constructs the required \texttt{ArithmeticCPSReflectedThetaSource}; neither
statement consumes an \texttt{ArithmeticCPSAnalyticCandidate3D} or a pre-existing theta reflection.

The reverse assembly direction is also canonical.  Given an
...
This is now an explicit construction rather than only a consumer interface:
\texttt{ArithmeticCPSAllTwistsConverseCandidate3D.ofReflectedThetaSources} takes the independently
defined restricted-product representation, the bank bridge, one literal
\texttt{ArithmeticCPSReflectedThetaSource} for every required twist and completion, and the
equivariant residual family.  Its analytic field is definitionally the
\texttt{analyticCandidate} derived from that same reflected source, so the continuation,
identification, reflection, and niceness data cannot be imported from a different Euler bank.
...
There is now a forward all-twist entry point as well:
\texttt{ArithmeticCPSAllTwistsConverseCandidate3D.ofCarrierSeeds} accepts the independently defined
paired carrier seed for each twist and constructs every reflected source and analytic candidate
internally.  Thus the geometric route does not pass through
\texttt{ofAnalyticCandidate}; its only identification boundary is the explicit equality of the two
literal arithmetic banks with the helix and anti-helix carrier readouts.  The object-valued endpoint
\texttt{symmetricPowerFunctoriality3D\_ofCarrierSeeds} then returns the cuspidal symmetric-power
lift from that same family of paired carrier seeds, the restricted-product representation, the bank
landing, and the equivariant residual channels; it does not accept a separately supplied analytic
candidate.

For a genuine level-one GL$(2)$ source, the carrier exchange itself is now constructed directly
from modularity.  If $f$ has even weight $2n$, write
$W_{2n}(x)=x^{-n}e^{-x-x^{-1}}$ for the fixed completion envelope and define the normalized
vertical carrier by
\[
 E_f(z)=\frac{f\!\left(i e^{\Im z}\right)}
 {W_{2n}\!\left(e^{\Im z}\right)}.
\]
Mathlib's slash transformation under $S$ gives
$f(i/x)=(xi)^{2n}f(ix)$, while
$W_{2n}(x^{-1})=x^{2n}W_{2n}(x)$; hence
$E_f(\overline z)=i^{2n}E_f(z)$.  This is the compiled theorem
\texttt{GenuineGL2Carrier.deEnvelopedVerticalCarrier\_exchange}.  Moreover the completion is
not silently inserted twice:
\texttt{completedLogTheta\_deEnvelopedVerticalCarrier} proves exactly
$W_{2n}(x)E_f(i\log x)=f(ix)$ for every $x>0$.

The coupling type this exchange first fed is itself audited at the type level, and the audit is
a finding: a coupling demanding that the \emph{twisted} bank readout equal the raw modular
vertical value is \emph{uninhabitable on genuine data} --- the $(r{+}1)m\ge2$-clock tensor
completion kernel decays like $e^{-c\sqrt x}$ while a cusp form's vertical restriction decays
like $e^{-2\pi x}$, and the bank's leading coefficient is $1$, so no instance exists at any
$(r,m)$ (the finding is recorded in the retired structure's docstring,
\texttt{ArithmeticBankCoupling3D}).  The corrected per-rung obligation is the \emph{theta
profile pair} (\texttt{BankThetaProfileCoupling3D}): two functions on the positive ray, one
pointwise weighted reflection between them, and the two identifications of the literal bank
readouts with those profiles --- nothing else.  Its constructors discharge everything
downstream: \texttt{toReflectedThetaSource} builds the reflected strong-pair source with
integrability and rapid decay derived from the polynomial coefficient bounds;
\texttt{toCarrierSeed} \emph{derives} the seed's conjugation exchange from the pointwise
reflection and the envelope inversion; \texttt{ArithmeticCPSCarrierSeed3D.toProfileCoupling} is
the converse, so the seed-typed and profile-typed obligations interconvert and the per-rung
frontier is exactly one pointwise theta reflection; and \texttt{allProfileCarrierSeeds} packages
per-twist couplings into the seed family the all-twist constructor
(\texttt{ofCarrierSeeds}) consumes.

The chain downstream of the coupling is one compiled endpoint
(\texttt{symmetricPowerFunctoriality3D\_ofProfileCouplings}): from the restricted-product
representation, the bank bridge, the residual channels, and \emph{one theta-profile coupling per
CPS twist}, the object-valued cuspidal symmetric-power lift is returned with the carrier seeds,
reflected sources, and analytic candidates all constructed internally --- the per-rung analytic
obligation of the entire converse chain is the coupling family and nothing else.  The
cited-classical entry point into that family is likewise compiled
(\texttt{BankThetaProfileCoupling3D.ofMellinFE}): a completed Mellin-space functional equation
between the literal prescribed banks, with the standard Mellin-inversion integrability
conditions, \emph{derives} the pointwise profile reflection
(\texttt{theta\_reflection\_of\_mellin\_functionalEquation}) and constructs the coupling ---
the shape in which the classical low rungs deliver their functional equations
(Langlands--Shahidi, Kim--Shahidi for $r\le4$ \cite{Shahidi,KimShahidi}), consumed as data
exactly as local Langlands and Hecke multiplicativity are consumed at their typed fields.
Four of that converter's six analytic side conditions are themselves discharged from the
compiled polynomial-decay layer (\texttt{CPSMellinFEDischarge}): primal Mellin convergence at
every initial-domain weight
(\texttt{cpsPolynomialFullPrimal3DBankReadout\_mellinConvergent}), continuity of the primal bank
and of the reflected contragredient readout on the positive ray --- an M-test over the compiled
rapid decay (\texttt{theta\_continuousAt\_of\_polynomial},
\texttt{reflectedDualReadout\_continuousAt}) --- and the kernel-continuity input itself for
every singleton clock (\texttt{conductorScaledCompletionKernelLog\_singleton\_continuousOn}).
The refined converter (\texttt{BankThetaProfileCoupling3D.ofMellinFE'}) therefore consumes
exactly the continuation content and nothing else: the completed Mellin functional equation
between the literal banks, the reflected-side Mellin convergence, and the two vertical
integrabilities.  The two vertical integrabilities are then themselves \emph{discharged}
(\texttt{CPSCompletedVerticalIntegrable}): the house tree carries the unconditional two-sided
vertical Stirling bound $|\Gamma(\sigma+it)|\le C|t|^{\sigma-1/2}e^{-\pi|t|/2}$
(\texttt{StirlingBound.gamma\_stirling\_bound}), which \texttt{gammaC\_vline\_integrable} turns
into vertical integrability of the completion's $\Gamma_{\mathbb C}$ factor; along the
initial-domain line the compiled identification displays the bank's Mellin transform as
$C^{s}\prod_j\Gamma_{\mathbb C}(s{+}\mu_j)\,D(s)$ with $D$ a bounded Dirichlet series, so
\texttt{completedPrimal\_verticalIntegrable} integrates the whole readout, and
(\texttt{mellin\_reflectedDual\_eq\_primal}) the functional equation makes the reflected transform
literally equal the primal one --- discharging both fields.  The fully-discharged converter
(\texttt{BankThetaProfileCoupling3D.ofMellinFE''}) thus consumes exactly the completed Mellin
functional equation and the reflected-ray Mellin convergence: the per-rung obligation is one
functional equation plus one ray-convergence, with nothing about vertical decay remaining.

The base rung of this corrected type is \emph{inhabited on a genuine automorphic object}.
\texttt{GenuineGL2Carrier.StandardRungEulerData} is the standard (degree-two) certificate of an
actual level-one cusp form in the det-one analytic chart --- its one arithmetic field identifies
the form's canonical Mathlib \texttt{qExpansion} coefficients with the det-one-normalized Euler
bank at shift $n-\tfrac12$ (Hecke's multiplicativity, a cited classical input typed as a
consumed field, exactly as local Langlands is consumed by Thm.~\ref{prop:localid}); the
contragredient-bank equality is \emph{derived}, not certified --- det-one forces the dual weight
family to be the swap of the primal, and the Euler coefficients are symmetric functions of the
weights (\texttt{radialLocalEulerCoeff\_comp\_equiv},
\texttt{cpsPolynomialDualCoeff\_eq\_primal\_of\_detOne}), so the constructor
\texttt{StandardRungEulerData.ofDetOne} builds the certificate from Hecke multiplicativity and
the Satake normalization alone.  From that
certificate alone: \texttt{theta\_standard\_eq} proves the bank's completed coefficient theta
equals the enveloped modular vertical value (Mathlib's q-expansion convergence plus exact
$\Gamma$-clock evaluation --- the envelope algebra \emph{forces} the analytic chart: weight one,
shift $n-\tfrac12$, exchange constant $i^{2n}$); \texttt{standardRung\_reflection} derives the
literal-bank reflection from \texttt{hecke\_inversion} --- the modular slash law on the vertical
line --- consuming no analytic candidate, no reflected source, and no supplied theta reflection;
\texttt{StandardRungEulerData.toReflectedThetaSource} then constructs the complete
\texttt{StrongFEPair} whose two functions \emph{are} the literal bank readouts (by \texttt{rfl}),
with entirety of the completed transform and the global functional equation
$\Lambda(1-s)=i^{2n}\,\Lambda^\vee(s)$ as outputs
(\texttt{standardRung\_completed\_entire}, \texttt{standardRung\_functional\_equation}); and
\texttt{toProfileCoupling} places the rung inside the corrected coupling type --- its
non-vacuity certificate on genuine modularity.  The completion this pair carries is the
\emph{standard} one, not a synthesized kernel: the rung's clock is the prescribed
$\Gamma$-clock at conductor one and Deligne shift $n-\tfrac12$, and on the initial half-plane
the pair's completed transform \emph{is} the literal standard completed readout
$1^{s}\,\Gamma_{\mathbb C}(s+n-\tfrac12)\,D(s)$ --- Mathlib's own $\Gamma_{\mathbb C}$ ---
equivalently one half of the classical completed Hecke series
$\Gamma_{\mathbb C}(s+n-\tfrac12)\sum_m a_f(m)\,m^{-(s+n-1/2)}$ of the actual form
(\texttt{standardRung\_Lambda\_eq\_standardCompletion},
\texttt{standardRung\_dualLambda\_eq\_standardCompletion},
\texttt{standardRung\_Lambda\_eq\_heckeCompletion}, instantiating the general
prescribed-completion identification
\texttt{cpsPolynomialFullCompletion3D\_identification}).  Bundled, this is Hecke's classical
theorem for the form as one compiled statement about one object
(\texttt{standardRung\_classicalHeckeTheorem}): entirety with entire dual, the classical
completed Hecke series on the initial half-plane on both legs, and the global functional
equation with the standard conductor, $\Gamma$-factor, contragredient, and root number ---
the functional equation of the standard completed automorphic $L$-function of the genuine
form, not of an auxiliary kernel.  The synthesized-kernel constructor
(\texttt{CarrierTheta.automaticCoefficientThetaStrongFEPair}) remains the generic,
hypothesis-free payload; the CPS-facing chain --- the reflected sources, the profile couplings,
and the all-twist converse candidates --- is typed on the prescribed
conductor/$\Gamma$-product banks throughout, so no synthesized completion stands in for the
standard one anywhere on the identification path.  All at the standard footprint, no
\texttt{sorry}.

At the representation-theoretic layer the symmetric-power action is now constructed rather than
named.  The Lean file \texttt{RepresentationSymmetricPower3D} descends a linear endomorphism through
Mathlib's symmetric-tensor quotient, proves preservation of identity and composition, and defines
\texttt{Representation.symmetricPower}.  Consequently
\texttt{RestrictedSymmetricPowerRepresentation3D.ofBaseLocalRepresentations} builds every finite
local $\operatorname{Sym}^r$ component from the corresponding genuine base GL$(2)$ component, while
\texttt{ofBaseAdelicRepresentation} applies the same functor to the base representation of the
archimedean-times-restricted-product group and uses the resulting action definitionally on the
global symmetric carrier.  The remaining assembly is thereby separated cleanly into the adelic
\texttt{CPSBankBridge} on that carrier and the literal all-twist carrier/Gamma identification.

The representation-side structures are moreover \emph{inhabited}, with every status field proven
rather than supplied (\texttt{CPSRepresentationInstance3D}).  Three reusable bricks --- any
representation on a simple module is irreducible
(\texttt{Representation.isIrreducible\_of\_isSimpleModule}), the trivial representation is
smooth, and any finite-dimensional representation is admissible --- feed the first compiled term
of the restricted-product structure: the scalar (Hecke-character-shaped) instance
\texttt{scalarRestrictedSymmetricPowerRepresentation3D}, polymorphic over every archimedean and
finite group family, all irreducibility/smoothness/admissibility/compatibility fields theorems.
The equivariant residual channels exist whenever the Schur data holds
(\texttt{EquivariantCPSResidual3D.ofSchurData}), and by the compiled
\texttt{residue\_eq\_zero} the zero intertwiner is their \emph{only} inhabitant, so that
constructor is exhaustive.  At rank one, where the CPS twist range is empty, these terms
assemble a complete all-twist converse candidate and the object-valued cuspidal lift compiles
end-to-end (\texttt{scalarRankOneCuspidalLift}) --- the capstone chain executing on constructed
terms rather than hypotheses.  Register: a structural inhabitation of the interfaces; the
arithmetic content of higher ranks enters through the per-twist theta-profile couplings above,
and the genuine adelic readout remains the named Tate field of \texttt{CPSBankBridge} --- a
cited classical input (Tate's thesis), not an open claim.
...
proposition is left as a theorem parameter.  The place-by-place local factors, conductor, and root number are read \emph{natively} off the carrier,
not matched against an external object: the transverse block \emph{is} the Satake/Frobenius
conjugate-pair block definitionally (\texttt{FrobeniusSimilitude.frobeniusBlock\_eq\_conjPairBlock}, by
\texttt{rfl}), and deprojection is lossless and faithful unconditionally
(\texttt{ConeProjection.record\_bijective}).  So Theorem~\ref{prop:localid} is closed on the
carrier: the named automorphic local factors and $\varepsilon$-factors --- local Langlands and Deligne
\cite{Tate}, ramified monodromy included --- are the $1$D \emph{readout} of that native data, the name
of the shadow rather than an arithmetic input the carrier still owes.  The carrier completion,
...
generators suffice.  The cusp-form instance is now compiled as well (\texttt{CPSCuspSeed3D}):
for an actual level-one \texttt{CuspForm}, full $\mathrm{SL}(2,\mathbb Z)$-invariance of the
readout lands through the same audited engine (\texttt{cuspSeedReadout\_landing}), and the
cuspidality-side content is likewise \emph{derived} --- the readout is exponentially small at
the cusp (\texttt{cuspSeedReadout\_exp\_decay}, from Mathlib's
\texttt{CuspFormClass.exp\_decay\_atImInfty}).  At the base of the tower both substantive
inputs of the audit's warning --- invariance and cusp decay --- are theorems of the genuine
...
unit (Thm.~\ref{prop:localid}); the unimodular chiral block
...
\medskip\noindent\textbf{The reframe: temperedness is a helix, non-temperedness a spiral.}  On the
carrier, $|\alpha_p|=1$ is the statement that the strand is a \emph{helix} --- an elliptic rotation
$e^{i\theta}$, the transverse shadow of the constant-radius $3$D trajectory --- rather than a
\emph{spiral}, a real off-circle stretch.  This dictionary is machine-checked
(\texttt{HelixStrandTemperedness}, standard footprint): the strand is a helix iff $\|\alpha_p\|=1$
(\texttt{isHelixStrand\_iff\_norm\_one}); a helix's chart readout is the tempered
$a_p=2\cos\theta\in[-2,2]$ (\texttt{helix\_strandTrace\_mem\_Icc}) while a spiral reads $|a_p|>2$
(\texttt{spiral\_strandTrace\_gt\_two}); and a nonzero real-trace strand is a helix or a spiral and
nothing else (\texttt{strand\_dichotomy}) --- the radial-channel twin of
\texttt{SourceHolonomy.source\_dichotomy}.  The forcing direction \emph{helix source $\Rightarrow$
tempered} is proven (\texttt{tempered\_of\_hasHelixSource}); that \emph{every} Frobenius has a helix
source --- no spiral orphan (\texttt{EveryFrobeniusHasHelixSource}) --- is the \emph{favored branch},
the radial twin of \texttt{EveryZeroHasSource}, grounded in the proven Frobenius similitude and never
asserted as a from-$1$D proof.  The Jacquet--Shalika ceiling used below is the classical $1$D
realization of that same \emph{no spiral orphan}.
...
Satake multiset $\{\alpha_p^{\,r-2k}\}_{k=0}^{r}$ at $p$ (Thm.~\ref{prop:localid}). The top and
...
bounds only (Lemma~\ref{lem:bounded}, where Ramanujan is not used; Thm.~\ref{prop:localid},
...
The non-circularity is moreover a compiled \emph{control}, not only a register sentence
(\texttt{RadiusLiveSpiralWitness}).  The maximally non-tempered det-one datum
\texttt{spiralPair} --- strand radius $p$ at every prime, a genuine spiral, never unit-modulus
(\texttt{spiralPair\_radius}, \texttt{spiralPair\_not\_unitary}) --- passes the entire
radius-live niceness payload verbatim: entirety and the global functional equation of its every
CPS twist (\texttt{spiralPair\_twisted\_entire}, \texttt{spiralPair\_twisted\_FE}).  So the
niceness layer provably encodes no temperedness: it accepts the spiral.  The same spiral
violates the rank-uniform tower ceiling already at the first rung
(\texttt{spiralPair\_violates\_towerCeiling}), so the radial capstone
(\texttt{strand\_radius\_one\_of\_tower\_ceiling}), whose only inputs are the two ceilings,
correctly refuses it: unit modulus enters the chain exactly once, as the radial-limit
\emph{conclusion} from the ceilings the automorphic tower supplies, and nowhere as an input.
\end{remark}
...
\begin{center}
\fbox{\parbox{0.94\textwidth}{\textbf{Unconditional equidistribution closure from the CPS
automorphic tower.} The exact passage from literal arithmetic symmetric-power prime-trace
cancellation to three-dimensional equidistribution is machine-checked in Lean~4 at the standard footprint
$\{\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound}\}$, no \texttt{sorry}.  The
all-rank tower first derives $|\alpha_p|=1$; \texttt{ArithmeticSatakePrimeFamily.angle} then extracts
$\theta_p=|\arg\alpha_p|\in[0,\pi]$; and
\texttt{ArithmeticSatakePrimeFamily.character\_eq\_satakeTrace} proves
$U_r(\cos\theta_p)=\operatorname{Re}\sum_{j=0}^{r}
\alpha_p^{,r-j}(\alpha_p^{-1})^j$.  These are the literal local symmetric-power roots
(\texttt{symmetricPowerSatakeTrace\_eq\_sum\_roots}), and
\texttt{primeTestAverage\_eq\_arithmetic} identifies the finite averages term-by-term.  Thus no
independently supplied angle bank or one-dimensional chart intervenes.  From that exact arithmetic
bank, the
method-of-moments density bridge --- every polynomial trace test
(\texttt{polynomial\_tendsto}), every bounded-continuous test via the Weierstrass theorem after the
$\arccos$ change of coordinate (\texttt{integral\_tendsto}), and the carrier convergence
(\texttt{empiricalPrimeCarrierMeasure\_tendsto\_of\_characterAveragesZero}) --- together with the
Sato--Tate moment computations (\texttt{chebyshev\_U\_angleMeasure\_integral\_zero}). The
analytic input consumed by this density bridge is cancellation of the literal arithmetic prime traces
$(N+1)^{-1}\sum_{k\le N}\operatorname{Re}\sum_{j=0}^{r}
\alpha_{p_k}^{,r-j}(\alpha_{p_k}^{-1})^j\to0$ for each $r\ge1$
(\texttt{ArithmeticSymmetricPowerPrimeCancellation}) --- the Wiener--Ikehara step that is
\emph{provably on the equidistribution critical path}: the machine-checked biconditional
\texttt{arithmeticSatakeCancellation\_iff\_carrierEquidistribution} (standard footprint, no
\texttt{sorry}) proves this cancellation is not merely sufficient but \emph{equivalent} to weak
convergence of the empirical
prime-angle measures to the Sato--Tate law, so --- unlike the off-path summatory shadow of
\S\ref{sec:aristotle-rs} --- discharging it \emph{is} the Sato--Tate conclusion.  It is discharged
below from the symmetric-power functoriality of item~1.  For every $r\ge1$, the standard
$L$-function of the cuspidal lift $\Pi_r=\Sym^r\pi$ is entire, Jacquet--Shalika nonvanishing gives
$L(s,\Pi_r)\ne0$ on $\Re s=1$, and the automorphic prime number theorem applied to
$-L'/L(s,\Pi_r)$ gives the logarithmically weighted character cancellation.  The already-derived
temperedness makes the prime-power tail $O_r(X^{1/2}\log^2X)$; partial summation then gives exactly
the unweighted averages in \texttt{ArithmeticSymmetricPowerPrimeCancellation}.}}
\end{center}

\medskip
\noindent The family is now also instantiated at the campaign's \emph{typed} Maass
seed (\S\ref{sec:mechanism-converse}): \texttt{maassSatoTateFamily} builds the
arithmetic Satake tower of a \texttt{MaassEigenData} under the typed per-rung
ceiling, \texttt{maass\_temperedness} derives part~(i) at the seed through the
compiled radial limit, and \texttt{maass\_satoTate\_iff} instantiates the
biconditional --- so the Sato--Tate chain and the automorphy chain (packages,
converse, wave-form identification) consume one and the same seed object, with the
same automorphic tower.  Both consequences are derived at their stated strength.  The ceiling enters
polynomially: the per-rung classical input may carry constants growing like the
$r{+}1$ channels of $\Sym^r$, and the radial limit still closes ---
$\rho^r\le C(r{+}1)^k$ for all $r$ forces $\rho\le1$, the exponential beating
every polynomial (\texttt{le\_one\_of\_pow\_le\_poly},
\texttt{maass\_temperedness\_poly}); once the radius closes at $1$ the uniform
ceiling is manufactured with $C=1$
(\texttt{MaassPolyCeiling.toCeiling}), so the family, the fork, and the shadow
detector inherit verbatim.  And the corollary's non-Galois-type hypothesis is
compiled rather than prose: an equidistributing seed admits no monomial shadow
at any finite order (\texttt{maass\_no\_shadow\_of\_ST}), and conversely every
finite-order seed provably fails the drift --- the exclusion and the fork are
the same theorem pair.

\smallskip
\noindent\emph{The closure, and its exactness.}  The corollary is now one
compiled implication (\texttt{maass\_satoTate}: polynomial ceiling and per-rank
cancellation in, temperedness and carrier equidistribution out), and the
assembly is terminal (\texttt{maass\_satoTate\_exact}): each input is
individually \emph{equivalent} to the conclusion it feeds --- the ceiling exists
iff the seed is tempered (\texttt{temperedness\_iff\_polyCeiling}), and the
cancellation is equivalent to the equidistribution (the compiled biconditional).
The interface has zero slack; reducing it further is proving the theorem's own
conclusion.

\smallskip
\noindent\emph{The CPS-to-Sato--Tate connection.}  The file
\texttt{CPSMaassSatoTate3D} now joins that exact interface to the object-valued
all-rank CPS conclusion.  The canonical base \texttt{maassCPSBase} is the
rank-two polynomial Satake pair of the typed Maass seed, and
\texttt{CPSMaassSymmetricPowerTower3D.ofRungAutomorphicLifts} consumes one
\texttt{CPSRungAutomorphicLift3D} for every $r$.  With the standard local bound
$\lVert\alpha_{p,j}(\Sym^r\pi)\rVert\le\sqrt p$ on those unitary automorphic
lifts, \texttt{CPSMaassSymmetricPowerTower3D.toCeiling} reads the extreme roots
$j=0,r$, identifies them with $\alpha_p^r$ and $\alpha_p^{-r}$, and constructs
the Maass tower ceiling.  The typed \texttt{CPSAutomorphicPrimeTheorem3D} is the formal boundary
occupied by the Jacquet--Shalika/automorphic-prime-theorem output on the resulting literal
arithmetic family; the proof below discharges that boundary mathematically from the CPS lifts.
Finally \texttt{cpsMaassSatoTate3D} returns both $|\alpha_p|=1$ and
convergence of the empirical native carrier measures to the Sato--Tate law.
Thus the compiled endpoint now consumes the actual all-$\Sym^r$ CPS lift tower,
not an unrelated angle bank or a separately postulated unit-radius family.

\smallskip
\noindent The Rankin--Selberg positivity that powers both classical engines is
extracted at the surface (\texttt{RSPositivity}): the
unit-strand trace is real, so the diagonal Rankin--Selberg weights are
nonnegative; the Rankin--Selberg square of a real self-dual surface is again a
surface with nonnegative Dirichlet readout at every real point --- Landau-ready
--- and the de la Vall\'ee-Poussin/Hoffstein--Lockhart positivity is exact in
trace coordinates: $2+2\,(S_1)_{\mathrm{re}}+(S_2)_{\mathrm{re}}
=(1+2\,\mathrm{Re}\,\alpha)^2$.  The CPS theorem supplies the all-rank continuation
that this argument needs; Jacquet--Shalika supplies boundary nonvanishing after
the converse landing, and the automorphic prime number theorem converts the
resulting logarithmic derivatives into the character cancellation calculated
...
There is now also no identification gap between the arithmetic prime roots and that character
boundary.  \texttt{ArithmeticSatakePrimeFamily} is indexed by the already identified
\texttt{ArithmeticSatakeTowerAtPrime}, so radius one is obtained from
\texttt{ArithmeticSatakeTowerAtPrime.radius\_one} before an angle is defined.  Lean proves
\[
 \operatorname{tr}(\Sym^r A_p)
 =\sum_{j=0}^{r}\alpha_p^{,r-j}(\alpha_p^{-1})^j
 =U_r(\cos|\arg\alpha_p|),
\]
and equality of the corresponding finite first-prime averages.  The capstone
\texttt{arithmeticSatakeCancellation\_iff\_}\allowbreak
\texttt{carrierEquidistribution} therefore has the actual
arithmetic Satake-trace average on its left and convergence of native 3D carrier measures on its
right; it does not declare that a separately chosen angle sequence represents the arithmetic
fiber.
\end{remark}
...
\emph{(ii) Equidistribution.} Now $\alpha_p=e^{i\theta_p}$, $\theta_p\in[0,\pi]$, and
$a_p=2\cos\theta_p$ is real.  Fix $r\ge1$ and put $\Pi_r=\Sym^r\pi$.  By the first paragraph,
$\Pi_r$ is a cuspidal automorphic representation of $\mathrm{GL}(r+1)$, and
Theorem~\ref{prop:localid} identifies its unramified standard local factor with
\[
 L_p(s,\Pi_r)=\prod_{j=0}^{r}
   \left(1-\alpha_p^{\,r-2j}p^{-s}\right)^{-1}.
\]
Godement--Jacquet holomorphy and Jacquet--Shalika nonvanishing \cite{GJa,JS} give
$L(s,\Pi_r)\ne0$ throughout $\Re s\ge1$.  Consequently its logarithmic derivative has the
Euler expansion
\[
 -\frac{L'}{L}(s,\Pi_r)
 =\sum_{p}\sum_{m\ge1}
   \frac{\operatorname{tr}(A_p(\Pi_r)^m)\log p}{p^{ms}}
 \qquad(\Re s>1),
\]
with no pole on the boundary line.  The Jacquet--Shalika automorphic prime number theorem
(equivalently its Wiener--Ikehara/de la Vall\'ee--Poussin proof) therefore gives
\[
 \Psi_r(X):=\sum_{p^m\le X}
   \operatorname{tr}(A_p(\Pi_r)^m)\log p=o_r(X).
\]
This is already the required arithmetic sequence, not an auxiliary angle model.  Indeed the
$m=1$ coefficient is, by the displayed local identification,
\[
 \operatorname{tr}(A_p(\Pi_r))
   =\sum_{j=0}^{r}\alpha_p^{\,r-2j}=U_r(\cos\theta_p).
\]
Moreover part~(i) gives
$|\operatorname{tr}(A_p(\Pi_r)^m)|\le r+1$.  Hence the terms with $m\ge2$ contribute
\[
 \ll_r\sum_{m\ge2}\sum_{p\le X^{1/m}}\log p
 \ll_r X^{1/2}\log^2X=o_r(X),
\]
so
\[
 B_r(X):=\sum_{p\le X}U_r(\cos\theta_p)\log p=o_r(X).
\]
Abel summation now gives
\[
 \sum_{p\le X}U_r(\cos\theta_p)
 =\frac{B_r(X)}{\log X}
   +\int_{2}^{X}\frac{B_r(t)}{t(\log t)^2}\,dt
 =o_r\!\left(\frac{X}{\log X}\right).
\]
...
\[
  ...
  \qquad(r\ge1).
\]
Thus the character averages consumed by
\texttt{ArithmeticSymmetricPowerPrimeCancellation} have been derived from the all-rank CPS tower.
The characters $U_r$ span the polynomial class functions and are uniformly
...
\section{The worked example, condensed}\label{sec:worked-example}

The simplest configuration the machinery admits is the \(\pi/3\) carrier
with an ordinary Dirichlet \(L\)-function riding it, and every mechanism the
paper generalizes can be watched there at full resolution.  The carrier is
the positive number line, rescaled: the integer \(n\) sits at physical
height \(Z=n\), a unit step advances the winding by \(\pi/3\), six steps
close a turn, so the finite harmonic cells are the \(\mu_6\) buckets and the
closure arithmetic is exact in \(\mathbb Z[\zeta_6]\) --- integer
arithmetic, no limit taken.  The carrier is double-ended --- helix and
conjugate anti-helix over a common parameter, exchanged by the involution
\(J\) --- and the functional equation of every readout is that involution
read phasor by phasor: what the classical theory obtains from Poisson
summation, the carrier carries as a geometric symmetry fixed before any
source is attached.  A character \(\chi\bmod q\) attaches its fiber ---
one phasor per integer per lane, synthesized from \(\chi\)'s finite data
alone, each phasor born at its own height at zero magnitude and growing
continuously, with no convergence gate --- and the conductor routes the
integers into positive, negative, and neutral buckets.  The transport
between the 3D state and the 1D chart runs in both directions with the
ledger booked at every stage, both composites the identity
(Theorem~\ref{thm:g2-ledgered-projection}); the chart ordinate of a
completed native event at height \(Z\) is \(y=\log Z\).  The midpoint is a
geometric \emph{output}: the arclength area law derives the unique
scale-balanced fiber exponent \(\sigma_*=\tfrac12\), and the no-drift law
fixes the native source modes there --- Riemann's real axis, the classical
midline, and the carrier's weld axis are one locus in three charts, and
nowhere is a decimal stipulated.

Two decisions --- and only two --- separate this audited geometry from the
critical-line statement itself: the \emph{identity reading} (every analytic
zero is a native carrier event) and the \emph{correlation reading} (a
pointwise chart-zero-to-kernel correspondence).  Neither is taken in this
paper.  The full development --- the construction at textbook pace, the
carrier-geometry evidence dossier, the Gram operators and their spatial
lifts, the ambient carrier operator, the two conditional GRH routes under
those two readings with their exact premises compiler-audited, and the
operator reduction sharpened to a single named limit provably equivalent to
Mathlib's \texttt{RiemannHypothesis} --- is the companion paper
\cite{GRHCompanion}.
...
\texttt{WarpRemovalTransfer.warpRemoval\_transfer\_tendsto}.  Unit modulus and cell closure alone do
not imply its first-difference hypothesis: the compiled separating instance
\texttt{WarpRemovalTransferAudit.unwarped\_half\_not\_summable} uses the constant coefficient bank and the
...
\begin{theorem}[Global identification of the candidate with $\Sym^r\pi$]\label{prop:localid}
...
Theorem~\ref{prop:localid} upgrades the \emph{identification} to all places: the candidate's
...
Theorem~\ref{prop:localid} --- decides \emph{which} $L$-function is read, not \emph{whether} it has a
...
Thm.~\ref{prop:localid}: the det-one tensor makes the two conjugate legs balance place by place, and the
...
term-by-term unfolding against the local factors of Thm.~\ref{prop:localid} --- used only \emph{after} the
...
\texttt{WarpRemovalTransfer.warpRemoval\_transfer\_tendsto}, or the fixed-theta reflection just
...
\subsection{The symmetric-power functoriality cancellation at \texorpdfstring{$r=2$}{r=2}: the carrier route and the projection shadow}\label{sec:aristotle-rs}

\begin{center}
\fbox{\parbox{0.94\textwidth}{\textbf{Status of $r=2$ functoriality: closed on the carrier; the
sharp-cutoff Perron bound is a projection shadow, off the critical path.} Symmetric-square
functoriality is discharged by the niceness route of item~1 --- entirety, the functional equation,
and vertical-strip bounds for the $\Sym^2$ twist bank, machine-checked in Lean~4 at the standard
kernel footprint $\{\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound}\}$ with no
\texttt{sorry} and no axiom (\texttt{cpsDualPair\_twistedNiceness},
\texttt{cpsAllTwists\_unconditional3DAnalyticPayload}), fed to the converse theorem
(Thm.~\ref{thm:cps}) --- \emph{with no Perron input}. The carrier-native cancellation content is
likewise compiled: the \emph{smoothed} coefficient cancellation (the vertical readout
$\sum_n a_n e^{-2\pi n x}=O(x^k e^{-2\pi x})$,
\texttt{HeckeCancellation.hecke\_smoothed\_cancellation}) and, for the second moment, the
Rankin--Selberg continuation of the \emph{actual infinite} series (\texttt{rs\_master}), with
entire-part holomorphy (\texttt{rs\_entirePart\_differentiable}), the contour shift with residue
(\texttt{rectangleBoundaryIntegral\_holo\_add\_residues}, from Cauchy--Goursat vanishing plus the
inverse-kernel residue), and the effective Perron kernel in both regimes
(\texttt{PerronKernel.perron\_kernel\_bound}, \texttt{perron\_kernel\_bound\_lt}). The
\emph{sharp-cutoff} summatory bound $\sum_{n\le x}\lambda_n=O(x^\theta)$ is the $1$D-projection
repackaging of that smoothed content --- a Chandrasekharan--Narasimhan Tauberian de-smoothing, a
chart device by Ground~Rule~4, isolated as the named hypothesis $\mathrm{PerronTauberian}$ and
\emph{not on the functoriality critical path}. In one line: \emph{functoriality is closed; the
sharp cutoff is a projection shadow, not a gap.}}}
\end{center}

\medskip
The proposition above builds the completed transform $\Lambda(s,\Sym^r\pi\times\tau)$ from the
carrier theta, already \emph{nice} (entirety, functional equation, vertical-strip bounds; item~1)
--- which is all the converse theorem consumes, so functoriality is closed without what follows.
The sharper, projection-side statement that a hostile review isolated as the crux to attack directly
--- \emph{not} required for that closure --- is the summatory cancellation
\[
...
over an abstract coefficient sequence and archimedean parameter $\kappa$, taking the
Perron/Tauberian transfer as its single explicit hypothesis --- a projection-side de-smoothing, off
the functoriality critical path, not a carrier gap; the rung continuation is discharged
separately --- the fixed-kernel functional equation, entirety, and vertical-strip bounds are
compiled for \emph{arbitrary} $r$ by the carrier's CPS twist machinery
(\texttt{cpsDualPair\_twistedNiceness}, \texttt{cpsPolynomialAllTwists\_payload}), with the
finite$\to$infinite Euler limit and its infinite-bank summability, exact reflection, and rapid decay
compiled (\texttt{GlobalHelix.symTwistEulerLimit\_eq\_div},
\texttt{GlobalHelix.summable\_fullTensorEuler\_zeroModeTerm},
\texttt{GlobalHelix.fullTensorEulerZeroModeDualTheta\_eq\_div}), and at $r=2$ the
continuation is additionally grounded in the arithmetic Rankin--Selberg series with the entire-part
holomorphy \texttt{rs\_entirePart\_differentiable} and the compiled contour shift.
...
\item $r=3,4$: functoriality is classically known (Kim--Shahidi); the classical kernel is
Langlands--Shahidi rather than an elementary lattice theta, but the \emph{carrier} kernel's
functional equation is compiled here for these rungs like any other (the CPS twist machinery below);
...
\emph{arithmetic input}: the Satake/Frobenius data $\theta^{e}$ feeding the local factors, which
is identified on the carrier ---
\texttt{arithmeticCPS\_localFactor\_identification} and
\texttt{arithmeticCPS\_globalCoefficient\_identification} equate the carrier bank's local numerator
and every global Euler coefficient with the literal twisted symmetric-power datum, by
\texttt{rfl} --- together with the residual vanishing E4 (completeness for non-exceptional $\pi$).
Perron remains the $r$-uniform gap \emph{of the sharp-cutoff cancellation} --- a projection-side
de-smoothing, off the functoriality critical path, which closes via niceness and the converse
theorem (Thm.~\ref{thm:cps}); the summatory bound is that route's shadow, not a gap in the transfer.
The reason $r=2$ is the \emph{compiled} rung is not that the
...
\paragraph{The fiber and the explicit weight.} The genuine $\Sym^2$ coefficients are the M\"obius
peel of the \emph{normalized} Rankin square $b_n=\lVert a_n\rVert^2/n^{k-1}$, namely
$\lambda_n=(\mu\star b)(n)$ (\texttt{sym2CoeffNorm}). Normalizing \emph{before} the peel is
essential: the Rankin--Selberg pole of $\sum\lVert a_n\rVert^2 n^{-s}$ sits at $s=k$, so peeling
$\zeta(s)$ from the \emph{un}-normalized square leaves that pole in place --- $\sum_{n\le x}(\mu\star
\lVert a\rVert^2)\sim c\,x^{k}$, no cancellation at all; after normalizing, the pole sits at $s=1$,
exactly where $\zeta(s)$ cancels it, leaving the \emph{entire} $L(\Sym^2 f,s)$
(\texttt{rankin\_sym2\_transfer} is the definitional $\zeta$-peel $L(b,s)=\zeta(s)\,L(\lambda,s)$,
no Hecke-operator theory). So $\sum_{n\le x}\lambda_n=O(x^{\theta})$ with $\theta<k/2$ is a
\emph{true} statement (convexity for the degree-3 entire $L(\Sym^2 f)$ gives $O(x^{1/2+\epsilon})$,
and $\tfrac12<\tfrac k2$). The weight
...
\noindent The averaged bank itself is now a compiled theta profile
(\texttt{RSAveragedThetaProfile3D}): with
$\bar\theta_f(t)=\int_{\mathcal D}P(z)\,\Theta_z(t)\,d\mu$ the Petersson average of the general
lattice bank of the \emph{actual} form, the exact weight-one reflection
$\bar\theta_f(1/t)=t\,\bar\theta_f(t)$ is derived by integrating the pointwise weld
\texttt{latticeTheta\_inv} against the Petersson density (\texttt{averagedTheta\_inv} --- no
candidate, no supplied functional equation); the tail approaches the Petersson mass at the
saddle rate (\texttt{averagedTheta\_tail\_bound}); and the packaged pair
(\texttt{rsAveragedWeakFEPair}, weight one, both constant terms the Petersson mass --- the
profile genuinely carries the $\zeta$-factor constant term, so its home is the \emph{weak} pair,
the same typing law that retired the raw-vertical coupling above) returns from Mathlib's
abstract functional-equation machinery the entire part, the global equation
$\Lambda(1-s)=\Lambda(s)$, and the edge residue equal to the Petersson mass
(\texttt{rsAveraged\_residue\_one}) --- rank-is-DC-residue at the Rankin--Selberg level, at the
profile interface.  The named identification steps from this profile to the $\Sym^2$ bank are
the $\zeta$-peel (\texttt{rankin\_sym2\_transfer}, compiled definitionally) and the coefficient
certificate against the literal twisted Satake bank --- instantiation points of the closed
identification layer, not open interfaces.

\paragraph{The one classical input.} What remains is the passage from this continued Dirichlet
series to the summatory bound. The contour shift past the pole line is itself compiled: the
residue theorem for finitely many simple poles in a rectangle,
$\oint\bigl(g+\sum_\rho w_\rho/(\cdot-\rho)\bigr)=2\pi i\sum_\rho w_\rho$, is assembled from the
Cauchy--Goursat vanishing and the inverse-kernel residue
(\texttt{rectangleBoundaryIntegral\_holo\_add\_residues}, built on
\texttt{rectangleBoundaryIntegral\_eq\_zero\_of\_differentiableOn} and
\texttt{rectangleBoundaryIntegral\_weighted\_divisor}); the Mellin inversion is likewise compiled
(\texttt{mellin\_inversion\_eq}). The genuinely-cited remainder is therefore the \emph{Perron
summation identity} --- the sharp-cutoff $\sum_{n\le x}$ against the contour-integral representation
of the partial sum, together with the classical convexity estimate of the shifted vertical leg
(whose edge bounds are compiled, \texttt{zetaRegularizer\_bound\_left}/\texttt{\_right}) --- classical
and standard in analytic number theory. We isolate it as a
...
functional equation, explicit poles, tail decay, finite measure --- and the sharp-cutoff cancellation
rests on one
classical Tauberian step, the Perron summation identity (a projection shadow off the functoriality
critical path, which closes via niceness, item~1), with the contour shift, residue, and
Mellin inversion all compiled, and $\omega_W$ made explicit as the
Petersson--Eisenstein density. The fixed-kernel functional equation \emph{and} the arithmetic
identification --- local factors, all-place coefficient bank, and the full completed readout with
its conductor and Gamma shifts --- are compiled carrier facts at \emph{every} rung, the
identification by \texttt{rfl} (\texttt{arithmeticCPS\_localFactor\_identification},
\texttt{arithmeticCPS\_globalCoefficient\_identification},
\texttt{arithmeticCPSFullCompletion3D\_identification}); so every rung's cancellation closes to the
single Perron step, with $r=2$ additionally grounded, concretely, in the arithmetic Rankin--Selberg series
(\texttt{rs\_master}, the Gaussian--lattice weld above).
...
and $\tau$, using no automorphy of $\Sym^r\pi$ and no zero-source argument.
\begin{description}
\item[Local data (Thm.~\ref{prop:localid}).] The transverse block \emph{is} the Satake
...
functional equation; and Thm.~\ref{prop:localid} identifies the completed package --- local factors,
...
cells do not close.  Its closure is carried by its \emph{own} synthesized completion --- the kernel built
from the fiber's coefficients against the self-dual clock --- under which the completed pair readout is
entire with vertical-strip bounds and the functional equation at every rank
(\texttt{cpsDualPair\_twistedNiceness}, radius-live \texttt{cpsPolynomialAllTwists\_payload}).  The
registration of that completed readout in the prescribed conductor/$\Gamma$-product chart is the per-rung
coupling (\texttt{BankThetaProfileCoupling3D}); machine-checked end-to-end for the character classes
(\texttt{dirichlet\_strip\_tendsto\_LFunction}) and at the standard rung of a genuine cusp form
(\texttt{standardRung\_classicalHeckeTheorem}), derived at $r=2$ from the lattice weld, classical at
$r=3,4$, and independently evidenced by the discriminating instruments (the oracle-free critical-line
readout to $2\times10^{-16}$, the growth locator to $10^{-12}$, the FE closures, App.~\ref{app:repro}). Warp controllability itself (Lemma~\ref{lem:forcible}) contributes only the
...
machine-checked inputs are \texttt{reachable\_independent\_stage} and \texttt{residual\_forcible}, and their
composition is itself a Lean theorem (\texttt{reachable\_residual\_forcible}): a continuous nonconstant
dual-lane phase supplies a stage and real weights closing any prescribed residual.
...
cells for every admissible fiber, and the neutrality is now itself a theorem: one stage closes any two
residuals identically, so forcibility carries no information about the fiber and can never serve as
niceness evidence (\texttt{ForcibleClosure.forcible\_nondiscriminating}) --- so it is the
\emph{transport} half of the architecture, consumed by
...
warp-removal transfer of \S\ref{sec:construct}. Its controllability and reachability legs are proved
(\texttt{ForcibleClosure.residual\_forcible}, \texttt{CarrierReachability.reachable\_independent\_stage});
the inverse-kernel summability and boundary legs are separately exposed by
\texttt{WarpRemovalTransfer.warpRemoval\_transfer\_tendsto}. A
...
The cited Lean intertwiners prove the involution, lossless coordinate ledger, clock exchange, and
logarithmic inversion.  The global reflection of the all-place pair banks is itself compiled: the primal
bank and its reciprocal-height contragredient bank satisfy the exact fold identity at every positive
height, for every rank and every admissible Satake family, radius-live
(\texttt{GlobalHelix.cpsDualPair3D\_globalHelixReflection};
\texttt{GlobalHelixCPSPolynomialDualPair}), and the completed pair readout carries the functional
equation with entirety and vertical-strip bounds in the fiber's own completion
(\texttt{cpsDualPair\_twistedNiceness}, every rank).  The registration of this reflection in the
prescribed conductor/$\Gamma$-product chart --- the classical naming --- is the per-rung coupling of
\S\ref{sec:cps-3d-engine} (\texttt{BankThetaProfileCoupling3D}, reduced by the discharge layer to one
completed Mellin functional equation and one ray convergence, \texttt{ofMellinFE''}): proven at the
standard rung from the modular slash law (\texttt{standardRung\_reflection}), derived at $r=2$ from the
lattice weld (\texttt{averagedTheta\_inv}), consumed classically at $r=3,4$.
...
\emph{Step 2: the pair carries the identity everywhere.} Both completed pair readouts are analytic on the
whole plane: the all-place dual-pair strong pair is entire with vertical-strip bounds and the functional
equation in the fiber's own completion, at every rank and radius-live
(\texttt{cpsDualPair\_twistedNiceness}, \texttt{cpsPolynomialAllTwists\_payload}), and the fold identity
of the two banks holds at every positive height
(\texttt{cpsDualPair3D\_globalHelixReflection}).  A character's cells close at the base harmonic scale and
its readout is additionally identified with the Mathlib oracle end-to-end
(\texttt{dirichlet\_strip\_tendsto\_LFunction}, \texttt{eta\_strip\_tendsto}).  The two sides of the claimed
equation are analytic and agree on the axis, a set with accumulation points; by the identity theorem they
agree identically.  The registration of the pair identity in the prescribed conductor/$\Gamma$-product
chart is the per-rung coupling (\texttt{BankThetaProfileCoupling3D.ofMellinFE''}); at the standard rung it
is proven from the slash law (\texttt{standardRung\_reflection}), at $r=2$ from the lattice weld
(\texttt{averagedTheta\_inv}).

\emph{Why no Poisson.} On the carrier the inversion $x\mapsto1/x$ maps the primal lane onto the
reciprocal-height anti-lane phasor by phasor: the fold is exact and termwise for the pair
(\texttt{cpsDualPair3D\_globalHelixReflection}), so no resummation is performed anywhere.  What the
classical frame computes by Poisson/modularity is the registration of this fold in the unit/$\Gamma$
chart --- a chart statement, supplied per rung by the coupling above, never a carrier input.
\end{proof}
...
\emph{(4) Identification.} Only now: by Thm.~\ref{prop:localid} and \textup{(E5)},
...
identification of Thm.~\ref{prop:localid}\,\textup{(E5)} --- the completed carrier readout \emph{is} the
...
\]
the all-place product of the local factors identified placewise in Thm.~\ref{prop:localid}, archimedean
...
it is Thm.~\ref{prop:localid}\,(E5), and the archimedean/conductor match is the completed identification, proved
at every place --- archimedean factors and conductor included --- by Thm.~\ref{prop:localid} with (E5) for the
...
transported Satake datum at every place (Thm.~\ref{prop:localid});
...
and root number at every place (Thm.~\ref{prop:localid}). \emph{Hypothesis-free on the completed
carrier object}: entireness and vertical-strip boundedness --- the continuation meromorphic, its
only poles the DC coalignment events (none, hence entire, for an irreducible parameter; a
$\zeta$-factor pole per trivial constituent otherwise, \S\ref{sec:cps-decomp}) --- together with
every twist a converse theorem consumes, are supplied by the fixed-completion constructor for
every admissible (polynomially bounded) fiber
(\texttt{CarrierTheta.automaticCoefficientThetaStrongFEPair}; on the prescribed
conductor/$\Gamma$-product banks, \texttt{cpsPolynomial\_twistedNiceness}, with the
standard-completion identification \texttt{cpsPolynomialFullCompletion3D\_identification}).
Cell closure is \emph{not} the supply of this niceness and could not be: forcible closure is
arithmetic-neutral by theorem
(\texttt{ForcibleClosure.forcible\_nondiscriminating}) --- it closes a random bank exactly as it
closes a true fiber --- so it contributes the transport half alone; the harmonic and Dirichlet
classes additionally close their cells outright (\texttt{CellClosure}, Lean), which is what
grounds their \emph{ordinary} (unwarped) readout continuation end-to-end.
...
$\sum_{k\in C}q_k(\tau)=0$ \emph{exactly} (a closed cell, not a cutoff limit).  The realization of vanishing
is the compiled \emph{zero-detector}, not the forcing: the harmonic-pencil rank drop occurs exactly at the
readout's zeros (Lemma~\ref{lem:rankgap}; Lean \texttt{focal\_residual\_zero\_iff\_L\_zero},
\texttt{carrierZero\_iff\_L\_zero}, with the channel-agnostic factorization
\texttt{HarmonicCell.harmonic\_residual\_no\_independent\_mode}), while forcible closure
(Lemma~\ref{lem:forcible}) supplies only the per-cell transport remainder and, being arithmetic-neutral
(\texttt{forcible\_nondiscriminating}), never the vanishing identification.  Where \emph{(b)} furnishes the
continuation, these detector closures are, by the identity
...
claim.
\end{itemize}
So admissibility asks only for a convergent Euler product of bounded degree. Niceness is never a
hypothesis and never a closure inference: the reflection and local data come out
unconditionally; entireness, strip bounds, and the functional equation of the completed carrier
object are the fixed-completion constructor's output, hypothesis-free for every admissible fiber
(\texttt{cpsPolynomial\_twistedNiceness}); and the complete faithful vanishing is the
cell-closure content of \textup{(c)}, whose per-cell mechanism
(\texttt{ForcibleClosure.residual\_forcible} with
\texttt{CarrierReachability.reachable\_independent\_stage}) is arithmetic-neutral by theorem
(\texttt{forcible\_nondiscriminating}) and is therefore consumed by transport and adaptation,
never by niceness.
\end{theorem}
...
while satisfying the carrier and ledger laws and preserving faithful readout. Two $\forall\exists$
quantifiers must never be traded for one another, and the proven one is named exactly: for every
$W$ a \emph{cell-closing} adapter suite exists, in closed form, not sampled --- any nonzero cell
residual is forcible to zero once two
...
application --- so the enumeration of every $(W,\text{suite})$ pair is not required.  The
\emph{recognition} quantifier --- that the synthesized suite reads the fiber's arithmetic rather
than merely closing its cells --- is precisely what cell closure can never supply
(\texttt{forcible\_nondiscriminating}) and is the separately registered program of the
contributions ledger (item~2), with its single clean falsifier stated there. The general family
...
unconditional and its niceness the fixed-completion constructor's output; that output is uniform in rank, for the
...
warp-removal analytic hypotheses remain explicit inputs as recorded above. Its three parts stand at their own strength:
...
inverse-kernel conditions handled by \texttt{WarpRemovalTransfer.warpRemoval\_transfer\_tendsto} ---
...
functional equation is the carrier reflection of the det-one tensor fiber (Thm.~\ref{prop:localid}),
...
$\Lambda(s,\Sym^r\pi\times\sigma)$ is unconditional and proved in Theorem~\ref{prop:localid} together with the
unfolding identity \textup{(E5)}: the carrier and classical constructions have the same local factors at every
place --- archimedean factors, conductor, and root number included (Thm.~\ref{prop:localid}) --- so their
...
(\texttt{transfer\_analytic}) are machine-checked; the completed carrier niceness --- entire
continuation, strip bounds, and functional equation --- is moreover proved unconditionally for every
twist as a single carrier object (\texttt{cpsPolynomial\_twistedNiceness}), the completed theta
converging on the whole positive ray (\texttt{cpsPolynomialFullPrimalTheta\_locallyIntegrableOn}), so
the $\Re s>1$ Euler-product abscissa is a projection-chart gate, not a carrier barrier.  The
classical-$L$ identification (Thm.~\ref{prop:localid} with \textup{(E5)}) is closed on the carrier;
neither it nor convergence is owed. The
...
\section{The mechanism and its converse: the analytic ring closed at every chart}\label{sec:mechanism-converse}

The completed analytic package of a carrier bank --- entire continuation, functional
equation, chart identification --- and the pointwise theta reflection of its prescribed
readouts are \emph{the same data}, in both directions, machine-checked.  This section
registers the compiled ring: the mechanism (identity $\Rightarrow$ package) at its true
generality, its converse (package $\Rightarrow$ identity) with the classical
boundedness hypothesis alone, and the identification carrying the reflection to the
modular transformation laws.  Every declaration cited here carries the standard axiom
footprint and no \texttt{sorry} (App.~\ref{app:repro}).

\subsection*{The coefficient surface and the mechanism}

Everything the chart engines consume from a bank is two coefficient sequences with one
polynomial bound: the surface \texttt{CoefficientSurface} $(a,a^\vee,A)$ with
$\|a_n\|,\|a^\vee_n\|\le (n+1)^A$.  A chart is packaged as a kernel
\texttt{MellinKernel} $(K,G,B_0)$: a positive-ray kernel with rapid decay whose Mellin
transform equals the multiplier $G$ beyond the bar $B_0$.  The compiled kernel families
instantiate it: the $\Gamma_{\mathbb C}$ clock (\texttt{gammaCKernel}, the holomorphic
chart type), the general Deligne chart $\prod\Gamma_{\mathbb R}\cdot\prod\Gamma_{\mathbb C}$
(\texttt{generalMellinKernel}), and the pure-$\Gamma_{\mathbb R}$ chain
(\texttt{realChainMellinKernel}, the Maass chart type, purely imaginary shifts
admissible).

\begin{theorem}[The mechanism, coefficient-level and kernel-generic;
\texttt{RamifiedMechanism.mechanismK}]\label{thm:mechanismK}
Let $P$ be a self-dual coefficient surface ($a^\vee=a$) and $\kappa$ a packaged chart
kernel.  One pointwise theta identity of the prescribed readouts,
$\Theta_P(1/x)=\varepsilon\,x\,\Theta_P^\vee(1/x)$ for all $x>0$, yields the complete
package: an entire $\Lambda$ with $\Lambda(1-s)=\varepsilon\Lambda(s)$ and
$\Lambda(s)=G(s)\,L(a,s)$ on $\Re s>\max(B_0,A+1)$.  The continuation is unique
(\texttt{KNicePackage.unique}).
\end{theorem}

The fixed-rank mechanisms are instances through
\texttt{PolynomialSatakeDualPair.surface}: every finite-rank bank of
\S\ref{sec:transport-machine} feeds the theorem, and the seed layer supplies the
arithmetic instances --- $\Sym^r$ of the holomorphic level-one eigenform
(\texttt{SatakeSeed.ofHecke}), of the typed Maass--Hecke package
(\texttt{MaassEigenData}, \texttt{SatakeSeed.ofMaass}), at every rank over every seed
(\texttt{symr\_package\_of\_theta\_of}), with the Maass spectral chart named
(\texttt{maass\_symr\_package\_of\_theta}, shifts $(r-2j)it$).  Rank three is typed
directly (\texttt{GL3Seed}): self-duality enters as a channel permutation hypothesis
--- rank three does not supply it for free --- and the $\Sym^2$ lift \emph{discharges}
it, with the recognition \texttt{ofSym2\_coeff\_eq\_sym2} identifying the lift's
standard coefficients with the $\Sym^2$ transport coefficients on the nose: the
Gelbart--Jacquet identification at seed level.  Level $N$ is not a separate
obstruction: the rank-varying surface \texttt{RamifiedWeightFamily} (per-prime rank
profile capped by $D$, degree drop at ramified places, degree zero fully ramified)
carries the polynomial bound $(n+1)^{D+B}$ --- the cap enters only through
$\binom{\cdot}{\cdot}$-majorant monotonicity --- and self-duality by \emph{per-place}
channel permutations; the mechanism fires on any such bank
(\texttt{ramified\_package\_of\_theta}).  The constant-rank family recovers the
unramified $\Sym^r$ bank definitionally (\texttt{ofSeed\_coeff\_eq}, by \texttt{rfl}),
and the principal-character family (rank $0$ at $p\mid N$, rank $1$ elsewhere) is the
genuine rank-drop witness (\texttt{unitRamified}).

\subsection*{The converse: the package forces the identity}

The reverse arrow is the Hecke--Weil converse in the carrier register.  Its one
genuinely analytic step is the contour shift, compiled as
\texttt{vertical\_line\_shift}: an entire integrand, integrable on two vertical lines
with vanishing horizontal integrals, has equal line integrals (rectangle
Cauchy--Goursat and passage to the limit).  The functional equation exchanges the two
inversion lines (\texttt{mellinInv\_reflect}), Mellin inversion runs on the chart line
where the bank's transform \emph{is} $\Lambda$ (\texttt{mellin\_theta\_eq\_lam\_on}),
and the assembly is:

\begin{theorem}[The converse; \texttt{WeilConverse.weil\_converse}]\label{thm:weil-converse}
Let $P,\kappa$ be as above with $\kappa$-kernel continuous on the ray, and let a
package for $P$ at $\kappa$ be given together with line integrability of $\Lambda$ at
one chart line $\sigma$ and a horizontal decay bound on the strip $[1-\sigma,\sigma]$.
Then the pointwise theta reflection of the prescribed readouts holds --- in exactly the
form consumed by Theorem~\ref{thm:mechanismK}.  Feeding it back returns the same
continuation (\texttt{weil\_converse\_ring}): the ring is closed.
\end{theorem}

The two analytic side conditions are then discharged.  The line data come from the
in-tree Stirling bound: the $\Gamma_{\mathbb R}$ line reduces to the compiled
$\Gamma_{\mathbb C}$ line by a constant-ratio norm identity
(\texttt{gammaR\_vline\_eq\_const\_mul}), chain multipliers are line-integrable and
exponentially decaying with the Dirichlet factor uniformly bounded on the line
(\texttt{StirlingLineDischarge}).  The strip decay is Phragm\'en--Lindel\"of:
\texttt{strip\_exp\_decay} interpolates boundary decay $A e^{-k|t|}$ through a bounded
strip \emph{with the same constants} (two multiplier applications of the vertical-strip
principle).  The composite is the converse under the classical hypothesis set:

\begin{theorem}[The converse from boundedness alone;
\texttt{PhragmenBV.weil\_converse\_of\_BV}]\label{thm:weil-converse-bv}
Package plus \emph{one} boundedness constant on the strip forces the theta reflection.
At both compiled chart families the kernel side is hypothesis-free: the
$\Gamma_{\mathbb C}$ chart (\texttt{weil\_converse\_of\_BV\_gammaC}; clock continuity
compiled) and the pure-$\Gamma_{\mathbb R}$ chain
(\texttt{weil\_converse\_of\_BV\_realChain$'$}; chain-kernel continuity compiled through
the weight exchange \texttt{weighted\_logMellinConvolution\_eq\_convolution}).  The
named Maass instance \texttt{maass\_converse\_of\_BV} runs at the spectral chart
$\prod_j\Gamma_{\mathbb R}(s+(r-2j)it)$ of $\Sym^r$, every rank, every spectral
parameter.
\end{theorem}

\subsection*{The identification: the generator laws}

The reflection identifies.  The q-expansion
$f(\tau)=\sum_n a_n e^{2\pi i(n+1)\tau}$ of a surface is holomorphic on the upper
half-plane (\texttt{qSeries\_analyticOnNhd}); its $T$-law is exact from the expansion
alone (\texttt{qSeries\_T}); the compiled $\Gamma_{\mathbb C}$-clock readout at height
$y$ is $2f(iy)$ (\texttt{theta\_eq\_qSeries\_axis}); and the axis reflection extends to
the full half-plane by the identity theorem
(\texttt{qSeries\_S\_of\_reflection}): $f(\tau)=\varepsilon\,(-i\tau)^{-1}f(-1/\tau)$
on $\mathbb H$.

\begin{theorem}[The Hecke identification;
\texttt{WeilIdentification.hecke\_identification}]\label{thm:hecke-ident}
An entire package at the $\Gamma_{\mathbb C}$ chart with the functional equation and
one boundedness constant yields both generator transformation laws of its
q-expansion: exact $1$-periodicity and the $S$-law on all of $\mathbb H$.
\end{theorem}

Since $S$ and $T$ generate $\mathrm{SL}(2,\mathbb Z)$ --- machine-checked in Mathlib
(\texttt{SpecialLinearGroup.SL2Z\_generators}) --- these are the complete generator set
of level-one modularity, and the multiplier packaging is itself compiled: the set of
$\gamma$ under which a function is a weight-$k$ slash eigenvector is a subgroup (the
slash-action axioms are the cocycle; \texttt{slashEigenSubgroup}), the two laws place
$S$ and $T$ in it, and the closure argument delivers

\begin{theorem}[Hecke modularity;
\texttt{MultiplierPackaging.hecke\_modularity}]\label{thm:hecke-modularity}
The q-expansion of an entire BV-package at the $\Gamma_{\mathbb C}$ chart is a
weight-$1$ slash eigenform at \emph{every} $\gamma\in\mathrm{SL}(2,\mathbb Z)$: for
each $\gamma$ there is a nonzero multiplier $v(\gamma)$ with
$f\,|_{1}\,\gamma=v(\gamma)\cdot f$, in Mathlib's own slash action ($v(T)=1$,
$v(S)=-i\,\varepsilon^{-1}$).
\end{theorem}

\noindent The theorem is quantified over an \emph{arbitrary} self-dual coefficient
surface, so its instances are the entire compiled bank family --- automorphy at the
holomorphic chart is universal over it: $\Sym^r$ of every seed and every self-dual
rank-varying level-$N$ bank are named instances
(\texttt{symr\_automorphy}, \texttt{ramified\_automorphy}).  The register per chart:
at the Maass chart the converse input is equally compiled
(\texttt{maass\_converse\_of\_BV}) and the wave-form identification is now compiled
up to exactly one typed field: the wave form
$\sqrt y\,\sum_n a_n\cos(2\pi(n{+}1)x)\,K((n{+}1)y)$ on the abstract radial kernel
has exact $T$-periodicity and evenness, its axis value is the compiled theta
readout, the axis $S$-law is the compiled reflection, and \emph{both halves of the
Cauchy data on the axis match unconditionally} (an even function has vanishing
transversal derivative, no differentiability hypothesis needed);
\texttt{maass\_modularity\_of\_BV} then gives full weight-$0$
$\mathrm{SL}(2,\mathbb Z)$-modularity of the wave form from package + one
boundedness constant + the typed Cauchy-uniqueness field --- the classical elliptic
step (both functions are eigenfunctions of the hyperbolic Laplacian; eigenfunctions
with equal Cauchy data on a curve coincide), consumed at its exact strength since
Bessel functions and elliptic unique continuation are not yet in Mathlib.  The
kernel itself needs no Bessel API: the chain kernel at the chart $(it,-it)$ has
Mellin transform $\Gamma_{\mathbb R}(s+it)\Gamma_{\mathbb R}(s-it)$, and kernels
with equal Mellin transforms on a line agree on the ray
(\texttt{eqOn\_pos\_of\_mellin\_eq}), so it \emph{is} the classical
$\sqrt y\,K_{it}$-kernel.  At higher rank the identification is the cited
converse-theorem layer.  The level-$N$ statement --- finitely many twisted functional
equations sufficing for $\Gamma_0(N)$-automorphy --- is Weil's lemma, consumed at the
cited converse-theorem layer of \S\ref{sec:cps-thm} exactly as before.

\subsection*{Obstruction detection at the ring}

The two arrows make the pointwise theta identity an obstruction \emph{detector},
both directions compiled (\texttt{ObstructionDetection}).  Soundness is the
converse's contrapositive: a single pointwise failure of the identity obstructs
every BV-package at every root number
(\texttt{reflection\_failure\_obstructs\_gammaC}).  Detection is the mechanism's
contrapositive, with the pole instance compiled: the unit ($\zeta$-) surface admits
no entire package at any $\Gamma_{\mathbb C}$ chart --- the $\zeta$-pole collides
with entirety along the ray (\texttt{zeta\_no\_package}) --- hence its theta
identity \emph{fails pointwise} (\texttt{zeta\_reflection\_fails}): the trivial
channel's pole is detected by the carrier readouts alone, no $L$-value evaluated.
The dichotomy \texttt{detection\_dichotomy} states completeness: every self-dual
surface either satisfies the identity everywhere or admits no BV-package.  This
upgrades the beyond-endoscopy demonstration from the representation bank to the
coefficient surface, where every compiled bank lives.

\subsection*{The exchange instrument at $\Sym^r$}

Both arrows compose at the $\Sym^r$ bank of any seed
(\texttt{ExchangeInstrument.symr\_package\_of\_reflection},
\texttt{symr\_reflection\_of\_package}): converting between the one-sided theta
identity and the analytic package is an \emph{interface}, not missing machinery.  The
per-rung obligation on either side is one classical datum --- the completed package
with bounded vertical strips, the precise output shape of the classical
symmetric-power automorphy results (Kim--Shahidi for $r\le4$, Newton--Thorne for all
$r$ at level one; cited).  A cited package of that shape now yields the pointwise
identity \emph{compiled}, and an identity yields the package \emph{compiled}; the
rank-varying level-$N$ banks carry the same two arrows through their surface.
...
dual-pair reciprocity at its exact interface (\texttt{DualPairFiber}).  The multiplicative core of
step~(2) --- the global functional equation as the \emph{product} of the per-place reciprocal FEs ---
is machine-checked directly: \texttt{CompletedReflection.prod\_completed\_FE} assembles a finite Euler
product's completed FE from its local factors, and \texttt{CompletedReflection.infinite\_completed\_FE}
carries it to the infinite product by passage to the limit (\texttt{tendsto\_nhds\_unique}), the sole
remaining input being Euler-product convergence (\texttt{TransferContinuation.transfer\_analytic}) ---
no Poisson summation.  The E5 bridge is likewise
...
identification used by Thm.~\ref{prop:localid} is wired into the CPS completion surface rather than
left as an unnamed manuscript transition.  The all-place arithmetic identification, ramified local
factors included, is \emph{not} a remaining scope item: Thm.~\ref{prop:localid} closes it on the
carrier --- the native local factor is read off the definitional Satake/Frobenius block
(\texttt{frobeniusBlock\_eq\_conjPairBlock}, by \texttt{rfl}) through the lossless, faithful deprojection
(\texttt{ConeProjection.record\_bijective}), with local Langlands for $\mathrm{GL}(2)$ \cite{Tate}
furnishing $\varphi_{\pi,v}$ (a cited theorem, not an owed formalization) and $\Sym^r$ a functor on
parameters.  Nor is the completion a remaining item: the completed carrier niceness --- entire
continuation, vertical-strip bounds, and functional equation --- is proved unconditionally for every
twist as a \emph{single} carrier object (\texttt{cpsPolynomial\_twistedNiceness},
\texttt{cpsPolynomial3D\_globalHelixReflection}), and the completed coefficient theta converges on the
whole positive ray by polynomial-growth/kernel-decay local integrability
(\texttt{cpsPolynomialFullPrimalTheta\_locallyIntegrableOn}).  The Euler-product convergence abscissa
$\Re s>1$ is the projection chart's gate, not a barrier on the carrier, which has no abscissa; the
standard-$\Gamma$ normalization of the reflection rides the per-fiber reciprocal identity
(\texttt{FiniteWeightFiber.symTensorCompleted\_FE}, every $s$, every $r$).  Neither the identification
nor the convergence is owed.
...
The mechanism--converse ring of \S\ref{sec:mechanism-converse} is machine-checked end
to end at the same footprint: the coefficient-level mechanism and its instances
(\texttt{RamifiedMechanism}, \texttt{GeneralSeed}, \texttt{GL3Seed},
\texttt{RealChainCompletion}), the converse with its contour shift and
Phragm\'en--Lindel\"of discharge (\texttt{WeilConverse},
\texttt{StirlingLineDischarge}, \texttt{PhragmenBV},
\texttt{ChainKernelContinuity}), the Hecke identification and its multiplier
packaging (\texttt{WeilIdentification}, \texttt{MultiplierPackaging},
Theorem~\ref{thm:hecke-modularity}), the Maass wave-form identification
(\texttt{MaassIdentification}), and the $\Sym^r$ exchange interface
(\texttt{ExchangeInstrument}).  The classical inputs consumed there remain exactly
two, both cited at typed fields: the completed package with bounded vertical strips
per rung, and Weil's finite-twist lemma at level $N$.

\section{Hodge Connection}\label{sec:hodge-connection}

\emph{The junction at the seed: freeze or drift.}  The Langlands--Hodge junction
--- temperedness is a freeze law (\texttt{StrandLatticeFreeze}: a lattice strand
under the tower ceiling is a sixth root of unity, purity supplied by the compiled
radial limit, $\mu_6$ by the compiled lattice classification) --- now lands at the
campaign's typed seed (\S\ref{sec:mechanism-converse}), the same object carrying
the packages, the converse, the wave-form identification, and the Sato--Tate
family: a Hodge-type certificate at a prime (the Satake datum a nonzero
$\mathbb Z[\zeta_6]$-integer — the rational constant-mode datum of the harmonic
frame) forces, under the same ceiling field the Sato--Tate wiring consumes, the
sixth-power freeze (\texttt{HodgeSeedFork.seed\_freeze}) and pins the family's
angle to the $\mu_6$ boundary set $\{0,\pi/3,2\pi/3,\pi\}$ of the house cells
(\texttt{seed\_freeze\_angle}).  The drift branch is the compiled Sato--Tate
chain, and the incompatibility is now closed with one exact common mode
(\texttt{FrozenDrift}): for any sixth root of unity the trace ladder satisfies
$2S_2-S_4=1$ on the nose ($\alpha^4=\alpha^{-2}$ collapses it,
\texttt{trace\_mu6\_identity}), so a fully certified seed's prime averages obey
the constant relation $2\,\mathrm{avg}_2-\mathrm{avg}_4=1$ at every stage
(\texttt{frozen\_average\_relation}) --- ranks $2$ and $4$ cannot both cancel
(\texttt{frozen\_violates\_cancellation}), and by the compiled biconditional a
fully frozen seed provably does \emph{not} equidistribute
(\texttt{frozen\_not\_equidistributed}).  No moment matrix, no compactness, no
measure theory: the common mode is computed exactly and books the contradiction.
The fork is Sato--Tate's own non-Galois-type hypothesis, typed at the seed and
now two-sided: rational data freeze and provably do not drift; generic data
drift.

\emph{The shadow detector at every order.}  The $\mu_6$ certificate is one
instance of the structure that actually matters --- a \emph{monomial} (induced,
CM-type, Galois-type) shadow of \emph{any} finite order --- and the detector
covers them all at once (\texttt{ShadowDetector}).  The general common mode is
exact: for $\alpha^N=1$ the rank-$N$ symmetric-power trace collapses to a
geometric sum in $\alpha^{-2}$ of period $N$ and equals
\emph{exactly} $1$, or $N+1$ in the degenerate case $\alpha^2=1$
(\texttt{trace\_root\_of\_unity\_re}) --- in particular its real part is $\ge 1$
with a gap independent of the datum.  Hence a family carrying one common finite
order at every prime (\texttt{MonomialShadow}) has rank-$N$ prime average $\ge 1$
at every stage, its rank-$N$ cancellation fails, and it provably does not
equidistribute (\texttt{shadow\_not\_equidistributed}); contrapositively, a
drifting family admits \emph{no} finite-order shadow at any order
(\texttt{no\_shadow\_of\_equidistributed}).  A single bounded-rank moment
separates the branches, with constant gap --- no moment matrix, no compactness.
These are rigidity and detectability statements about the trace family; they
carry no computational-cost content and say nothing about recovering an inducing
datum.  Beyond the fork itself, the compiled ring of this section supplies the
\emph{template} the Hodge companion's terminus names for its constructive arm
\cite{HodgeCompanion}: the cycle-level recognition problem there is exactly the
two-arrow shape compiled here --- the graded tower record with its ledger
playing the package, the sourcing cycle playing the theta identity --- so the
companion's ``build the recognition theorem'' is sharpened to ``instantiate
this shape at cycle level.''

\medskip
\emph{The loss-ledger$\,\leftrightarrow\,$cycle-filtration correspondence.}
The carrier's descent $3\mathrm D\!\to\!2\mathrm D\!\to\!1\mathrm D$ books a
loss ledger --- height (ordinate), radius, angle --- and its leading central
jet factors, by BSD/Gross--Zagier~\cite{BSD,GrossZagier}, into three
invariants the census resolves \emph{independently}: the vanishing order,
the regulator, and $|\Sha|$ --- three cycle-filtration layers ordinarily
invisible to Hodge projection, each carrying a \emph{distinct} retained
coordinate.  This is theorem-level for semisimple carrier states
(\S\ref{sec:hodge-companion}) and the \emph{target} for extension classes.
The instruments corroborate: the regulator/value channel fires for every
homologically-trivial cycle in the census (no silent cycle), and a class
silent at depth $1$ first fires at depth $2$ with its first nonzero slot at
the expected tower level (\texttt{hodge\_griffiths\_probe.py},
\texttt{tower\_exhaustion\_test.py}, App.~\ref{app:repro}); the
signature-based cycle detection of \S\ref{sec:general} remains preliminary
(\texttt{hodge\_layer\_calib.py} surfaced a structural question out of
scope here).  The full development is the Hodge companion paper
\cite{HodgeCompanion}: the carrier filtration $F_{\mathrm{car}}$ and the
depth dictionary at its exact (definitional) pricing, the Bloch--Beilinson
coincidence priced as an interface, finite-extension retention, the
five-condition realization interface with its executed rungs (the rational
group algebra, Artin motives, the divisor and function-field-extension
pairing rungs), the Klein-quartic $L$-side record, and the typed
Ceresa/Gross--Schoen landing
(\texttt{CeresaGrossSchoenLanding.complete\_landing}). \subsection{Cycle-filtration faithfulness: finite extension retention and the Ceresa frontier}\label{sec:hodge-companion}
\emph{The structural half is unconditional; the frontier is priced.}  On
semisimple states and on every finite normalized jet stack the
moment/extension tower is jointly faithful --- trivial radical,
\textsc{Retention}, no Bloch--Beilinson input
(\texttt{momentTower\_exhaustive},
\texttt{generalExtensionTower\_exhaustive}; Lean) --- so for a
homologically-trivial cycle the remaining obligation is exactly the typed
realization of its extension datum, and retention then follows by transport
rather than by a new no-silent-layer assumption.  The Griffiths registers
--- why the transcendental class is \emph{not} placed in the carrier
radical, the unconditional Fermat-quartic first rung (Harris \cite{Harris};
Ceresa \cite{Ceresa}), and the Laga--Shnidman control \cite{LagaShnidman}
showing the bielliptic Beilinson--Bloch height does not penetrate
$\ker AJ_{\mathbb C}$ --- are carried at full strength in the companion
\cite{HodgeCompanion}, together with the named deeper target (a class with
zero or torsion $AJ_{\mathbb C}$ and a nonzero \emph{arithmetic} level-three
invariant, where the non-archimedean local height \cite{ZhangGS} or the
$\ell$-adic Ceresa class is the retained coordinate) and the typed
analytic-to-cycle implication
(\texttt{CeresaGrossSchoenLanding.complete\_landing}). \\
...
The worked example and its GRH interfaces now live in the companion paper
\cite{GRHCompanion}, at the same registers they carried here: the helix
midpoint a geometric output of area-law scale balance and no drift; a
native 3D zero an exact focal cancellation at physical height \(Z>0\),
exactly the analytic \(L\)-zero at \(\sigma_*+i\log Z\), its finite-bank
detector equivalent to harmonic-pencil rank drop; the global \(S(t)\)
coordinate law carrying the carrier-scale census to the unit chart,
sharpened by the independent contour census to
\(S_\Gamma(T)=S_{\mathrm{mult}}(T)+N_{\mathrm{off}}^{\mathrm{mult}}(T)\);
and the operator route sharpened to a single named limit --- windowed
exhaustion of the compiled ledger --- proved \emph{equivalent} to Mathlib's
\texttt{RiemannHypothesis} from both sides
(\texttt{rh\_iff\_symmetrizedLineTower\_traceLimit}), with the earlier
uncompleted typing retired by its own impossibility theorem
(\texttt{no\_selfAdjoint\_uncompleted\_capture}).  Nothing in that
companion is consumed by the theorems of this paper.
...
\texttt{TransferContinuation.transfer\_analytic}); the completed niceness --- entire continuation,
strip bounds, and functional equation --- is moreover proved unconditionally for every twist as a
single carrier object (\texttt{cpsPolynomial\_twistedNiceness}), so the $\Re s>1$ Euler-product
abscissa is a projection-chart gate, not a carrier barrier.  The classical-$L$ identification
(Thm.~\ref{prop:localid} with \textup{(E5)}) is closed on the carrier and reproduced numerically
through $\Sym^{13}$; neither it nor convergence is owed. \texttt{fiberCompleted}
...
\emph{warp-removal transfer} of \S\ref{sec:construct}. What is machine-checked by
...
\texttt{WarpRemovalTransfer.warpRemoval\_transfer\_tendsto}; applying it requires its summable
...
\bibitem{GRHCompanion} S.~Lavery, \emph{The Carrier Geometry of the Critical Line: A Worked Example
with Two Conditional GRH Routes}, companion preprint draft (2026); the worked example at full
resolution, the two compiler-audited conditional routes, and the operator reduction certified
equivalent to Mathlib's \texttt{RiemannHypothesis} from both sides.
...
Soc.\ (1973), 181--193.
\bibitem{Turing53} A.~M.~Turing, \emph{Some calculations of the Riemann zeta-function},
Proc.\ London Math.\ Soc.\ (3) \textbf{3} (1953), 99--117.
\bibitem{Lehman70} R.~S.~Lehman, \emph{On the distribution of zeros of the Riemann
zeta-function}, Proc.\ London Math.\ Soc.\ (3) \textbf{20} (1970).
\bibitem{Trudgian11} T.~Trudgian, \emph{Improvements to Turing's method}, Math.\ Comp.\
\textbf{80} (2011), no.~276, 2259--2279.
\bibitem{Boo06} A.~R.~Booker, \emph{Artin's conjecture, Turing's method, and the Riemann
hypothesis}, Experiment.\ Math.\ \textbf{15} (2006), no.~4, 385--407.
\bibitem{Buethe15} J.~B\"uthe, \emph{A method for proving the completeness of a list of
zeros of certain \(L\)-functions}, Math.\ Comp.\ \textbf{84} (2015), no.~295;
arXiv:1308.6704.
