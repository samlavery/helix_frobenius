...
$N^{1/2}$, the per-cell forcibly-closed one is cell-bounded) \emph{through the inverse-warp transfer of
\S\ref{sec:construct} --- the arithmetic-sensitive step and the chain's named per-fiber input; the closure
...
the \emph{transfer-legitimate adapted continuation} --- the warped-Abel representation through the inverse-warp
transfer of \S\ref{sec:construct}, the chain's named per-fiber arithmetic input, machine-checked for the
character warp and evidenced for the true families by the instruments that \emph{do} discriminate (the
...
\emph{Step 2: continuation carries the axis identity everywhere.} Both completed readouts continue
analytically to a connected neighbourhood of the axis by the \emph{warped-Abel} route --- the adapted warp's
bounded primitive and inverse-warp readout (\S\ref{sec:construct}; Lemma~\ref{lem:residual} for the harmonic
and Dirichlet classes; for the cuspidal fibers the \emph{transfer-legitimate adapted continuation}, the chain's
named per-fiber input --- the warp controllability that books the primitive is itself arithmetic-neutral
transport, \texttt{detuned\_closure\_control.py}) --- and the route is
...
continuation~$\to$~axis reality~$\to$~functional equation~$\to$~theta identity; entireness is the Riemann--Hecke
