...
Before anything below is read as new: it is not.  Both halves of the law proved in \S3 are the
proof mechanisms of Pomerance, \emph{Divisors of the middle binomial coefficient}, Amer.\ Math.\
Monthly \textbf{122} (2015), no.~7, 636--644.  His \S5 opens by announcing ``an important
cleavage between the cases when $k\ge2$ and the cases when $k\le0$'' --- the sign asymmetry ---
and the two arcs appear as follows.  The positive half is display~(4) in his proof of Theorem~2:
for $p\ge2k$, $v_p\binom{2n}{n}\ge v_p(n+k)$, because if $v_p(n+k)=j>0$ then the $j$ least
significant base-$p$ digits of $n+k$ vanish, so those of $n$ are at least $p-k\ge p/2$.  The
negative half is his proof of Theorem~3, at the same threshold as ours: if $k\ge0$, $n>2k^{2}$
and $m=n-k$ has a prime factor $p>\sqrt{2n}$, then both base-$p$ digits of $n$ are below $p/2$,
so by Kummer $p\nmid\binom{2n}{n}$.  The smooth-window consequence of \S3 is one application of
that per prime factor and is therefore also essentially in print.  Section~1's criterion is
Legendre and Kummer, in the form Pomerance himself gives in his \S3.

What follows is written up because the arc formulation makes the mechanism reusable at other
turn-divisions --- see the companion note on the turn-division ledger --- and because the
digit-sum criterion of Theorem~\ref{thm:crit} is a convenient exact form.  No priority is
claimed for any of it.  See also Pomerance, \emph{Remarks on the middle binomial coefficient},
Integers \textbf{26} (2026), \#A47, whose \S2 gives the positive-shift statement in full
prime-power form; and Ford--Konyagin, Trans.\ Amer.\ Math.\ Soc.\ \textbf{374} (2021), 923--953,
who proved Pomerance's conjecture that $\{m:m\mid\binom{2m}{m}\}$ has a positive density.
...
Let $n_k$ denote the least $n$ with $\prod_{0\le i\le k}(n-i)\mid\binom{2n}{n}$.  Searching by
the criterion of Theorem~\ref{thm:crit}, with the window prefiltered by
Corollary~\ref{cor:smooth}, gives
\[
  n_0=1,\quad n_1=2,\quad n_2=2480,\quad n_3=8178,\quad n_4=45153,\quad n_5=3648841 .
\]
\emph{Correction.} An earlier version of this note recorded $n_0=2$ and $n_1=210$.  Both were
wrong, and for the same reason: the search had been started at $n=2k+2$, a floor that \#396 does
not impose.  With the search begun at $n=k+1$ one finds $n_0=1$ (as $1\mid\binom21$) and
$n_1=2$ (as $2\cdot1=2$ divides $\binom42=6$); $210$ is a solution for $k=1$ but not the least
one.  The sequence $n_1,n_2,\dots$ is OEIS A375077, which is known to sixteen terms ---
$2,\,2480,\,8178,\,45153,\,3648841,\,7979090,\,101130029,\,339949252,\dots$ up to
$2.0\times10^{19}$ --- with minimality of several terms certified by a formally verified
argument.  The computational frontier on \#396 is far ahead of anything here, and no
computational contribution is claimed.
