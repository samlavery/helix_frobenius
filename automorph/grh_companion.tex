% ============================================================================
% grh_companion.tex — standalone companion paper, split from universal.tex
% on 2026-07-26 (formerly the \input part grh_worked_example_draft.tex; see
% git history of that file for provenance).  Self-contained: compiles alone.
% Needed bibitems queued (currently uncited in body, carried for the
% Hilbert--Polya discussion): Montgomery, OdlyzkoPolya, BerryKeating, Connes.
% ============================================================================
...
% --- typography / layout (matches universal.tex) ---
...
\title{The Carrier Geometry of the Critical Line:\\
A Worked Example with Two Conditional GRH Routes}
...
\begin{abstract}
The classical $L$-function is a one-dimensional readout of a
three-dimensional object.  This paper runs that frame on the simplest
configuration it admits --- the $\pi/3$ carrier with an ordinary Dirichlet
$L$-function riding it --- as an exact audit of the geometric midpoint, the
native focal events, and the interface needed to identify carrier events
with the analytic zero set.  The unconditional content: the helix midpoint
is a geometric \emph{output} (the unique area-law scale balance and the
no-drift law), a completed native 3D zero is an exact focal cancellation at
physical height $Z>0$ whose chart ordinate is $\log Z$, the finite-bank
detector is exactly equivalent to harmonic-pencil rank drop, and both Gram
operators admit spatial lifts $G\otimes I_3$ that preserve and reflect rank
drop.  Two conditional GRH routes are retained under the identity and the
correlation readings, each with its exact premises displayed and
compiler-audited.  The genuine operator reduction is compiled: GRH follows
for any Hermitian tower whose canonical eigenvalue resolvent traces converge
locally uniformly off the real axis to the completed channel
(\texttt{grh\_of\_hermitian\_traceLimit}); the earlier uncompleted typing is
proved uninstantiable by any operator
(\texttt{no\_selfAdjoint\_uncompleted\_capture}); and the one open box ---
windowed exhaustion of the compiled ledger --- is proved \emph{equivalent}
to Mathlib's \texttt{RiemannHypothesis}
(\texttt{rh\_iff\_symmetrizedLineTower\_traceLimit}): the wall neither
overshoots nor undershoots RH.  This paper does not prove RH or GRH; it
isolates them to a single named limit, machine-checked from both sides.
\end{abstract}

\medskip
\noindent Three registers are used and never blended.  \emph{Proven}:
machine-checked in Lean~4, every named theorem at axiom footprint
$\{\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound}\}$ with no
\texttt{sorry}, built in-tree; the Lean sources live with the main paper
\cite{UniversalCarrier} and are named at each use.  \emph{Measured}:
precision-tracking numerics --- calibration and independent reproduction of
proven statements, never their proof.  \emph{Conditional}: the two GRH
routes are displayed with their exact premises, and the compiler audit
identifies which premise supplies the critical-line conclusion.  No claim in
this paper exceeds its register.  The falsifier posture is inherited from
the main paper \cite{UniversalCarrier}: the working dichotomy is stated with
its disconfirmation criteria in print, and searches against the favored
branch are part of the program, not a threat to it.

\tableofcontents

\section{Overview}\label{sec:grh-overview}

This paper runs the machinery of the main development \cite{UniversalCarrier}
on the simplest configuration it
...
given argument honest. Throughout this paper, GRH means: for every modulus
...
$L$-function, not a surrogate for it --- and this is now a packaged
\emph{relational-equality theorem}, not a pair of convergence statements.
The capstone family
(\texttt{DirichletFiberEquivalence}, \texttt{UniversalFiberEquivalence},
\texttt{PrincipalFiberEquivalence}) proves, unconditionally: for every
nonprincipal character the fibre readout converges throughout $\Re s>0$ to
the analytically \emph{continued} $L(s,\chi)$, and the classical
$L$-function is \emph{the unique entire extension} of that readout --- any
entire function reading the fibre on the strip equals $L(\cdot,\chi)$ on all
of $\mathbb C$ (\texttt{dirichletFiberEquivalence}, packaging
\texttt{spectralFiber\_readout\_tendsto\_LFunction} with the
identity-theorem core \texttt{entire\_eq\_of\_eqOn\_isOpen}).  The
uniqueness core is class-free: \emph{any} coefficient bank admits at most
one entire reading (\texttt{entire\_readings\_unique} --- no character, no
growth condition, no functional equation consumed).  The principal character
rides the eta conduction: $\zeta$ is recovered from the rescaled fibre
readout wherever $1-2^{1-s}\neq0$, with the pole booked as the conduction
factor's zero at $s=1$ and the alternating window cancelling exactly per
adjacent pair (\texttt{principalFiberEquivalence}, packaging
\texttt{etaSpectralFiber\_readout\_recovers\_zeta}).  Alongside: conduction
is lossless, the ledgered projection exact, and vanishing transports exactly
--- the completed focal residual vanishes at height $Z$ iff the represented
$L$-value does.  The state is three-dimensional, while its analytic readout
is complex-valued; the equivalence eliminates \emph{representation} as an
interface question --- what remains between the fibre and the analytic zero
set is coverage, never identity of the represented function.
...
A source audit of this chain confirms the registration and locates precisely
what it does not supply. The constructor
\path{principalZero_focalCancellation_on_carrier} has the single hypothesis
\(\rho\in\texttt{ZD.NontrivialZeros}\), with the eta factor's nonvanishing
derived from the strip bound; no \path{CarrierScaleBalanced},
\path{RidesCarrier}, \(\Re\rho=\tfrac12\) or GRH occurs in its arguments. The
mechanism is that \path{ambientState} is
\(\texttt{carrierState3D }1\,1\,(\Im\rho)\) --- a function of the ordinate
alone --- so the registration is faithful and, for the same reason, assigns
the identical event to every nontrivial zero whether or not it lies on the
line.

The absence of radial drift is an effect of the geometry and assumes nothing.
\path{CarrierScaleBalanced} requires only
\(n^{-\sigma}\,\texttt{carrierRadius}(n)\to L>0\), a condition naming no zero
set and no preselected midpoint; the radius is computed from winding at
integer sites, with no arithmetic input. Because
\(\texttt{carrierRadius}(n)/\sqrt n\to c>0\), the balance product is
\((\texttt{carrierRadius}(n)/\sqrt n)\cdot n^{1/2-\sigma}\), positive and
finite in the limit exactly when \(\sigma=\tfrac12\)
(\path{sigma_half_is_scale_critical}). The abscissa is then \emph{selected} by
uniqueness, and \path{carrierAbscissa_eq_half} is a theorem, not a definition:
the critical line is a consequence of the helix's arclength growth. With
\path{noRadialDrift_iff_carrierAbscissa} giving \(n^{x-\sigma_*}=1\iff
x=\sigma_*\), no drift characterises that geometrically determined abscissa,
which is why ``no radial drift at every zero'' renders the Riemann hypothesis
while explaining the value \(\tfrac12\) rather than assuming it.

Two statements of the same name must nevertheless be kept apart. The event's
\path{.noRadialDrift} follows from
\(({\texttt{carrierPointAtHeight}}\,Z).\mathrm{re}=\sigma_*\), whose exponent
is identically zero, and so holds for every \(Z\); carrier points do not
drift because they are carrier points. The area-law equation
\(n^{\Re\rho-\sigma_*}=1\) invoked by
\path{principalZeroNative3DSourceTransfer_of_noRadialDrift} is a statement
about \(\rho\), equivalent to \(\Re\rho=\sigma_*\). The displayed equivalence
above is thus exactly right and should be read at its strength: the source
transfer exists precisely when \(\Re\rho=\sigma_*\), so it reformulates the
conclusion faithfully rather than advancing toward it.
...
Everything in this section is main-development machinery \cite{UniversalCarrier} specialized to the basic
...
retains (\S\ref{sec:grh-projection}), every event in this paper is indexed by
...
fixed, by the source-independent double-carrier theorem of
\cite{UniversalCarrier}, before any character is
...
--- integer arithmetic, no limit taken (the primary-construct section of
\cite{UniversalCarrier}). Nothing about
...
(the carrier reflection theorem and bank reflection lemma of
\cite{UniversalCarrier}). What
...
(the carrier reflection theorem and bank reflection lemma of
\cite{UniversalCarrier}, step~1) --- no Poisson summation is consumed. The only mismatch the exchange
...
(the identity-retention/native-synthesis theorem of
\cite{UniversalCarrier}) --- the phasor of \(n\) carries
...
(its complete-state identity corollary). The 3D representation of
\(L(s,\chi)\) referred to throughout this paper is the \emph{realized} fiber:
the complete state of the main development \cite{UniversalCarrier} ---
carrier, fiber, native clock, native cells --- specialized to the basic
configuration.
...
(the double-carrier theorem of \cite{UniversalCarrier}).
...
of the reflected readout (the mode-bridge theorem of
\cite{UniversalCarrier}). The same integers
...
(the ledgered-projection theorem of \cite{UniversalCarrier}). Neither
direction is a metaphor:
...
(the carrier reflection theorem of \cite{UniversalCarrier}, item (P)). The
second forgets the
phase,
\[
\Pi_{21}(\theta,z)=z,
...
The two directions do different work in this paper. Downward is
...
reads the bank, and the two reading-hypotheses of this paper are statements
...
(the projected-equality corollary of \cite{UniversalCarrier}).
...
scales (the \(S(t)\) carrier-scale analysis of \cite{UniversalCarrier}).
The violet thread is the midline surviving
...
The elementary arithmetic shadow of (MP1) is worth one line, because it
states the balance with no limit taken: at the balanced exponent the
squared radius IS the harmonic carrier measure,
\[
  R_n^2=\Bigl(n^{-1/2}\Bigr)^{2}=\frac1n=\Delta c_n,
  \qquad\text{so}\qquad
  \sum_{n\le N}R_n^2=H_N,
\]
the harmonic number --- carrier step, sphere area weight, and
critical-line amplitude are one law expressed in three channels.  Any
other exponent breaks the identity at every \(n\), not merely in the
limit.  The booking principle that pairs with it: \emph{phase and radius
may decouple dynamically, but not informationally} --- the two channels
evolve independently, and the loss ledger carries both.
...
two charts is itself a theorem-level object (the \(S(t)\) carrier-scale
compensation theorem of \cite{UniversalCarrier}).
...
subsequent argument in this paper is an argument about heights.  The midline
...
when the signed closure channel does (the mode-bridge theorem of
\cite{UniversalCarrier}).
...
\medskip
\noindent\textbf{The geometry, stated once.} The carrier is a double-ended
helix and it is conjugate: the two lanes \(C_+\sqcup C_-\) share radius and
height law and differ only in the sign of the transverse phase, exchanged by
the involution \(J\) with \(J^2=\mathrm{id}\) (\cite{UniversalCarrier},
double carrier); the functional equation is the end-swap read through the
projection (\texttt{feReflect\_involutive}). The carrier rides both sides and
is scaled harmonically: the scale is a constant on its own integer lattice
(\texttt{Admissible.scale}), and scaling never moves a vanishing
(\texttt{vanishing\_preserves\_readout}). The \(L\)-function is the fiber, and
the fiber rides the carrier: fiber readout and \(L\) are equivalent
representations (\texttt{DirichletFiberEquivalence},
\texttt{UniversalFiberEquivalence}, \texttt{PrincipalFiberEquivalence}). The
fiber is warped to vanish at the harmonic cell edges --- the \(\mu_6\) walls at
\(k\equiv3\pmod 6\) (\texttt{pi3\_arcs\_eq}), reached from any scale by the
unit-modulus warp (\texttt{fiber\_converts\_to\_harmonic}) --- and it leaves no
residue when it vanishes: the cell channel is exactly \((\pi/3)\,L\)
(\texttt{cell\_normalized\_scalar\_exact}, \texttt{scalar\_zero\_iff\_L\_zero})
and each complete block closes in \(\mathbb Z[\zeta_6]\) in integer arithmetic.
The vanishing creates the eigenvalue at its height: the completed event at
physical height \(Z\) is an eigenvector of the ambient carrier operator with
eigenvalue \(\log Z\) and zero radial drift
(\texttt{completedThreeDZeroAtHeight\_focal\_rankDrop\_eigenvector\_noRadialDrift}).
That height corresponds to the analytic zero ordinate; it is not equal to it,
it coincides with it. Height and ordinate are coordinates in two charts, one
the exponential of the other (\texttt{carrierPointAtHeight\_exp}), the event
registers in both (\texttt{eventHeight\_eq\_focalCancellationHeight}), the
projection carries its loss ledger and is a bijection with it
(\texttt{record\_bijective}), and the global registration between the two
charts is exactly \(S(t)\) (\texttt{carrier\_scale\_compensation\_S}). The
mechanism is focal cancellation in the fiber's banks --- the lanes stay
individually large and their weighted sum closes
(\texttt{focalClosure\_iff\_rankDrop},
\texttt{completedThreeDZeroAtHeight\_iff\_L\_zero}) --- carried by phasors
that rotate at their carrier angle \(-y\log x_n\), born at magnitude zero and
grown (\texttt{resonanceHeightEquiv}).
...
\texttt{sorry}, no \texttt{axiom}, built in-tree (\S\ref{sec:grh-repro};
build instructions in \cite{UniversalCarrier}).
...
Representation itself is no longer an interface question: the
relational-equality capstones prove the fibre and its classical
\(L\)-function are equivalent representations --- unique entire reading of
the fibre readout, lossless conduction, exact ledger --- for every
character, principal included (\texttt{DirichletFiberEquivalence},
\texttt{UniversalFiberEquivalence}, \texttt{PrincipalFiberEquivalence}).
The residual interface between fibre and analytic zero set is coverage
alone.
...
The status of the assembly, recorded here at the same strength as in the
main paper's ledger.  Lean defines one fixed symmetric operator on the
completed 3D event type, proves its explicit event eigenvectors, and
packages the two resolvent versions: 3D-zero/eigenvector and
1D-zero/3D-zero/eigenvector.  The compiled
\texttt{SpectralCarrierKernelCoupling3D} joins the full analytic zero
kernel, completed focal event, ambient eigenvector, and real-generator
resolvent kernel at one identical analytic parameter; the principal contour
certificate carries that coupling as a field.  This event-space assembly
does not by itself route an arbitrary analytic zero into the event subtype.
Lean proves that the old 3D identity premise is GRH conjoined with
midpoint-free native coverage; it also proves that the old 1D routing
disjunction is automatic and that its pointwise chart-to-kernel premise
alone implies the critical-line conclusion.

The native carrier registration precedes every radial comparison.  For
...
The completed-reflection geometry is explicit before that routing step.  For every
...
The global \(S(t)\) coordinate law of the main paper remains the exact
registration theorem behind the audit of \S\ref{sec:grh-audit}: it carries
the carrier-scale census to the unit-chart census and matches jumps,
residues, and multiplicities, and the independent contour census sharpens it
to \(S_\Gamma(T)=S_{\mathrm{mult}}(T)+N_{\mathrm{off}}^{\mathrm{mult}}(T)\),
with positive multiplicity summands.  The same defect is exactly the
positive multiplicity sum of full analytic kernels lacking a
parameter-preserving \texttt{SpectralCarrierKernelCoupling3D}; consequently
the coordinates agree exactly when every enclosed kernel is coupled, and
uncoupled kernels cannot cancel one another in the contour ledger.  Its
global native branch constructs the complete physical-height event, ambient
eigenvector, harmonic rank drops, and 1D/3D trace relation in
...
\section{Reproducibility}\label{sec:grh-repro}

Every named Lean theorem in this paper is built in-tree with the main
paper's development, at axiom footprint
$\{\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound}\}$, no
\texttt{sorry}: the principal files are
\texttt{HelixResolventCapture.lean} (the general-modulus GRH interfaces and
\texttt{GRHComplete}), the spectral-coupling and zero-ledger files
(\texttt{SpectralCarrierKernelCoupling3D}, \texttt{xiZeroReflection}), and
the trace-limit family
(\texttt{grh\_of\_hermitian\_traceLimit},
\texttt{rh\_of\_symmetrizedLineTower\_canonicalTraceLimit},
\texttt{traceLimit\_of\_rh},
\texttt{rh\_iff\_symmetrizedLineTower\_traceLimit},
\texttt{no\_selfAdjoint\_uncompleted\_capture}).  Build instructions,
toolchain pin, and the instrument scripts are in the reproducibility
appendix of the main paper \cite{UniversalCarrier}.

\begin{thebibliography}{9}
\bibitem{UniversalCarrier} S.~Lavery, \emph{Carrier, Fiber, Adapters, and
Functoriality: A Novel Geometric Transport Framework --- From Repairing
Beyond Endoscopy To Universality}, companion manuscript (2026); the common
carrier system (Part~I), the $S(t)$ carrier-scale compensation theorem, the
symmetric-power tower, and the reproducibility appendix.
