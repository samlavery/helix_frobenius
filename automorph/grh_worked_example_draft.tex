...
\(L(\rho,\chi)=0\) in the critical strip.  The remaining coverage question is
whether every such \(\rho\) equals the readout of some physical-height event.
...
coordinate bookkeeping used in the correlation route; the pointwise
analytic-zero-to-native-event assertion remains the separately named
...
\texttt{NativeCarrierCoverage}\((\chi)\) & a separate premise & connects
analytic zeros to native focal events without mentioning a midpoint\\
