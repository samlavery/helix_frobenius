...
\begin{abstract}
We introduce a formally specified three-dimensional, double-ended \emph{Frobenius-dual helix
carrier} for Dirichlet $L$-function phasors.  The main point is not numerical agreement with known
zeros, but a Lean-verified package of geometric, analytic, and spectral statements.  Uniform
arclength placement on the growing-radius helix forces the area law $r_n\sim C\sqrt n$
(\code{windIntegerSite\_radius\_sq\_tendsto}); consequently $n^{-1/2}$ is the unique reciprocal
scale for a stable fiber (\code{sigma\_half\_is\_scale\_critical},
\code{scaleBalanced\_iff}).  Thus the abscissa $\Ree s=\tfrac12$ is derived from the carrier's
scale geometry before any zero-location statement enters.

The analytic fiber is equally explicit.  For a non-principal Dirichlet character $\chi$, the
phasor partial sums $\sum_{n<N}\chi(n)n^{-s}$ go all the way into the half-plane $\Ree s>0$ and
converge exactly to $L(s,\chi)$ (\code{dirichlet\_strip\_tendsto\_LFunction}), not merely in the
absolutely convergent region.  The proof formalizes bounded character sums, Abel summation, locally
uniform tail summability, and an identity-theorem identification with Mathlib's $L$-function.  For
the principal case, the eta-regularized channel converges exactly to $(1-2^{1-s})\zeta(s)$ on
$\Ree s>0$, $s\neq1$ (\code{eta\_strip\_tendsto}); on the critical line the correction factor is
nonzero, so vanishing of the regularized fiber is exactly vanishing of $\zeta$
(\code{continuous\_model\_zeta}, \code{FullFiber\_eq\_zero\_iff\_zeta}).  There is no cutoff error or
asymptotic remainder in these identities.

The three-dimensional lift separates the charged spinning channels from the neutral mass channel
while preserving the $L$-series readout (\code{phasor3D\_plane\_tsum}, \code{phasor3D\_mag3}).
The carrier's angular winding is built from prime angles by the fundamental theorem of arithmetic;
the logarithm appears only in the external Mellin bridge
$\mathrm{wind}\,n\leftrightarrow n^{it}$ (\code{wind\_glog\_eq\_cpow}).  Prime multiplication acts
by a Frobenius similitude $w\mapsto\sqrt m\,\mathrm{wind}(m)w$; conjugate chiral eigenphases form a
determinant-one unitary transverse block (\code{frobeniusSimilitude\_mul},
\code{frobeniusBlock\_unitary}).  The local vanishing mechanism is also spectral: on the line the
finite fiber is the $\ell^2$ inner product of the sampled eigenwave
$\psi_\gamma(t)=e^{i\gamma t}$ with the arithmetic data, and vanishing is exactly orthogonality
(\code{fiberCarrierFinite\_eq\_inner}, \code{fiberCarrierFinite\_eq\_zero\_iff\_orthogonal}).

The harmonic-pencil layer packages the same event algebraically.  The exact $\pi/3$ cell scaling is
not cosmetic: it is the nonzero six-cell normalization that turns the signed channel closure into
$(\pi/3)L(\tfrac12+i\log Z,\chi)$, so the harmonic cancellation event is exactly the $L$-zero event
(\code{Ucell\_ne\_zero}, \code{cell\_normalized\_scalar\_exact},
\code{scalar\_zero\_iff\_L\_zero}).  The signed and unsigned channels assemble a $2\times2$ pencil
whose determinant, and equivalently its Gram rank-drop, vanishes exactly at the corresponding
critical-line $L$-zero (\code{harmonic\_pencil\_det\_zero\_iff\_L\_zero},
\code{gramH\_rank\_drop\_iff\_L\_zero}).  More generally, the no-log-trap helix-height pencil has
Hermitian positive Gram form and real generalized eigenheights as Rayleigh quotients
(\code{helix\_eigenheight\_real}, \code{helix\_eigenheight\_rayleigh}).  A real source-height readout
lands on the critical line, and every point on that line has such a readout
(\code{admissible\_iff\_re\_half}); projection primacy is the separate completeness principle that
the real-height readout exhausts all nontrivial zeros (\code{ProjectionPrimacy},
\code{projectionPrimacy\_iff\_exhausts}).

Every named statement below is formalized in Lean~4 with Mathlib and checked by the kernel.  The
project builds with no \code{sorry}, \code{admit}, or construction-specific axiom; the axiom
footprint of the named theorems is
$\{\mathrm{propext},\ \mathrm{Classical.choice},\ \mathrm{Quot.sound}\}$.
\end{abstract}

\tableofcontents

\section{Main formalized contributions}\label{sec:contrib}

The formal development proves four independent pieces which are easily blurred if the helix picture
is read only as a visualization.

\begin{enumerate}[label=\textup{(\arabic*)}]
\item \emph{Scale geometry.}  The $\sqrt n$ radius is not stipulated.  Integers are placed at fixed
arclength spacing on the unwound line and then wound onto the helix; the resulting cylindrical
radius satisfies $r_n^2/n\to r(\pi/3)/\pi$ (Theorem~\ref{thm:arealaw}).  From this, the unique
scale-balanced exponent is $\sigma=\tfrac12$ (Theorem~\ref{thm:scalecrit}).

\item \emph{Exact analytic accumulation.}  For non-principal characters, the phasor channel goes
into all of $\Ree s>0$ and converges exactly to $L(s,\chi)$ (Theorem~\ref{thm:dstrip}); for $\zeta$,
the eta channel reaches the same half-plane and identifies zero events on the critical line exactly
(Theorems~\ref{thm:etastrip},~\ref{thm:cont},~\ref{thm:fullfiber}).  This is the analytic core of
the model: the fiber has an exact value in the strip, not a finite-window approximation.

\item \emph{Spectral and pencil readouts.}  The local on-line vanishing event is an eigenwave-data
orthogonality identity (Proposition~\ref{prop:ortho}); the harmonic pencil and its Gram form have
rank-drop exactly at the same $L$-zero event (Theorems~\ref{thm:pencildet},~\ref{thm:gram}).  The
normalizing factor is the exact $\pi/3$ cell unit, so the harmonic cancellation is an exact scalar
multiple of the $L$-fiber rather than a fitted calibration.  The finite helix-height pencil has the
self-adjoint positivity mechanism that forces real eigenheights.

\item \emph{Admissibility boundary.}  A real source-height readout reaches exactly the critical
line, and every point of the critical line is represented by such a height
(Theorem~\ref{thm:admiss}).  Completeness of this readout is named separately as projection primacy;
it is not used to prove the unconditional on-line identities.
\end{enumerate}

The rest of the paper expands these four points and records the Lean identifiers for each theorem.
...
\subsection{Python carrier-fiber method for locating vanishings}\label{sub:pythonfinder}
...
\code{FocalCancellationEigenstate.lean}, \code{GeometricReadout.lean}, and
