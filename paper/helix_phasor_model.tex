...
of every zero is a conditional bridge, left open. We do not claim that the structure function for $\Lambda$
...
(Remark~\ref{rem:eisenstein}) --- for an arithmetic that has none in its analytic form. (The closed
cell enters the readout as a unit-modulus factor, divided out to recover the $L$-value in
Section~\ref{sec:strip}; the contrast here is structural, between carrier and analytic winding.)
\end{remark}
...
than heuristically; it is the foundation the rest of the development stands on, even though the
convergence itself (Abel/$\eta$ on $\Ree s>0$) is classical.
\end{remark}
...
\begin{remark}[A faithful re-expression, not an independent detector]\label{rem:cohtaut}
Since $H^\bullet(\rho)=\C/\langle L(\rho,\chi)\rangle$, Theorem~\ref{thm:thmD} is the field-theoretic
identity ``$\C/\langle x\rangle\neq0\iff x=0$'' at $x=L(\rho,\chi)$: a faithful cohomological
\emph{re-expression} of the vanishing, carrying no information about the zeros beyond $L(\rho,\chi)$
itself. We record it as structural language, not as independent leverage; the genuine content of this
section is the polarization (Theorem~\ref{thm:kahler}), the Frobenius scaling (Theorem~\ref{thm:thmB}),
and the trace identity (Theorem~\ref{thm:thmC}).
\end{remark}
...
principle is the representability bridge \code{EveryZeroAdmissible} and, for $\zeta$, the de Branges
$\mathrm{IsHB}$ hypothesis (Remark~\ref{rem:hbscope}) --- three faces of one undecided question
(Remark~\ref{rem:admissiblescope}). It is posed, never assumed: RH fails only if a factor outside the
...
(Remark~\ref{rem:admissiblescope}) --- which appears in three equivalent guises, all faces of GRH: that
the real-height readout exhausts the zeros (\code{projectionPrimacy\_iff\_exhausts}), that a vanishing
\emph{produces} the eigenstate (Remark~\ref{rem:openlink}), and that the structure function for $\Lambda$
is Hermite--Biehler (Remark~\ref{rem:hbscope}). This work does not decide it: it poses the dichotomy ---
