...
# This file was *autogenerated* from the file sixfold_step0.sage
from sage.all_cmdline import *   # import sage library

_sage_const_0 = Integer(0); _sage_const_2 = Integer(2); _sage_const_1 = Integer(1); _sage_const_6 = Integer(6); _sage_const_3 = Integer(3); _sage_const_4 = Integer(4); _sage_const_100 = Integer(100); _sage_const_1en6 = RealNumber('1e-6'); _sage_const_7 = Integer(7); _sage_const_8 = Integer(8); _sage_const_12 = Integer(12); _sage_const_5 = Integer(5); _sage_const_11 = Integer(11); _sage_const_13 = Integer(13)# sixfold step-0 (2026-07-27): construct the non-split Weil sixfold candidate
...
AW = _sage_const_2 
BW = (_sage_const_1 , _sage_const_1 , _sage_const_0 )
KMAX = _sage_const_6 
OUT = "sixfold_step0.jsonl"

E0Q = EllipticCurve(QQ, [_sage_const_0 , _sage_const_1 ])
phiQ = E0Q.isogeny(E0Q(_sage_const_0 , _sage_const_1 ))
E1Q = phiQ.codomain()
psiQ = phiQ.dual()
XQ, YQ = psiQ.rational_maps()

PATTERNS6 = {"split": [_sage_const_3 ]*_sage_const_6 , "partial": [_sage_const_1 , _sage_const_3 , _sage_const_1 , _sage_const_3 , _sage_const_1 , _sage_const_3 ],
             "inert": [_sage_const_0 , _sage_const_0 , _sage_const_3 , _sage_const_0 , _sage_const_0 , _sage_const_3 ]}

def root_count(F, Rw, w, c):
    f = w**_sage_const_3  + F(AW)*w + c
...
def counts_at(p, k):
    ...
    nC0 = _sage_const_0 ; nE0 = _sage_const_0 
    for u in F:
        rhs = u**_sage_const_3  + F(_sage_const_1 )
        vs = []
        ...
        for v in vs:
            nE0 += _sage_const_1 
            nC0 += root_count(F, Rw, w, F(BW[_sage_const_0 ])*u + F(BW[_sage_const_1 ])*v + F(BW[_sage_const_2 ]))
    nCt = _sage_const_0 ; ker = _sage_const_1    # O_{E1} in the kernel bucket
    for x in F:
        # E1 affine points: y^2 + a1 x y + a3 y = x^3 + a2 x^2 + a4 x + a6
        # complete the square (p != 2)
        bb = a1*x + a3
        cc = x**_sage_const_3  + a2*x**_sage_const_2  + a4*x + a6
        disc = bb**_sage_const_2  + _sage_const_4 *cc
        ys = []
        if disc == _sage_const_0 :
            ys = [-bb/_sage_const_2 ]
        elif disc.is_square():
            r = disc.sqrt(); ys = [(-bb + r)/_sage_const_2 , (-bb - r)/_sage_const_2 ]
        for y in ys:
            d = Xd(x, y)
            if d == _sage_const_0 :
                ker += _sage_const_1 
            ...
            u0 = Xn(x, y)/d
            v0 = Yn(x, y)/Yd(x, y)
            nCt += root_count(F, Rw, w, F(BW[_sage_const_0 ])*u0 + F(BW[_sage_const_1 ])*v0 + F(BW[_sage_const_2 ]))
...
def newton_fe(svals, deg_half, p):
    ...
    c = [QQ(_sage_const_1 )]
    for k in range(_sage_const_1 , deg_half + _sage_const_1 ):
        acc = svals[k]
        for i in range(_sage_const_1 , k):
            acc += c[i]*svals[k-i]
        ck = -acc/k
        c.append(ck)
    if any(x.denominator() != _sage_const_1  for x in c[_sage_const_1 :]):
        return None
    full = list(c) + [QQ(_sage_const_0 )]*deg_half
    for i in range(_sage_const_0 , deg_half):
        full[_sage_const_2 *deg_half - i] = QQ(p)**(deg_half - i)*c[i]
    return full

def weil_roots(full, p, deg):
    C100 = ComplexField(_sage_const_100 )
    R = PolynomialRing(QQ, 'T'); T = R.gen()
    P = sum(full[j]*T**(deg - j) for j in range(deg + _sage_const_1 ))
    rts = []
    for r, m in P.change_ring(C100).roots():
        rts += [CDF(r)]*m
    if len(rts) != deg:
        return None, P
    if max(abs(abs(r) - math.sqrt(p)) for r in rts) > _sage_const_1en6 :
        return None, P
    return rts, P

def cluster(vals, digits=_sage_const_7 ):
    cl = {}
    for v in vals:
        key = (round(v.real(), digits), round(v.imag(), digits))
        cl[key] = cl.get(key, _sage_const_0 ) + _sage_const_1 
    prof = sorted(cl.values(), reverse=True)
    tate = _sage_const_0 
    for kk, m in cl.items():
        if abs(CDF(kk[_sage_const_0 ], kk[_sage_const_1 ]) - _sage_const_1 ) < _sage_const_1en6 :
            tate = m
    return prof, sum(m*m for m in prof), tate

def run_prime(p, fh):
    print(f"---- p = {p} ({'SPLIT' if p % _sage_const_3  == _sage_const_1  else 'INERT'}) ----", flush=True)
    ...
    for k in range(_sage_const_1 , KMAX + _sage_const_1 ):
        ...
        print(f"  k={k}: C0_aff={nC0} Ct_aff={nCt} ker={ker} E0={nE0 + _sage_const_1 }",
              flush=True)
    # fit the infinity pattern on C0 (deg-8 gate) then build the Prym factor
    for name, pat in PATTERNS6.items():
        s0 = [None] + [QQ(p**k + _sage_const_1  - (aff0[k] + pat[k-_sage_const_1 ]))
                       for k in range(_sage_const_1 , KMAX + _sage_const_1 )]
        full0 = newton_fe(s0, _sage_const_4 , p)
        if full0 is None:
            continue
        r0, P0 = weil_roots(full0, p, _sage_const_8 )
        if r0 is None:
            continue
        # C~ totals with the same pattern through the kernel bucket
        sT = [None] + [QQ(p**k + _sage_const_1  - (afft[k] + kers[k]*pat[k-_sage_const_1 ]))
                       for k in range(_sage_const_1 , KMAX + _sage_const_1 )]
        sB = [None] + [sT[k] - s0[k] for k in range(_sage_const_1 , KMAX + _sage_const_1 )]
        fullB = newton_fe(sB, _sage_const_6 , p)
        gate_int = fullB is not None
        rB = PB = None
        if gate_int:
            rB, PB = weil_roots(fullB, p, _sage_const_12 )
        rec = {"p": int(p), "pattern": name,
               "kind": "SPLIT" if p % _sage_const_3  == _sage_const_1  else "INERT",
               "sB": [int(x) for x in sB[_sage_const_1 :]],
        ...
        if p % _sage_const_3  == _sage_const_1 :
            K3 = CyclotomicField(_sage_const_3 )
        ...
        vals6 = []
        q3 = CDF(p)**_sage_const_3 
        for idx in combinations(range(_sage_const_12 ), _sage_const_6 ):
            v = CDF(_sage_const_1 )
            for i in idx:
                v *= rB[i]
            vals6.append(v/q3)
        ...
        rec.update({"H1_profile": prof1, "H1_commutant": int(comm1),
                    "L6_profile_head": prof6[:_sage_const_8 ], "L6_D": len(prof6),
        ...
        print(f"    H^1 profile {prof1} commutant {comm1}; "
              f"L^6: D={len(prof6)} head={prof6[:_sage_const_8 ]} commutant={comm6} "
              f"Tate={tate6}", flush=True)
        fh.write(json.dumps(rec, default=int) + "\n"); fh.flush()

import os
PRIMES = [int(a) for a in sys.argv[_sage_const_1 :]] if len(sys.argv) > _sage_const_1  else [_sage_const_5 , _sage_const_7 , _sage_const_11 , _sage_const_13 ]
fh = open(OUT, "a")
for p in PRIMES:
    try:
        run_prime(p, fh)
