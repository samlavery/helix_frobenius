...
P1 = primes_upto(410)
P2 = primes_upto(200)
P3 = primes_upto(80)    # v3 refinement: k=3 powered up 5->10 primes (feasible full-field enum)
KP = {1: P1, 2: P2, 3: P3}

# generic rows (A = 2 blocks via mu4, B = 1 block), excluding collisions/special loci:
#   A collision s=1, A mild branch s=0 ; B special s=0 (= A s=2, the mu4 member)
A_GEN = {-2, 2, 3}
B_GEN = {2, 3}

F = [1, 0, 0, 0, 1]
lines = ["== GGK probe v3: k-ladder, windowed, phase-coherent (k=3 powered up) =="]
lines.append("primes: k1 n=%d (<=409), k2 n=%d (<=199), k3 n=%d (<=79)"
...
for name, fam, svals in [("A", lambda s: [1, 0, 0, 0, s], [-2, 0, 1, 2, 3]),
    ...
    for row in probe(F, name, fam, svals, KP):
        store[name][row["s"]] = row
        lines.append("  s=%3d  M1=%7.3f C1=%.3f | M2=%8.3f C2=%.3f | M3=%9.3f C3=%.3f"
                     % (row["s"], row["M1"], row["C1"], row["M2"], row["C2"],
                        row["M3"], row["C3"]))

def gmean(fam, sset, kk):
    vs = [store[fam][s]["M%d" % kk] for s in sset if s in store[fam]]
    return sum(vs) / len(vs)

lines.append("-- block-count summary (generic means; A=2 blocks, B=1 block expected) --")
for kk in (1, 2, 3):
    a, b = gmean("A", A_GEN, kk), gmean("B", B_GEN, kk)
    lines.append("  k=%d:  A_gen=%8.3f  B_gen=%8.3f  ratio A/B=%6.3f  (law: ~2.00)"
                 % (kk, a, b, a / b if b else float("nan")))
out = "\n".join(lines)
print(out)
open("/Users/samuellavery/work/helix_frobenius/tmp/ggk_drift_probe_v3_results.txt", "w").write(out + "\n")
