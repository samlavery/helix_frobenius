...
| `satake_clean.py` | clean unclipped regeneration of the Satake weight tables (Δ, E11, Sym²Δ; per-run calibration on exact series = 1.00000) | **old ±4% / 0.82 figures retired as method artifacts**: Δ k=1 median 0.9997 ± 1.1%; **Sym²Δ at bank-adequate height 1.0006 ± 0.2% — the true GL(3) functoriality figure** (t≤2200 median understated by the undersized degree-3 bank — floor effect); exact silences confirmed at floor (E11 ln4/ln19/ln29). OPEN FLAG: E11 k=1 median 0.9636 ± 1.0% — a tight, reproducible −3.6% offset (calibration excludes estimator bias): conductor-11 normalization or genuine — one follow-up owed |
| `gl3_vanishing.py` | GL(3) vanishing side: locator + cells + pinning + census on Sym²Δ with the completed GL(3) kernel (Γ_ℝ(s+1)Γ_ℂ(s+11)) | **vanishing-side machinery verified at degree 3**: weld pinned to the ε=+1 ray at spread 4.0e-14 (tightest in the program, 7 orders below GL(2)), cells 1.0000π ± 0.0000π (21 cells), 22 ordinates (independently confirmed by a separate 25-digit mpmath Hardy-Z build), census 15/15 simple; **the pinning DISCRIMINATES the archimedean factor** — the wrong-parity gamma lands 0.2497π off the ray: the fiber reads its ∞-place local datum, as spectroscopy reads the finite places |
| `rank6_weld.py` | rank 6: [1,1,0,−2582,48720] (ANTS VI Table 2 — agent caught + fixed a wrong equation in the brief via the \|disc\|/N = 6 invariant), N = 5,187,563,742 (4 multiplicative bad primes), 435k-term hybrid kernel, forced-zero sign search | **agree 1.00000** (c₆ = 3,677,144.76 vs 3,677,144.73, stable to 7 sig figs across 4 window/degree choices); triple forced zero at 4.6e-9 selects bad signs (a₃₁₁₃₄₁ effectively undetermined — e⁻²⁷ weight); dead layers 3e-17–9e-11 rel; L⁽⁶⁾(1)/6! = 320.781172; **ladder 0–6**. METHOD FIXES (latent in rank-5 path, bite at large √N): G-L tail needs an x-threshold split (mpmath for x ≤ 0.5), and deg-19 raw Vandermonde (cond 6e19) floods low jets — use Chebyshev on the rescaled window |
...
## The frontier campaign (2026-07-02)

| script | does | established |
|---|---|---|
| `sha_hinge.py` | full BSD formula: \|Ш\| = L(1)·T²/(Ω·∏c_p) from scratch (E-series L(1), AGM periods, Tate's algorithm, group-law torsion); Sage as component oracle only | **\|Ш\| lands on exact square integers at machine precision, 9/9 curves** (calibration \|Ш\|=1 ×4; targets 571.a1=4, 960.d1/n1=4, 681.b1=9, 2849.a1=9); squareness has teeth (5% ingredient error → 0.4–3.0 off any square) — **the local-global obstruction measured at the hinge** |
| `maass_clocks.py` | Maass clock faces (3 level-1 forms, LMFDB rigorous a_n≤1000): pinning, cells, census, medium | **every face on the unit circle** (max \|a_p\| 1.97 < 2, 0 Kim–Sarnak exceedances) — consistent with OPEN Ramanujan–Selberg; cells 1.000π±0.0008 (**quantization analyticity-blind**); pinning discriminates parity at 1e7–1e8 and fixes the LMFDB sym-col convention |
| `ym_clocks.py` | compact-clock gap: 2D U(1) exact character expansion, ℤ_N/Villain decompactification, 3D Polyakov MC | compact ⇒ discrete dial ⇒ gap; noncompact ⇒ gapless (known: Polyakov/Göpfert–Mack — literature already states the compactness-causation thesis); **NEW [conjecture]: gap = −log(defect)/rate — the purity-defect template (`clock_zero_depth` twin); mass gap and zero displacement as one law**; falsifiers F1–F4 pre-committed; 4D untouched |
| `st_export.py` | S(t)-removal export: quarter-stadium (1085 GOE levels, validated MPS solver) + rectangle control (1281, Poisson) | partial export, honestly quantified: drift removal universal (calibration anchor 100%/0.0000π); **rigidity ladder in residual sd: Poisson 0.917π > GOE 0.531π > arithmetic ~0** — ζ measurably MORE rigid than quantum chaos; **residual peaks ON periodic-orbit lengths (Δ≤0.01) — the Gutzwiller twin of the Euler clocks** |
| `reverb_tax_noclip.py` | the no-clip control variant of reverb_tax.py (clip median−12) | the decisive control: unclipped, the Satake deficit VANISHES (mean +0.0003) — the ~1% was clip bias; law exact to ~0.1% |
