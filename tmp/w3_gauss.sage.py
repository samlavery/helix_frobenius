...
# This file was *autogenerated* from the file w3_gauss.sage
from sage.all_cmdline import *   # import sage library

_sage_const_2 = Integer(2); _sage_const_3 = Integer(3); _sage_const_1 = Integer(1); _sage_const_0 = Integer(0); _sage_const_3719 = Integer(3719); _sage_const_4000 = Integer(4000); _sage_const_7 = Integer(7); _sage_const_6 = Integer(6); _sage_const_99 = Integer(99); _sage_const_10 = Integer(10); _sage_const_4 = Integer(4); _sage_const_5 = Integer(5)# W3/J2 layer 0: generic depth-one base point + the s=1 trick.
...
R2b = PolynomialRing(F, names=('X', 'Y',)); (X, Y,) = R2b._first_ngens(2)
Gc = Y**_sage_const_2  + _sage_const_2 *X*Y + Y - X**_sage_const_3 
hp2 = -_sage_const_2 *X**_sage_const_3  + X**_sage_const_2  - X*Y + _sage_const_3 *X + _sage_const_1 
RXb = PolynomialRing(F, names=('xx',)); (xx,) = RXb._first_ngens(1)
...
import random
random.seed(int(_sage_const_3719 ))
base = None
for _ in range(_sage_const_4000 ):
    c1, c2, c3 = [F(random.randrange(_sage_const_7 )) for _ in range(_sage_const_3 )]
    l = c1 + c2*X + c3*Y
    Fq = l**_sage_const_2  - hp2
    res = Fq.resultant(Gc, Y)
    resx = RXb(res(xx, _sage_const_0 ))
    ...
    for (pfac, mfac) in resx.factor():
        if pfac.degree() == _sage_const_0 :
            continue
        d = pfac.degree()
        Kd = GF(_sage_const_7 **d, 'a') if d > _sage_const_1  else F
        x0 = pfac.roots(Kd)[_sage_const_0 ][_sage_const_0 ]
        ...
        for (y0, _) in RY(Gc(x0, yy)).roots():
            if Fq(x0, y0) != _sage_const_0 :
                continue
            got = True
            if x0 == _sage_const_0  or y0 == _sage_const_0  or hp2(x0, y0) == _sage_const_0  or Dop(Fq)(x0, y0) == _sage_const_0 :
                ok = False
                break
        ...
        if not got:
            ...
            break
    if ok and sum(d for (d, _, _, _) in pts) == _sage_const_6 :
        base = (c1, c2, c3, l, pts)
        break
...
for (d, x0, y0, Kd) in pts:
    RT = PolynomialRing(Kd, names=('zz',)); (zz,) = RT._first_ngens(1)
    t0 = RT(zz**_sage_const_3  - y0).roots()[_sage_const_0 ][_sage_const_0 ]
    u0 = -l(x0, y0)
...
for (d, x0, y0, u0, t0, Kd) in LIFT:
    ...
    assert _sage_const_6  % d == _sage_const_0 
    if d == _sage_const_1 :
        cx, cy, cu, ct = K6(x0), K6(y0), K6(u0), K6(t0)
    else:
        emb = Kd.hom([Kd.gen().minimal_polynomial().roots(K6)[_sage_const_0 ][_sage_const_0 ]], K6)
        cx, cy, cu, ct = emb(x0), emb(y0), emb(u0), emb(t0)
    for i in range(d):
        rows.append([evalpool(vb, cx, cy, cu, ct) for vb in OMEGA_BASIS])
        cx, cy, cu, ct = cx**_sage_const_7 , cy**_sage_const_7 , cu**_sage_const_7 , ct**_sage_const_7 
M0 = matrix(K6, rows)
...
WITS = []
random.seed(int(_sage_const_99 ))
while len(WITS) < _sage_const_10 :
...
for r in range(KM.nrows()):
    vec = [sum(KM[r][b] * OMEGA_BASIS[b][c] for b in range(_sage_const_10 ))
           for c in range(len(POOL))]
    row = []
    for (x0, y0, u0, t0) in WITS:
        vals = [evalpool(vec, x0, y0, u0, ZETA**k * t0) for k in range(_sage_const_3 )]
...
d6, x0r, y0r, u0r, t0r, Kd6 = LIFT[_sage_const_0 ]
if d6 > _sage_const_1 :
    emb6 = Kd6.hom([Kd6.gen().minimal_polynomial().roots(K6)[_sage_const_0 ][_sage_const_0 ]], K6)
    X0, Y0, U0, T0 = emb6(x0r), emb6(y0r), emb6(u0r), emb6(t0r)
else:
    X0, Y0, U0, T0 = K6(x0r), K6(y0r), K6(u0r), K6(t0r)
assert len(LIFT) == _sage_const_1 , "tau-layer written for the single-closed-point base"

F0poly = l**_sage_const_2  - hp2
...
def first_order(l1):
    ...
    b = vector(K6, [_sage_const_0 , -_sage_const_2 *l(X0, Y0)*l1(X0, Y0)])
    x1, y1 = A.solve_right(b)
    u1 = -(lx(X0, Y0)*x1 + ly(X0, Y0)*y1 + l1(X0, Y0))
    t1 = y1 / (_sage_const_3 *T0**_sage_const_2 )
    return x1, y1, u1, t1
...
def dual_matrices(x1, y1, u1, t1):
    ...
    for i in range(_sage_const_6 ):
        ...
        for vb in OMEGA_BASIS:
            v0 = K6(_sage_const_0 ); v1 = K6(_sage_const_0 )
        ...
        cx, cy, cu, ct = cx**_sage_const_7 , cy**_sage_const_7 , cu**_sage_const_7 , ct**_sage_const_7 
        dx, dy, du, dt = dx**_sage_const_7 , dy**_sage_const_7 , du**_sage_const_7 , dt**_sage_const_7 
...
def prvals(poolvec):
    ...
    for (xw, yw, uw, tw) in WITS:
        vals = [evalpool(poolvec, xw, yw, uw, ZETA**k * tw) for k in range(_sage_const_3 )]
        row.append(vals[_sage_const_0 ] - sum(vals)/_sage_const_3 )
    return vector(K6, row)


def poolcomb(coeffs):
    return [sum(coeffs[b] * OMEGA_BASIS[b][c] for b in range(_sage_const_10 ))
            for c in range(len(POOL))]
...
assert TB.nrows() == _sage_const_6 , f"target coord rank {TB.nrows()} != 6"
...
def quotient_coords(w6):
    ...
    return vector(K6, [sol[_sage_const_3 ], sol[_sage_const_4 ], sol[_sage_const_5 ]])


Hvecs = []
M0chk = None
for l1 in (R2b(_sage_const_1 ), X, Y):
    x1, y1, u1, t1 = first_order(l1)
    M0d, M1d = dual_matrices(x1, y1, u1, t1)
    if M0chk is None:
        M0chk = M0d
        assert M0d.right_kernel().dimension() == _sage_const_4 , "tau0 kernel mismatch"
...
print(f"GAUSS RANK (rank d-gamma) = {GR}  "
      f"({'nondegenerate, theta-like' if GR == _sage_const_3  else 'DEGENERATE — structural news'})",
      flush=True)
