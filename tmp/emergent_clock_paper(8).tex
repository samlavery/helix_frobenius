...
converse theorem consumes, entireness is instead an unconditional rank gap --- the $r+1$ Satake parameters of
$\Sym^r\pi$ cannot be dual to the $r-1$ or $r-2$ of $\sigma$, so the convolution has no pole --- with only
the finer warp dynamics exhibited per instance.
\end{abstract}
...
--- $\sigma$ on $\mathrm{GL}(r-1)$ or $\mathrm{GL}(r-2)$ --- it is the unconditional rank gap of
Lemma~\ref{lem:rankgap}: the $r+1$ Satake parameters of $\Sym^r\pi$ cannot form the dual multiset of the
$r-1$ or $r-2$ of $\sigma$, so the tensor has no trivial channel and no $s=1$ pole. Either route uses no
global parameter and no coefficient-average-to-multiplicity step, and is Galois-free.
...
converse theorem consumes, the residual vanishing \textup{(E4)} is instead the \emph{unconditional} rank gap
of Lemma~\ref{lem:rankgap}, and continuation and the vertical bound come not from a bounded primitive --- a
...
per-instance closure but a theorem, by a parameter count that needs no automorphy of $\Sym^r\pi$.

\begin{lemma}[Residual-mode vanishing for converse-theorem twists, by rank]\label{lem:rankgap}
Let $\sigma$ be cuspidal on $\mathrm{GL}(r-1)$ or $\mathrm{GL}(r-2)$ --- the twists a converse theorem into
$\mathrm{GL}(r+1)$ consumes. Then the zero-frequency mode of the carrier theta $K_{r,\sigma}$ vanishes,
$R_{r,\sigma}=0$, and $\mathcal M K_{r,\sigma}$ is entire --- unconditionally, with no per-instance check.
\end{lemma}

\begin{proof}
$R_{r,\sigma}$ is the zero-frequency (trivial-isotypic) projection of the warped tensor bank, equivalently
the residue at $s=1$ of $L(s,\Sym^r\pi\times\sigma)$. At each unramified place this residue is fed only by a
\emph{trivial channel} of the tensor of Satake data: writing $\Sym^r A_{\pi,p}=\mathrm{diag}
(\alpha^{r},\alpha^{r-2},\dots,\alpha^{-r})$ --- $r+1$ entries --- and $A_{\sigma,p}=\mathrm{diag}
(\beta_1,\dots,\beta_m)$ with $m=r-1$ or $r-2$, the eigenvalues of $\Sym^r A_{\pi,p}\otimes A_{\sigma,p}$ are
$\alpha^{\,r-2j}\beta_i$, and a trivial channel --- an eigenvalue $\equiv 1$, i.e.\ a $\zeta_p$-factor
forcing the $s=1$ pole --- requires the multiset $\{\beta_i\}_{i\le m}$ to equal the inverse symmetric-weight
multiset $\{\alpha^{-(r-2j)}\}_{0\le j\le r}$. These have $m$ and $r+1$ elements, and $m<r+1$, so they cannot
coincide: there is no trivial channel at any place, and the zero-frequency projection carries no mass,
$R_{r,\sigma}=0$. The argument counts Satake parameters --- $r+1$ against $r-1$ or $r-2$ --- and uses the
given cuspidality of $\sigma$; it invokes no global Weil--Galois parameter (so it holds for Maass $\sigma$)
and no automorphy of $\Sym^r\pi$, only the constructed $(r+1)$-dimensional local classes of
Prop.~\ref{prop:localid}. This is the rank gap the retracted global-parameter argument was reaching for,
stated as what it always was --- a parameter count, $r+1\neq r-1,\,r-2$ --- rather than an irreducibility
claim about $\Sym^r$ of the local parameter, which fails at unramified places.
\end{proof}
...
route: for the cuspidal twists a converse theorem consumes, the residual mode vanishes by the rank gap of
Lemma~\ref{lem:rankgap}, unconditionally, and continuation follows from the entire reflected carrier theta.
...
Lemma~\ref{lem:residual} for a harmonic fiber, the rank gap of Lemma~\ref{lem:rankgap} for the cuspidal
twists) and of finite order, Phragm\'en--Lindel\"of interpolates the two bounds across the strip, giving
