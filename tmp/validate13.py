...
from hida_family import chi_mod_p
...
def rd(a, r):
    ...
    return s
# self-check: chi reduced at r=7 == hida_family.chi_mod_p
bad=[n for n in range(1,124) if gcd(n,124)==1 and rd(chi(n),7)!=F(chi_mod_p(n))]
print("chi self-check vs hida_family.chi_mod_p (r=7):", "PASS" if not bad else f"FAIL at {bad[:5]}",flush=True)

good=[3,5,7,11,17,19,23,29]
exps={l:a_p_h_exponent(l) for l in good}
prim=[2,6,7,11]
k=13
t=time.time()
M=ModularSymbols(chi,k,sign=1); S=M.cuspidal_subspace(); n=S.dimension()
print(f"S_13(124,chi) cuspdim={n} field={S.hecke_matrix(3).base_ring()} [{time.time()-t:.0f}s]",flush=True)
Tk={l:S.hecke_matrix(l) for l in good}
print(f"hecke done [{time.time()-t:.0f}s]",flush=True)
for r in prim:
    V=None; per={}
    for l in good:
        e=exps[l]; tgt=F(0) if e is None else F(pow(r,e,13))  # k=13: ell^{12}=1 so no twist
    ...
    print(f"[r={r}] per-ell dims {per} -> g-residual eigenspace dim = {V.dimension()}",flush=True)
    if V.dimension()>0:
        ...
        print(f"   a_13 (T_13) eigenvalues on V mod13: {MU.eigenvalues()} (weight-13 13-old: unit root=a_13(g) res 10)",flush=True)
