...
def stage_nodes(U):
    ...
    return [x for x, w in nodes], [Phi(x)*w for x, w in nodes]
...
for U in (0.8, 1.0, 1.2, 1.4):
    ...
    print("U = %.1f (box [0,%.0f]x[0.5,40]): non-real zeros by warp t: %s ; t*(U) = %s   [%.0fs]" % (U, X, ", ".join("t=%.2f→%d" % (t, w) for t, w, m in row), tstar, time.time()-t0), flush=True)
