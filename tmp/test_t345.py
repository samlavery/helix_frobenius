...
k=3
for t in [3,4,5]:
    charr=chi.extend(1612)*om.extend(1612)**(2*t-2)   # nebentype chi*om^{2t-2}
    T=time.time()
    M=ModularSymbols(charr,k,sign=1); S=M.cuspidal_subspace(); n=S.dimension()
    print(f"\n### t={t}  nebentype chi*om^{2*t-2}  cuspdim={n}  [{time.time()-T:.0f}s]",flush=True)
    ...
    for r in prim:
        V=None
        for l in good:
            e=exps[l]; tgt=F(0) if e is None else F(pow(r,e,13))*F(pow(l,t,13))
        ...
        print(f"    [r={r}] g_om^{t} eigenspace dim = {V.dimension()}",flush=True)
        if V.dimension()>0:
            found=True
    ...
    if found: print(f"    *** CAPTURED at t={t} ***",flush=True)
