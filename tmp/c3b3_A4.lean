...
/-! ## Refutation of the Schoenberg / Widder factorization route

A Hankel kernel `K(a,b) = m((a+b)/2)` is positive semidefinite against EVERY nonnegative
weight exactly when `m` is *exponentially convex*, i.e. `m(t) = ∫ e^{2λt} dμ(λ)` for a
positive measure `μ` (Widder).  Only then does `K(a,b) = ∫ e^{λa}e^{λb} dμ` factor as a
Gram kernel.  Both weights that the target needs FAIL this at the very first minor: the
`2×2` Hankel minor at nodes `a = 0, b = t` is `m(0)·m(t) − m(t/2)²`, and `m(0) = 0`.

Consequence, stated as a property of the INSTRUMENT: no argument that uses only
`Φ ≥ 0` and `Φ` even can reach the target — a Gram/autocorrelation factorization of the
seat weight does not exist.  (Which is the compiled form of the `att525` obstruction
`Φ = ½(δ₁+δ₋₁) + aδ₀`: positive kernel, transform `cos z + a` with off-axis zeros.) -/

/-- **Exponential convexity fails for `g_y`** — the `2×2` Hankel minor is negative. -/
theorem gY_hankel_minor_neg {y t : ℝ} (hy : 0 < y) (ht : 0 < t) :
    gY y 0 * gY y t - gY y (t / 2) * gY y (t / 2) < 0 := by
  have h0 : gY y 0 = 0 := gY_zero y
  have hs : 0 < Real.sinh (2 * y * (t / 2)) := by
    have h : Real.sinh 0 < Real.sinh (2 * y * (t / 2)) :=
      Real.sinh_lt_sinh.mpr (by positivity)
    simpa using h
  have hpos : 0 < gY y (t / 2) := by
    simp only [gY]
    positivity
  have := mul_pos hpos hpos
  rw [h0, zero_mul]
  linarith

/-- **Exponential convexity fails for every even monomial `t ↦ t^{2n}`, `n ≥ 1`** — so no
single member `L_n` of the finite-difference hierarchy is free either. -/
theorem monomial_hankel_minor_neg {t : ℝ} (ht : 0 < t) {n : ℕ} (hn : 0 < n) :
    (0 : ℝ) ^ (2 * n) * t ^ (2 * n) - (t / 2) ^ (2 * n) * (t / 2) ^ (2 * n) < 0 := by
  have hz : (0 : ℝ) ^ (2 * n) = 0 := zero_pow (by omega)
  have hpos : 0 < (t / 2) ^ (2 * n) := by positivity
  have := mul_pos hpos hpos
  rw [hz, zero_mul]
  linarith

/-! ## The J-kernel bridge: `JKernelSignLaw`'s integrand IS the `g_y` pairing -/

open CriticalLinePhasor.JKernel

/-- The `d`-fiber weight of the J-kernel, `W_d(u) = Φ(u + d/2)·Φ(u − d/2)`. -/
...
/-- **THE BRIDGE.**  The closed `sinh` form of `JKernelSignLaw.jKernel_pairing_eq`,
read at `s = 2u`, is exactly `4·g_y(u)·W_d(u)`.  So the vertical sign law of the
J-kernel and the weighted-tail machinery are about the *same integrand*. -/
...
/-- **The J-kernel fiber, under ramp positivity.**  For each `d`, nonnegativity of the
tail primitive of `W_d` gives nonnegativity of the whole `d`-fiber of the vertical
pairing, at every `y ≥ 0`.  The two side conditions are the honest ones: measurability
of `W_d`, and the `Φ`-decay integrability `∫_{u>0} u·sinh(2yu)·W_d(u) du < ∞`. -/
...
#print axioms CriticalLinePhasor.WeightedTail.gY_hankel_minor_neg
#print axioms CriticalLinePhasor.WeightedTail.monomial_hankel_minor_neg
