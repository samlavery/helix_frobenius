...
\begin{remark}[Map of the construction]\label{rem:construction-map}
The construction begins with the geometry.

First comes the \emph{double-ended helix carrier}.  The carrier is the common
three-dimensional geometric object on which the construction is realized.  Its
winding, orientation, scale, and helix/anti-helix symmetry are fixed before any
particular arithmetic source is attached.  The carrier is therefore not an
$L$-function, an automorphic representation, or a source fiber.  It is the
shared geometric support on which such fibers may be realized.

The carrier is then observed through a sequence of \emph{projections}.
The first projection passes from the three-dimensional helix to its
two-dimensional carrier image; the second passes from that image to the
one-dimensional functional readout.  These maps are the link between the
geometric construction and the familiar scalar functions that appear later in
the paper:
\[
\text{3D carrier}
\longrightarrow
\text{2D projected geometry}
\longrightarrow
\text{1D functional readout}.
\]
The one-dimensional readout is an observable of the carrier state, not the
whole state itself.

Projection discards coordinates.  The construction therefore attaches a
\emph{loss ledger} to the projected state.  Radius, angle, clock data, adapter
data, and any other coordinate suppressed by a lower-dimensional readout are
retained rather than forgotten.  Thus projection simplifies the visible
representation without declaring the discarded information nonexistent.
Faithful reconstruction belongs to the projection together with its loss
ledger:
\[
\boxed{\text{projected readout}+\text{recorded projection loss}
       =\text{retained carrier state}.}
\]

Only after this geometric and projection architecture is fixed do we place a
\emph{fiber} on the carrier.  The fiber carries the source-specific
information: local factors, rank, phase law, coefficients, and arithmetic
identity.  Distinct fibers remain distinct.  The common carrier supplies a
compatible geometry; it does not identify their arithmetic data.

Each fiber has a \emph{native clock}.  Its clock determines the natural
harmonic cell structure and the progression of phase through a cell.  A
complete native cell is the source's natural cancellation unit.  Exact focal
closure is therefore a statement at complete cell boundaries:
\[
\sum_{n\in C} a_W(n)\,\omega_W(n)=0.
\]
A partial observation made before the cell has completed is a transient
readout of an unfinished cancellation process; it is not a residual left
behind by the closed cell.

Scale, chart, and clock adapters place the native cells of heterogeneous fibers
into compatible coordinates on the fixed carrier.  This normalization changes
the coordinate in which the fiber is realized, not the identity of the fiber.
In this sense the construction is universal by normalization and compatible by
carrier realization.

The double-ended carrier also carries a geometric involution exchanging the
helix and anti-helix.  Projection and the completion clock transport this
involution through the two-dimensional carrier image to the one-dimensional
functional readout.  Consequently the functional reflection is the readout of
a carrier symmetry:
\[
\text{helix}
\longleftrightarrow
...
\]
The functional equation is therefore derived from the reflected carrier
geometry and its readout intertwining, rather than added later as an
independent analytic repair.

Finally, coherent adapters allow several source fibers to occupy the common
carrier simultaneously.  Their native clocks may differ, but normalization
places their harmonic cells in compatible carrier coordinates, while the loss
ledger preserves the information suppressed by projection.  Coherent
transport can then act on the shared carrier realization without erasing the
identity of the individual fibers.

Schematically, the construction is
\[
\boxed{
\text{double-ended 3D helix}
\longrightarrow
\text{2D projection}
\longrightarrow
\text{1D readout}
...
\text{source fiber}
\longrightarrow
\text{native clock and cells}
\longrightarrow
\text{normalized carrier realization}
\longrightarrow
\text{exact focal closure}
\longrightarrow
\text{reflected functional readout}
\longrightarrow
\text{coherent transport}.
}
\]

The later analytic, automorphic, functorial, obstruction-theoretic, and Hodge
applications consume this construction.  They are consequences and test cases
of the carrier architecture, not alternative descriptions of how the carrier
itself works.
\end{remark}
\begin{remark}[Unconditional proof contract]\label{rem:proof-contract}
The construction has a small number of load-bearing obligations.  They are
defined here once.  Later applications consume these obligations through the
carrier interface and must not replace them by application-specific analytic
surrogates.

\begin{enumerate}
\item[\textup{(U1)}] \textbf{Carrier geometry.}
The double-ended helix and anti-helix are defined globally, with fixed carrier
scale, orientation, height law, and involution
\[
J:C_+\longleftrightarrow C_-.
\]
The carrier geometry exists independently of the source fiber.

\item[\textup{(U2)}] \textbf{Faithful projection and loss accounting.}
The maps
\[
3\mathrm D\longrightarrow2\mathrm D\longrightarrow1\mathrm D
\]
are defined together with a loss ledger recording every coordinate discarded
by projection.  Projection together with the ledger is faithful: the realized
carrier state can be reconstructed exactly from the projected state and its
recorded loss data.

\item[\textup{(U3)}] \textbf{Native fiber realization.}
Every source in the declared admissible class determines a finite structural
fiber law, including its local data, rank, phase law, native clock, and native
harmonic cell structure.  The realization map places this fiber on the carrier
without changing the source data or identifying distinct fibers.

\item[\textup{(U4)}] \textbf{Exact complete-cell closure.}
For every realized admissible fiber and every complete native cell \(C\), the
adapted fiber state closes exactly:
\[
\sum_{n\in C} a_W(n)\,\omega_W(n)=0.
\]
This is an exact boundary statement.  No uncancelled cell remainder or
independent constant residual mode survives completion of the native cell.

\item[\textup{(U5)}] \textbf{Readout identification.}
The one-dimensional carrier readout agrees exactly with the source functional
datum on its original domain of definition.  In the Euler setting this means
that the local carrier readout gives the specified local factor at every place
and that the global readout unfolds to the corresponding Euler--Dirichlet or
completed \(L\)-datum on its convergence domain.

\item[\textup{(U6)}] \textbf{Reflection intertwining.}
Carrier reflection commutes with projection and the completion clock.  If
\(\mathcal R\) denotes the completed one-dimensional readout, then
\[
\mathcal R(JW)(s)
=
\varepsilon_W\,\mathcal R(W^\vee)(\kappa-s).
\]
Thus the functional reflection is the one-dimensional image of the
helix--anti-helix involution.

\item[\textup{(U7)}] \textbf{Exhaustion.}
Every source event represented by the declared fiber law is faithfully present
in the carrier state and in the projection-plus-ledger record.  No additional
source event lies outside the realized carrier merely because it is invisible
in one projected coordinate.

\item[\textup{(U8)}] \textbf{Coherent synchronization and transport.}
For any finite compatible family of admissible fibers, the clock and chart
adapters place their native cells on one common carrier coherently.  Transport
preserves source identity, exact boundary closure, reflection, and retained
loss data, and composition of transports agrees with transport by the
composite source law.

\item[\textup{(U9)}] \textbf{Obstruction honesty.}
Failure of a realized fiber to satisfy the required carrier condition is
detected as retained obstruction data rather than erased by projection or
declared closed from an aggregate scalar alone.  Removal requires an explicit
admissible enlargement and an exact certificate that the obstruction itself,
not merely its aggregate readout, vanishes.
\item[\textup{(U4)}] \textbf{Identity preservation.}
Every admissible source fiber carries a distinguished source identity
\(\operatorname{Id}(W)\).  Carrier realization, clock and chart normalization,
projection together with its loss ledger, reflection to the dual fiber, and
coherent transport preserve the represented source identity in the precise
sense appropriate to each map.
...
\pi(\widetilde W)
\]
is a normalized carrier realization followed by projection, then the projected
readout together with its loss record represents the same source fiber:
\[
\operatorname{Id}
\bigl(\pi(\widetilde W),\operatorname{Loss}(\widetilde W)\bigr)
=
\operatorname{Id}(W).
\]
Likewise, an admissible transport \(T\) satisfies
\[
\operatorname{Id}(T W)
=
T_\ast\operatorname{Id}(W),
\]
and carrier reflection sends the fiber to its specified dual identity
\(W^\vee\), not to an unrelated fiber with the same scalar readout.

Thus equal carrier coordinates, equal focal values, or equal projected
readouts do not by themselves identify two fibers.  Source identity is retained
through the full carrier--projection--ledger state.


GPT PROOF OBLIGATIONS v1 
\begin{enumerate}

\item[\textup{(G1)}] \textbf{Complete double-ended carrier.}

Prove that the fixed three-dimensional helix/anti-helix carrier is globally

defined and carries an involution
...
\item[\textup{(G2)}] \textbf{Faithful ledgered projection.}

Prove that

\[

3D\longrightarrow2D\longrightarrow1D

\]

together with the loss ledger retains the complete realized state:

\[

\operatorname{Rec}\bigl(\Pi(\mathsf X_W),\mathscr D_W\bigr)
...
\item[\textup{(G3)}] \textbf{Identity preservation.}

Prove that realization, normalization, projection plus ledger, reflection, and

transport preserve the identity of the represented source, with reflection

sending \(W\) to the specified dual \(W^\vee\).

\item[\textup{(G4)}] \textbf{Native synthesis.}

Prove that every admissible finite source law determines its source fiber,

native clock, harmonic cell structure, phase law, and carrier adapter:
...
(F_W,\tau_W,\mathcal L_W,\omega_W).

\]

\item[\textup{(G5)}] \textbf{Exact complete-cell closure.}

Prove that every complete native cell of every admissible realized fiber closes

exactly:

\[

\forall W\in\mathsf{Adm}\;

\forall C\in\mathcal L_W,
...
\item[\textup{(G6)}] \textbf{Three-dimensional mode / one-dimensional pole

dictionary.}

Prove the exact correspondence between the retained three-dimensional

focal or constant mode and the one-dimensional constant or pole mode:
...
In particular, no independent one-dimensional residual mode may appear if the

corresponding three-dimensional carrier mode vanishes.

\item[\textup{(G7)}] \textbf{Exact functional readout identity.}

Prove that the one-dimensional projection of the realized carrier state is

exactly the specified source Euler or completed functional datum:

\[

\mathcal R_W^{\mathrm{comp}}(s)=\Lambda(s,W)

\]

on the initial domain, with the correct local factors at every place.

\item[\textup{(G8)}] \textbf{Reflection intertwining.}

Prove that carrier reflection projects exactly to functional reflection:
...
\varepsilon_W\Lambda(\kappa-s,W^\vee).

\]

\item[\textup{(G9)}] \textbf{One-dimensional analytic landing.}

Prove that the complete realized three-dimensional state projects to a valid

global one-dimensional function with:

\[

\text{analytic or meromorphic continuation},

\]

\[

\text{the pole structure dictated exactly by G6},
...
\text{the required polynomial vertical-strip growth}.

\]

\item[\textup{(G10)}] \textbf{Exhaustion and coherent transport.}

Prove that source events are neither lost nor invented by carrier realization

and ledgered projection, and that synchronization and structural transport

preserve identity, closure, reflection, ledger state, and readout while

composing coherently:

\[

\mathsf X(T_2\circ T_1)

=

\mathsf X(T_2)\circ\mathsf X(T_1).
...
\part{The construction}\inst{make this main part first}

\part{The exact gauge and the productivity boundary} \inst{reframe correctly}
...
\inst{MOVE THIS TO ITS OWN SECTION}
...
\emph{What we do and do not say.} \bullshit{muddy water} Not: ``$S(t)$ disappears.'' Rather: the lifted representation does not
...
\part{The universal transport} \inst{this should be first}
...
\paragraph{Active research.} \bullshit {half correct fabrication}Theorem~\ref{the:func} proves $\Sym^r$ functoriality from the candidate
