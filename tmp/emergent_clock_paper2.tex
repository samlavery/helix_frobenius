...
\noindent The transport construction of Part~II is independent of converse theory. We now specialize its
output (Theorem~\ref{thm:transport}) to the symmetric-power Satake fiber and ask whether the resulting adelic
datum satisfies the hypotheses of a classical $\mathrm{GL}_n$ converse theorem. This part is deliberately
organized as a hypothesis ledger: each converse-theorem input is discharged by a named result of Part~II, and
no carrier theory is reproved here.
...
of the reducible parameter. This is \emph{not} a carrier residue --- Theorem~\ref{thm:transport}(d) gives
$R_{r,\tau}=0$ for the configured fiber regardless --- but an intrinsic $\zeta$-factor of the reducible datum,
and there $\Sym^r\pi$ is the corresponding \emph{isobaric} automorphic representation, already known by
Gelbart--Jacquet \cite{GJ}, Kim--Shahidi \cite{KimShahidi}, and (for dihedral $\pi$) the theory of CM forms.
