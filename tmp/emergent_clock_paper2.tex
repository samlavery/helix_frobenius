...
statement is Galois-free and applies to Maass $\sigma$ verbatim.
\end{proof}
...
their real weights can be \emph{forced} to drive the cell residual $D_C=\sum_{n\in C}a_n\,\omega(n)$ to zero. The
...
rank-drop. The honest boundary: the controllability core is \emph{proved}; what is \emph{verified numerically},
not proved for every cuspidal fiber, is the \emph{reachability} --- that the readout-preserving warp supplies two
independent directions at each cell, closing it (entireness). \emph{Boundedness does not ride on this.} The
vertical-strip bound is structural, coming from the \emph{Euler factor}: on its convergence domain the full helix
is the convergent product of local factors $\det(1-A_{p}q_p^{-s})^{-1}$, each a bounded rational function with
$|A_p|$ Ramanujan-controlled, so the readout is finite order there; the functional equation extends that bound
across the strip (Lemma~\ref{lem:bounded}), given entireness --- \emph{independent} of the forcing chart. Hence
the per-cell forcing weights need \emph{not} be bounded for $L$ to be finite order (they stay $O(1)$ for
$\Sym^5$ but grow with height for $\Sym^{13}$, a cosmetic feature of the constructive route, not a bound on $L$).
Forcible closure thus upgrades the corroboration of Lemma~\ref{lem:rankgap} to a constructive mechanism with a
machine-checked core; the sole genuinely open input is the reachability of the forcing for every cuspidal fiber
(entireness), boundedness being the Euler-factor consequence above.
...
inductive (\texttt{composition\_test.py}). The whole tower is reachable --- there is no degree wall. Along the
...
Let $\pi$ have central character $\omega_\pi$, unitarily normalized so that
$\varphi_{\pi,v}=\omega_{\pi,v}^{1/2}\operatorname{diag}(\alpha_v,\alpha_v^{-1})$ at unramified $v$
($|\alpha_v|=1$): the balanced part $\operatorname{diag}(\alpha_v,\alpha_v^{-1})$ has determinant one, and
...
\[
  \Lambda(s,\Sym^r\pi\times\tau)=\varepsilon_{r,\sigma}\,\Lambda(1-s,\Sym^r\pi\times\tau^\vee),
...
ledger --- \emph{not} forced to one --- and $W_{r,\tau}^\vee\simeq W_{r,\tau^\vee}$ read on the anti-helix.
The genuinely self-dual sub-case ($\sigma\simeq\tau^\vee$, $\det\varphi_\tau=1$) is
...
is the anti-helix reading at $1-s$, so $\mathcal M K_{r,\tau}(s)=\varepsilon_{r,\sigma}\,\mathcal M
...
$\Lambda(s)=\varepsilon_{r,\sigma}\,\Lambda(1-s;\tau^\vee)$, $|\varepsilon_{r,\sigma}|=1$.
...
twisted niceness of $L(r\circ\varphi_\pi\times\tau)$ for all $\sigma$, entire and bounded and self-dual --- is
