...
$\log n$ (arithmetic functions, not arbitrary per-term phases) --- whose real weights drive the cell residual
...
independent. The native cell reaches that stage by the information-preserving transport along the carrier height:
the reparametrisation $x\mapsto kx+c$ is a bijection of $\mathbb R$ with an explicit two-sided inverse
(\texttt{ConeProjection.affineCoord\_bijective}, \texttt{affineCoordEquiv}), so moving between carrier stages
neither loses nor gains information, and \texttt{ConeProjection.reconstruct\_record} certifies that the fiber's
projection-and-ledger round-trip is exact ($\texttt{reconstruct}(\texttt{record}\,f)=f$) --- no channel is
destroyed in transit. Applying \texttt{residual\_forcible} at the independent stage kills the residual exactly,
and the \emph{inverse} stage transport --- being information-preserving --- returns the corrected state to the
originating cell without reintroducing the residual. Thus per-cell closure follows from \emph{global} carrier
reachability composed with the faithful stage round-trip, \emph{not} from cell-by-cell linear independence ---
every leg machine-checked (footprint $\{\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound}\}$, no
\texttt{sorry}). The constructive rate is what the numerics exhibit: three coherent generators drive $|D_C|$ to
$4\times10^{-15}$ on $\Sym^5\Delta$ and $\Sym^{13}\Delta$ with weights $|x|<1.5$ (\texttt{forcible\_closure.py}).
\end{lemma}
...
independent stage, not cell by cell, behind the structural rank-drop. The boundary is sharp, and not where a first reading places it. \emph{Entireness itself is
...
from a theta of the function.  The completed twisted reflection is theorem-level and \emph{general}: the
completed carrier readout $\Lambda=\gamma\,L$ of the whole $\Sym^r\pi\times\sigma$ family satisfies
$\Lambda(s)=\varepsilon\,\Lambda^\vee(1-s)$ as the carrier involution $J$, machine-checked with \emph{no}
Dirichlet hypothesis (\texttt{CompletedReflectionFiber.symTensorCompleted\_FE}, any $r$ and any duality-stable
twist, assembling the general per-place reflection \texttt{localPoly\_reciprocal} and the general completion
self-duality \texttt{clockCompletion\_selfdual}; footprint
$\{\texttt{propext},\texttt{Classical.choice},\texttt{Quot.sound}\}$), together with the residue-free entireness
of Lemma~\ref{lem:rankgap}.  The exact identification of this completed carrier readout with the classical global
...
only for the Dirichlet model --- and the completed twisted reflection itself is theorem-level for the whole
family (\texttt{symTensorCompleted\_FE}, above).  The remaining limitation is formalization scope only: the
