...
## 6. Reference modules and commands

The reference implementation is split across four modules (Python 3, `numpy`;
`mpmath` only for analytic ground truth):

- **`helix_geometry_3d.py`** — the 3-D carrier + arrows of this spec
  (`RealCharacter`, `EtaCoefficient`, `CHARACTERS`, `phasor_sum`,
  `carrier_point`, `phasors_per_turn`, `analytic_L`/`analytic_zeros` ground
  truth). Run `python3 helix_geometry_3d.py` for the validation demo.
- **`helix_phasor_tool.py`** — Lean-annotated definitions, Abel-summation strip
  extension, vanishing-point finder (no mpmath in the model).
  Run `python3 helix_phasor_tool.py`.
- **`focal_closure.py`** — exact harmonic cancellation + the focal-closure
  growth locator (lane-balance events of the growing finite bank; families
  include complex characters, Ramanujan Δ, elliptic curve 11.a).
  Run `python3 focal_closure.py test` / `demo`.
- **`carrier_fiber/carrier_fiber.py`** — the finite carrier/fiber harness
  (native π/3 scan, harmonic pencil marker, c1 clock validation; finite-first).
  Run `python3 carrier_fiber.py test` from `carrier_fiber/`.

```bash
python3 verify_first_zeros.py        # 10 zeros x 5 characters vs analytic truth
python3 pi3_highdps_zeros.py         # the same on the pi/3 coordinate, dps-30
```
...
2. **Phasor cancellation** — independently, the truncated phasor sum `S(γ_k)`
   ...
   `O(1)`. Specifically `|S_N(γ_k)| ≈ c_χ/√N` with a character-dependent
   constant (measured: `c ≈ 0.667` for `chi3`, `c ≈ 0.500` for `eta`/ζ,
   stable in `N`).
...
- **No exact finite zero:** at `σ=½` the series is conditionally convergent;
  `|S_N| ~ c_χ/√N` (character-dependent constant, §7), so a truncation is never exactly 0 — it has to be read as a
