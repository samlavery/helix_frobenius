...
theorem re_zeta58_lower {σ : ℝ} (h1 : 1/2 ≤ σ) (h2 : σ ≤ 2) :
    1 + (∑ n ∈ Finset.Icc (2:ℕ) 100, ((n : ℝ) ^ (-σ) * cos58c n))
      - (100 : ℝ) ^ (-σ) * (100/58 + 1/2)
      - 458 * ((100 : ℝ) ^ (-σ - 2) + (100 : ℝ) ^ (-σ - 1) / (σ + 1))
    ≤ (riemannZeta (↑σ + 58 * Complex.I)).re := by
  set s : ℂ := ↑σ + 58 * Complex.I with hsdef
  have hsre : s.re = σ := by rw [hsdef]; simp
  have hsim : s.im = 58 := by rw [hsdef]; simp
  ...
  have hre_ge : EMval.re - ‖riemannZeta s - EMval‖ ≤ (riemannZeta s).re := by
    ...
    linarith [this.1]
  -- the error majorant
  have hprod : ‖s‖ * ‖s + 1‖ ≤ 3660 := by
    have hns : ‖s‖ ≤ 60 := by
      calc ‖s‖ ≤ ‖(↑σ : ℂ)‖ + ‖(58 : ℂ) * Complex.I‖ := by
  ...
  have herr : ‖s‖ * ‖s + 1‖ * (1/8)
      * ((((100 : ℕ) : ℝ)) ^ (-s.re - 2)
      ...
      ≤ 458 * ((100 : ℝ) ^ (-σ - 2) + (100 : ℝ) ^ (-σ - 1) / (σ + 1)) := by
  ...
  have hhead_re : (∑ n ∈ Finset.range (100 + 1), (((n : ℝ)) : ℂ) ^ (-s)).re
      = 1 + ∑ n ∈ Finset.Icc (2:ℕ) 100, ((n : ℝ) ^ (-σ) * cos58c n) := by
    rw [Complex.re_sum]
    ...
    have hcong : ∑ n ∈ Finset.Icc (2:ℕ) 100, ((((n : ℝ)) : ℂ) ^ (-s)).re
        = ∑ n ∈ Finset.Icc (2:ℕ) 100, ((n : ℝ) ^ (-σ) * cos58c n) := by
      refine Finset.sum_congr rfl ?_
      intro n hn
      have hn2 : 2 ≤ n := (Finset.mem_Icc.mp hn).1
      have h58 : -s = -(↑σ + ↑(58 : ℝ) * Complex.I) := by
        rw [hsdef]
        norm_num
      rw [h58, (nat_cpow_neg_split (n := n) (by omega) σ 58).1]
      simp only [cos58c]
    rw [hcong]
    ring
  -- the boundary term
  have hbdy : ‖(((100 : ℕ) : ℝ) : ℂ) ^ (1 - s) / (s - 1)‖
    ...
    have hden : (58 : ℝ) ≤ ‖s - 1‖ := by
      have habsim := Complex.abs_im_le_norm (s - 1)
      have him : (s - 1).im = 58 := by simp [hsim]
      rw [him] at habsim
      calc (58 : ℝ) = |(58 : ℝ)| := by norm_num
        _ ≤ ‖s - 1‖ := habsim
  ...
  have hEMre : (1 + ∑ n ∈ Finset.Icc (2:ℕ) 100, ((n : ℝ) ^ (-σ) * cos58c n))
      - (100 : ℝ) ^ (-σ) * (100/58) - (100 : ℝ) ^ (-σ) * (1/2)
      ≤ EMval.re := by
    rw [hEMdef]
...
#print axioms CriticalLinePhasor.DVP.re_zeta58_lower
end AxiomAudit
