...
theorem re_zeta63_lower {σ : ℝ} (h1 : 1/2 ≤ σ) (h2 : σ ≤ 2) :
    1 + (∑ n ∈ Finset.Icc (2:ℕ) 100, ((n : ℝ) ^ (-σ) * cos63c n))
      - (100 : ℝ) ^ (-σ) * (100/63 + 1/2)
      - 537 * ((100 : ℝ) ^ (-σ - 2) + (100 : ℝ) ^ (-σ - 1) / (σ + 1))
    ≤ (riemannZeta (↑σ + ((63:ℕ):ℝ) * Complex.I)).re := by
  set s : ℂ := ↑σ + ((63:ℕ):ℝ) * Complex.I with hsdef
  have hsre : s.re = σ := by rw [hsdef]; simp
  have hsim : s.im = 63 := by rw [hsdef]; simp
  ...
  have hre_ge : EMval.re - ‖riemannZeta s - EMval‖ ≤ (riemannZeta s).re := by
    ...
    linarith [this.1]
  -- the error majorant
  have hprod : ‖s‖ * ‖s + 1‖ ≤ 4290 := by
    have hns : ‖s‖ ≤ 65 := by
      calc ‖s‖ ≤ ‖(↑σ : ℂ)‖ + ‖(((63:ℕ):ℝ) : ℂ) * Complex.I‖ := by
  ...
  have herr : ‖s‖ * ‖s + 1‖ * (1/8)
      * ((((100 : ℕ) : ℝ)) ^ (-s.re - 2)
      ...
      ≤ 537 * ((100 : ℝ) ^ (-σ - 2) + (100 : ℝ) ^ (-σ - 1) / (σ + 1)) := by
  ...
  have hhead_re : (∑ n ∈ Finset.range (100 + 1), (((n : ℝ)) : ℂ) ^ (-s)).re
      = 1 + ∑ n ∈ Finset.Icc (2:ℕ) 100, ((n : ℝ) ^ (-σ) * cos63c n) := by
    rw [Complex.re_sum]
    ...
    have hcong : ∑ n ∈ Finset.Icc (2:ℕ) 100, ((((n : ℝ)) : ℂ) ^ (-s)).re
        = ∑ n ∈ Finset.Icc (2:ℕ) 100, ((n : ℝ) ^ (-σ) * cos63c n) := by
      refine Finset.sum_congr rfl ?_
      intro n hn
      have hn2 : 2 ≤ n := (Finset.mem_Icc.mp hn).1
      have h63 : -s = -(↑σ + ↑(((63:ℕ)):ℝ) * Complex.I) := by
        rw [hsdef]
      rw [h63, (nat_cpow_neg_split (n := n) (by omega) σ (((63:ℕ)):ℝ)).1]
      simp only [cos63c]
    rw [hcong]
    ring
  -- the boundary term
  have hbdy : ‖(((100 : ℕ) : ℝ) : ℂ) ^ (1 - s) / (s - 1)‖
    ...
    have hden : (63 : ℝ) ≤ ‖s - 1‖ := by
      have habsim := Complex.abs_im_le_norm (s - 1)
      have him : (s - 1).im = 63 := by simp [hsim]
      rw [him] at habsim
      calc (63 : ℝ) = |(63 : ℝ)| := by norm_num
        _ ≤ ‖s - 1‖ := habsim
  ...
  have hEMre : (1 + ∑ n ∈ Finset.Icc (2:ℕ) 100, ((n : ℝ) ^ (-σ) * cos63c n))
      - (100 : ℝ) ^ (-σ) * (100/63) - (100 : ℝ) ^ (-σ) * (1/2)
      ≤ EMval.re := by
    rw [hEMdef]
...
#print axioms CriticalLinePhasor.DVP.re_zeta63_lower
end AxiomAudit
