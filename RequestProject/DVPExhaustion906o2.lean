...
import RequestProject.DVPLift906o2

/-!
# The exhaustion at height 906/2 (rung-453)
...
/-- Detector continuity to height 237 (same proof as the 58 version). -/
theorem hardyG_continuousOn906o2 : ContinuousOn hardyG (Set.Icc (10:ℝ) (906/2)) := by
  intro t ht
...
/-- Sign change to height 237. -/
theorem hardyG_sign_change906o2 {a b : ℝ} (hab : a ≤ b) (h10 : 10 ≤ a)
    (h237 : b ≤ 906/2) (hsign : hardyG a * hardyG b < 0) :
    ∃ t : ℝ, a < t ∧ t < b ∧ line t ∈ ZD.NontrivialZeros := by
  have hsub : Set.Icc a b ⊆ Set.Icc (10:ℝ) (906/2) :=
    Set.Icc_subset_Icc h10 h237
  have hcont : ContinuousOn hardyG (Set.Icc a b) :=
    hardyG_continuousOn906o2.mono hsub
...
def LadderData906o2 : Prop :=
  ∃ t : Fin 238 → ℝ, StrictMono t
    ∧ (∀ i, 10 ≤ t i ∧ t i ≤ 906/2)
    ∧ ∀ i : Fin 237, hardyG (t i.castSucc) * hardyG (t i.succ) < 0

/-- **Exhaustion**: the ladder plus the census force every strip zero of
ordinate `≤ 58` onto the line. -/
theorem online_below_906o2_of_ladder (hL : LadderData906o2) :
    ∀ ρ ∈ ZD.NontrivialZeros, |ρ.im| ≤ 906/2 → ρ.re = 1/2 := by
  obtain ⟨t, htmono, htrange, hsign⟩ := hL
  -- the fourteen interior zeros
  have hex : ∀ i : Fin 237, ∃ z : ℝ, t i.castSucc < z ∧ z < t i.succ
      ∧ line z ∈ ZD.NontrivialZeros := by
    intro i
    exact hardyG_sign_change906o2
      (htmono (Fin.castSucc_lt_succ (i := i))).le
      (htrange i.castSucc).1 (htrange i.succ).2 (hsign i)
  choose zs hz1 hz2 hz3 using hex
  have hzs_lt : ∀ i j : Fin 237, i < j → zs i < zs j := by
    intro i j hij
  ...
  have hmem_line : ∀ i : Fin 237, line (zs i) ∈ stripZeroFinset (906/2) := by
    intro i
    apply (stripZeroWindow_finite (906/2)).mem_toFinset.mpr
    exact ⟨hz3 i, by rw [lineIm]; exact hzs_pos i,
      by rw [lineIm]; exact hzs_le i⟩
  have hmem_ρ' : ρ' ∈ stripZeroFinset (906/2) := by
    apply (stripZeroWindow_finite (906/2)).mem_toFinset.mpr
    exact ⟨hρ'_mem, hρ'_im_pos, hρ'_im_le⟩
  set S : Finset ℂ :=
    insert ρ' (Finset.image (fun i => line (zs i)) Finset.univ) with hSdef
  have hinj : Function.Injective (fun i : Fin 237 => line (zs i)) := by
    intro i j hij
  ...
  have hcard : S.card = 238 := by
    rw [hSdef, Finset.card_insert_of_notMem hρ'_notmem,
      Finset.card_image_of_injective _ hinj]
    simp
  have hsub : S ⊆ stripZeroFinset (906/2) := by
    intro x hx
    rw [hSdef] at hx
  ...
  have hone : ∀ x ∈ S, 1 ≤ ZD.xiOrderNat x := by
    intro x hx
    have hx' := hsub hx
    have hmem := (stripZeroWindow_finite (906/2)).mem_toFinset.mp hx'
    exact ZD.xiOrderNat_pos_of_mem_NontrivialZeros hmem.1
  have h13 : 238 ≤ ∑ x ∈ S, ZD.xiOrderNat x := by
    calc (238:ℕ) = S.card := hcard.symm
      _ = ∑ _x ∈ S, 1 := by rw [Finset.card_eq_sum_ones]
      _ ≤ ∑ x ∈ S, ZD.xiOrderNat x := Finset.sum_le_sum hone
  have h12 : ∑ x ∈ S, ZD.xiOrderNat x ≤ stripZeroCountMult (906/2) := by
    rw [stripZeroCountMult]
    exact Finset.sum_le_sum_of_subset hsub
  rw [stripZeroCountMult_906o2] at h12
  omega

end CriticalLinePhasor.DVP

section AxiomAudit
#print axioms CriticalLinePhasor.DVP.online_below_906o2_of_ladder
end AxiomAudit
