import RequestProject.DVPCos743o2Psum
import RequestProject.DVPSegment126Core

/-!
# The segment core at `t = 743/2` (rung-371.5; N = 500)

The single Euler–Maclaurin extraction all seven σ-intervals consume:

`Re ζ(σ+(743/2)i) ≥ 1 + ∑_{n=2}^{500} n^{−σ}·cos(743·(log n)/2)
  − 500^{−σ}·(1000/743 + 1/2) − 17486·(500^{−σ−2} + 500^{−σ−1}/(σ+1))`
...
theorem re_zeta743o2_lower {σ : ℝ} (h1 : 1/2 ≤ σ) (h2 : σ ≤ 2) :
    1 + (∑ n ∈ Finset.Icc (2:ℕ) 500, ((n : ℝ) ^ (-σ) * cos743o2c n))
      - (500 : ℝ) ^ (-σ) * (1000/743 + 1/2)
      - 17486 * ((500 : ℝ) ^ (-σ - 2) + (500 : ℝ) ^ (-σ - 1) / (σ + 1))
    ≤ (riemannZeta (↑σ + ↑((((743:ℕ)):ℝ)/2) * Complex.I)).re := by
  set s : ℂ := ↑σ + ↑((((743:ℕ)):ℝ)/2) * Complex.I with hsdef
  have hsre : s.re = σ := by rw [hsdef]; simp
  have hsim : s.im = ((743:ℕ):ℝ)/2 := by rw [hsdef]; simp
  ...
  have hre_ge : EMval.re - ‖riemannZeta s - EMval‖ ≤ (riemannZeta s).re := by
    ...
    have := abs_le.mp habs
    linarith [this.1]
  have hprod : ‖s‖ * ‖s + 1‖ ≤ 559503/4 := by
    have hns : ‖s‖ ≤ 747/2 := by
      calc ‖s‖ ≤ ‖(↑σ : ℂ)‖ + ‖↑((((743:ℕ)):ℝ)/2) * Complex.I‖ := by
        ...
        _ ≤ 747/2 := by
            ...
            linarith
    have hns1 : ‖s + 1‖ ≤ 749/2 := by
      calc ‖s + 1‖ ≤ ‖s‖ + ‖(1 : ℂ)‖ := norm_add_le _ _
        _ ≤ 749/2 := by rw [norm_one]; linarith
    calc ‖s‖ * ‖s + 1‖ ≤ (747/2) * (749/2) :=
          mul_le_mul hns hns1 (norm_nonneg _) (by norm_num)
      _ = 559503/4 := by norm_num
  have herr : ‖s‖ * ‖s + 1‖ * (1/8)
      * ((((500 : ℕ) : ℝ)) ^ (-s.re - 2)
        + (((500 : ℕ) : ℝ)) ^ (-s.re - 1) / (s.re + 1))
      ≤ 17486 * ((500 : ℝ) ^ (-σ - 2) + (500 : ℝ) ^ (-σ - 1) / (σ + 1)) := by
  ...
  have hhead_re : (∑ n ∈ Finset.range (500 + 1), (((n : ℝ)) : ℂ) ^ (-s)).re
      = 1 + ∑ n ∈ Finset.Icc (2:ℕ) 500, ((n : ℝ) ^ (-σ) * cos743o2c n) := by
    rw [Complex.re_sum]
    ...
    have hcong : ∑ n ∈ Finset.Icc (2:ℕ) 500, ((((n : ℝ)) : ℂ) ^ (-s)).re
        = ∑ n ∈ Finset.Icc (2:ℕ) 500, ((n : ℝ) ^ (-σ) * cos743o2c n) := by
      refine Finset.sum_congr rfl ?_
      intro n hn
      have hn2 : 2 ≤ n := (Finset.mem_Icc.mp hn).1
      have h743 : -s = -(↑σ + ↑((((743:ℕ)):ℝ)/2) * Complex.I) := by
        rw [hsdef]
      rw [h743, (nat_cpow_neg_split (n := n) (by omega) σ ((((743:ℕ)):ℝ)/2)).1]
      rw [show ((((743:ℕ)):ℝ)/2) * Real.log ((n:ℕ))
          = (((743:ℕ)):ℝ) * (Real.log ((n:ℕ)) / 2) by ring]
      simp only [cos743o2c]
    rw [hcong]
    ring
  have hbdy : ‖(((500 : ℕ) : ℝ) : ℂ) ^ (1 - s) / (s - 1)‖
    ...
    have hden : (743/2 : ℝ) ≤ ‖s - 1‖ := by
      have habsim := Complex.abs_im_le_norm (s - 1)
      have him : (s - 1).im = ((743:ℕ):ℝ)/2 := by simp [hsim]
      rw [him] at habsim
      calc (743/2 : ℝ) = |((743:ℕ):ℝ)/2| := by norm_num
        _ ≤ ‖s - 1‖ := habsim
  ...
  have hEMre : (1 + ∑ n ∈ Finset.Icc (2:ℕ) 500, ((n : ℝ) ^ (-σ) * cos743o2c n))
      - (500 : ℝ) ^ (-σ) * (1000/743) - (500 : ℝ) ^ (-σ) * (1/2)
      ≤ EMval.re := by
    rw [hEMdef]
...
section AxiomAudit
#print axioms CriticalLinePhasor.DVP.re_zeta743o2_lower
end AxiomAudit
