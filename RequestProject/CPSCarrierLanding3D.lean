...
`htrans` is the arithmetic input: the all-directions winding invariance, i.e. the
Cogdell--Piatetski-Shapiro basic identity / the niceness of the full twist family.

## `htrans` closure by rank
...
carrier-native.  This is realized non-vacuously on an automorphic object in
...
**The tower (`r ≥ 2`): the closure is an assembly of in-tree proofs.** For
...
of the bank.  The reflection is angle-agnostic: carrier invariance is universal across
admissible banks --- the finite model of
`CPSLatticeInstance3D` proves its own invariance by pure reindexing --- and the classical
discrimination between automorphic and non-automorphic angle data lives in the downstream
identification of the carrier object with the classical completed `L` (the identification
layer: Newton--Thorne for the holomorphic tower and the adelic Poisson of Tate's thesis,
cited).  What remains for `htrans` on the twisted symmetric-power bank is assembly ---
constructing the bank's theta profile pair and discharging the Mellin-package hypotheses --- plus
the identification layer.  No automorphic-realization input enters the carrier landing.
