...
/-!
# The half-integer station core (rung-126 route-around)

Close zero pairs inside one integer cell need stations at half-integer
heights.  The station at `t = M/2` reuses the ℕ-multiplier evaluators
against HALVED log brackets (`cos(M·(log n / 2) − ψ)` — exact rational
...
/-- **The HALF-INTEGER phase-station lower bound** (height `M/2`).  All costs in closed form; the
head is the shifted-cosine sum the tables certify. -/
theorem phase_station_lower_half (M N : ℕ) (hM : 1 ≤ M) (hN : 1 ≤ N) (ψ : ℝ) :
    (∑ i ∈ Finset.range N,
        (((i+1 : ℕ)) : ℝ) ^ (-(1/2 : ℝ))
          * Real.cos ((M:ℝ) * (Real.log ((i+1 : ℕ)) / 2) - ψ))
      - (N:ℝ) ^ ((1/2 : ℝ)) / ((M:ℝ)/2) - (N:ℝ) ^ (-(1/2 : ℝ)) / 2
      - ((M:ℝ)/2+1) * ((M:ℝ)/2+2) / 8
  ...
  have hbdy : ‖(((N : ℕ) : ℝ) : ℂ) ^ (1 - s) / (s - 1)‖
    ...
    have hre1s : (1 - s).re = 1/2 := by
      simp [hsre]
      norm_num
    have hden : (M:ℝ)/2 ≤ ‖s - 1‖ := by
      have habsim := Complex.abs_im_le_norm (s - 1)
      have him : (s - 1).im = (M:ℝ)/2 := by simp [hsim]
      rw [him] at habsim
      calc (M:ℝ)/2 = |(M:ℝ)/2| := (abs_of_pos (by positivity)).symm
        _ ≤ ‖s - 1‖ := habsim
    rw [hb1, hre1s]
...
section AxiomAudit
#print axioms CriticalLinePhasor.DVP.phase_station_lower_half
end AxiomAudit
