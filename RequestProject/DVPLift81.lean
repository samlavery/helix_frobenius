import RequestProject.DVPSegment81
import RequestProject.DVPLiftPrincipal
import RequestProject.DVPTheta58

/-!
# The census at height 81: `N_all(81) = 21` (rung-81)

The segment certificate `segment81_re_pos` keeps `ζ` in the right
...
theorem re_pos_at_81 {z : ℂ} (hre1 : 1/2 ≤ z.re) (hre2 : z.re ≤ 2)
    (him : z.im = 81) : (1/5 : ℝ) ≤ (riemannZeta z).re := by
  have h := (Complex.re_add_im z).symm
  rw [him] at h
  have hz : z = ↑z.re + ((81:ℕ):ℝ) * Complex.I := by
    rw [h]
    norm_num
  rw [hz]
  exact segment81_re_pos hre1 hre2
...
theorem goodHeight_81 : GoodHeight 81 := by
  refine ⟨by norm_num, ?_⟩
  intro ρ hρ him
  obtain ⟨h0, h1, hz⟩ := hρ
  by_cases hge : (1/2 : ℝ) ≤ ρ.re
  · have hre := re_pos_at_81 hge (by linarith) him
    rw [hz] at hre
    simp at hre
    linarith
  · have hlt : ρ.re < 1/2 := lt_of_not_ge hge
    ...
    have hwre : ((starRingEnd ℂ) (1 - ρ)).re = 1 - ρ.re := by
      simp
    have hwim : ((starRingEnd ℂ) (1 - ρ)).im = 81 := by
      simp [him]
    have hre := re_pos_at_81 (z := (starRingEnd ℂ) (1 - ρ))
...
theorem standardContour81_facts (u : unitInterval) :
    riemannZeta (standardContour 81 u) ≠ 0
      ∧ standardContour 81 u ≠ 1
      ∧ riemannZeta (standardContour 81 u) ∈ Complex.slitPlane := by
  by_cases hu : (u:ℝ) ≤ 1/2
  · have hre : (standardContour 81 u).re = 2 :=
  ...
  · have hu' : (1:ℝ)/2 < (u:ℝ) := lt_of_not_ge hu
    have hcoords := standardContour_top_coords (T := 81) (u := u) hu
    have hu1 : (u:ℝ) ≤ 1 := u.2.2
    have hre1 : 1/2 ≤ (standardContour 81 u).re := by
      rw [hcoords.1]
      linarith
    have hre2 : (standardContour 81 u).re ≤ 2 := by
      rw [hcoords.1]
      linarith
    have hpos := re_pos_at_81 hre1 hre2 hcoords.2
    refine ⟨?_, ?_, ?_⟩
    · intro h
      rw [h] at hpos
      simp at hpos
...
theorem contourLogLift81_eq_log (hT : GoodHeight 81) (u : unitInterval) :
    contourLogLift hT u
      = Complex.log (riemannZeta (standardContour 81 u)) := by
  have hs_pre : IsPreconnected (Set.univ : Set unitInterval) :=
    isPreconnected_univ
  have hg₂cont : ContinuousOn
      (fun v : unitInterval =>
        Complex.log (riemannZeta (standardContour 81 v)))
      (Set.univ : Set unitInterval) := by
    intro v _
    have hc : ContinuousAt
        (Complex.log ∘ riemannZeta
          ∘ (standardContour 81 : unitInterval → ℂ)) v := by
      apply ContinuousAt.comp
      · exact continuousAt_clog (standardContour81_facts v).2.2
      · apply ContinuousAt.comp
        · exact (differentiableAt_riemannZeta
            (standardContour81_facts v).2.1).continuousAt
        · exact (standardContour 81).continuous.continuousAt
  ...
  have heq := Complex.isCoveringMap_exp.eqOn_of_comp_eqOn hs_pre
    ...
    (by
      ...
      show Complex.exp (contourLogLift hT v)
        = Complex.exp (Complex.log (riemannZeta (standardContour 81 v)))
      rw [exp_contourLogLift,
        Complex.exp_log (standardContour81_facts v).1])
    (a := 0) (Set.mem_univ 0)
    (by
      rw [contourLogLift_zero]
      have h0 : standardContour 81 0 = 2 := (standardContour 81).source
      rw [h0]
      rw [riemannZeta_two]
...
theorem abs_classicalSContour_81_lt_half (hT : GoodHeight 81) :
    |classicalSContour hT| < 1/2 := by
  have hend := contourLogLift81_eq_log hT 1
  have htarget : standardContour 81 1 = line 81 := (standardContour 81).target
  rw [htarget] at hend
  have hline_re : (line (81:ℝ)).re = 1/2 := by
    simp [line, lineC]
  have hline_im : (line (81:ℝ)).im = 81 := by
    simp [line, lineC]
  have hre := re_pos_at_81 (z := line 81) (by rw [hline_re])
    (by rw [hline_re]; norm_num) hline_im
  have hzne : riemannZeta (line 81) ≠ 0 := by
    intro h
    rw [h] at hre
    simp at hre
    linarith
  have harg : |Complex.arg (riemannZeta (line 81))| < Real.pi / 2 := by
...
theorem stripZeroCountMult_81 : stripZeroCountMult 81 = 21 := by
  have hT := goodHeight_81
  have hS := abs_classicalSContour_81_lt_half hT
  have hcensus := classicalSContour_eq_Smult_add_offLine hT
  have hsplit := stripZeroCountMult_eq_line_add_offLine 81
  have hsplitR : (stripZeroCountMult 81 : ℝ)
      = (zeroEventCountMult 81 : ℝ) + (offLineStripZeroCountMult 81 : ℝ) := by
    exact_mod_cast hsplit
  have hkey : classicalSContour hT
      = (stripZeroCountMult 81 : ℝ) - theta 81 / Real.pi - 1 := by
    rw [hcensus]
  ...
  have hinc := theta_increment_enclosure_from (T₁ := (30:ℝ)) (T := (81:ℝ))
    (by norm_num) (by norm_num)
  have hMain63 : thetaMain (81:ℝ)
      = ((81:ℝ) * (Real.log (81:ℝ) - Real.log 2 - Real.log Real.pi)
          - (81:ℝ)) / 2 := by
  ...
  have hl81 : ((43944491/10000000 : ℚ) : ℝ) ≤ Real.log (81:ℝ)
      ∧ Real.log (81:ℝ) ≤ ((10986123/2500000 : ℚ) : ℝ) := by
    have h := log_br_81
    norm_num at h ⊢
    exact h
  ...
  push_cast at hincb hl81 hl30 hl2 hlπ
  -- multiply the census through by π
  have hπN : Real.pi * (stripZeroCountMult 81 : ℝ)
      = theta 81 + Real.pi + Real.pi * classicalSContour hT := by
    have hdiv : theta 81 / Real.pi * Real.pi = theta 81 :=
      div_mul_cancel₀ _ hπ0.ne'
    have h2 : classicalSContour hT * Real.pi
        = ((stripZeroCountMult 81 : ℝ) - theta 81 / Real.pi - 1) * Real.pi := by
      rw [hkey]
    rw [sub_mul, sub_mul, hdiv, one_mul] at h2
    linarith [h2]
  have hSb := abs_lt.mp hS
  -- 13 < N < 15 over ℝ
  have hlow : (20:ℝ) < (stripZeroCountMult 81 : ℝ) := by
    nlinarith [hπN, hθ30b.1, hθ30b.2, hincb.1, hincb.2, hSb.1,
      hl81.1, hl81.2, hl30.1, hl30.2, hl2.1, hl2.2, hlπ.1, hlπ.2,
      hπl, hπu, hπ0]
  have hhigh : (stripZeroCountMult 81 : ℝ) < 22 := by
    nlinarith [hπN, hθ30b.1, hθ30b.2, hincb.1, hincb.2, hSb.2,
      hl81.1, hl81.2, hl30.1, hl30.2, hl2.1, hl2.2, hlπ.1, hlπ.2,
      hπl, hπu, hπ0]
  have hlowN : 20 < stripZeroCountMult 81 := by exact_mod_cast hlow
  have hhighN : stripZeroCountMult 81 < 22 := by exact_mod_cast hhigh
  omega

end CriticalLinePhasor.DVP

section AxiomAudit
#print axioms CriticalLinePhasor.DVP.goodHeight_81
#print axioms CriticalLinePhasor.DVP.contourLogLift81_eq_log
#print axioms CriticalLinePhasor.DVP.abs_classicalSContour_81_lt_half
#print axioms CriticalLinePhasor.DVP.stripZeroCountMult_81
end AxiomAudit
