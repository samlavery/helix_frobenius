import RequestProject.DVPSegment525o2
import RequestProject.DVPLog200Table
import RequestProject.DVPLiftPrincipal
import RequestProject.DVPTheta58

/-!
# The census at the half-integer height 525/2: `N_all(262.5) = 115` (rung-262.5)

The segment certificate `segment525o2_re_pos` keeps `ζ` in the right
...
theorem re_pos_at_525o2 {z : ℂ} (hre1 : 1/2 ≤ z.re) (hre2 : z.re ≤ 2)
    (him : z.im = 525/2) : (1/5 : ℝ) ≤ (riemannZeta z).re := by
  have h := (Complex.re_add_im z).symm
  rw [him] at h
  have hz : z = ↑z.re + ↑((((525:ℕ)):ℝ)/2) * Complex.I := by
    conv_lhs => rw [h]
    push_cast
    ring
  rw [hz]
  exact segment525o2_re_pos hre1 hre2
...
theorem goodHeight_525o2 : GoodHeight (525/2) := by
  refine ⟨by norm_num, ?_⟩
  intro ρ hρ him
  obtain ⟨h0, h1, hz⟩ := hρ
  by_cases hge : (1/2 : ℝ) ≤ ρ.re
  · have hre := re_pos_at_525o2 hge (by linarith) him
    rw [hz] at hre
    simp at hre
    linarith
  · have hlt : ρ.re < 1/2 := lt_of_not_ge hge
    ...
    have hwre : ((starRingEnd ℂ) (1 - ρ)).re = 1 - ρ.re := by
      simp
    have hwim : ((starRingEnd ℂ) (1 - ρ)).im = 525/2 := by
      simp [him]
    have hre := re_pos_at_525o2 (z := (starRingEnd ℂ) (1 - ρ))
...
theorem standardContour525o2_facts (u : unitInterval) :
    riemannZeta (standardContour (525/2) u) ≠ 0
      ∧ standardContour (525/2) u ≠ 1
      ∧ riemannZeta (standardContour (525/2) u) ∈ Complex.slitPlane := by
  by_cases hu : (u:ℝ) ≤ 1/2
  · have hre : (standardContour (525/2) u).re = 2 :=
  ...
  · have hu' : (1:ℝ)/2 < (u:ℝ) := lt_of_not_ge hu
    have hcoords := standardContour_top_coords (T := 525/2) (u := u) hu
    have hu1 : (u:ℝ) ≤ 1 := u.2.2
    have hre1 : 1/2 ≤ (standardContour (525/2) u).re := by
      rw [hcoords.1]
      linarith
    have hre2 : (standardContour (525/2) u).re ≤ 2 := by
      rw [hcoords.1]
      linarith
    have hpos := re_pos_at_525o2 hre1 hre2 hcoords.2
    refine ⟨?_, ?_, ?_⟩
    · intro h
      rw [h] at hpos
      simp at hpos
...
theorem contourLogLift525o2_eq_log (hT : GoodHeight (525/2)) (u : unitInterval) :
    contourLogLift hT u
      = Complex.log (riemannZeta (standardContour (525/2) u)) := by
  have hs_pre : IsPreconnected (Set.univ : Set unitInterval) :=
    isPreconnected_univ
  have hg₂cont : ContinuousOn
      (fun v : unitInterval =>
        Complex.log (riemannZeta (standardContour (525/2) v)))
      (Set.univ : Set unitInterval) := by
    intro v _
    have hc : ContinuousAt
        (Complex.log ∘ riemannZeta
          ∘ (standardContour (525/2) : unitInterval → ℂ)) v := by
      apply ContinuousAt.comp
      · exact continuousAt_clog (standardContour525o2_facts v).2.2
      · apply ContinuousAt.comp
        · exact (differentiableAt_riemannZeta
            (standardContour525o2_facts v).2.1).continuousAt
        · exact (standardContour (525/2)).continuous.continuousAt
  ...
  have heq := Complex.isCoveringMap_exp.eqOn_of_comp_eqOn hs_pre
    ...
    (by
      ...
      show Complex.exp (contourLogLift hT v)
        = Complex.exp (Complex.log (riemannZeta (standardContour (525/2) v)))
      rw [exp_contourLogLift,
        Complex.exp_log (standardContour525o2_facts v).1])
    (a := 0) (Set.mem_univ 0)
    (by
      rw [contourLogLift_zero]
      have h0 : standardContour (525/2) 0 = 2 := (standardContour (525/2)).source
      rw [h0]
      rw [riemannZeta_two]
...
theorem abs_classicalSContour_525o2_lt_half (hT : GoodHeight (525/2)) :
    |classicalSContour hT| < 1/2 := by
  have hend := contourLogLift525o2_eq_log hT 1
  have htarget : standardContour (525/2) 1 = line (525/2) := (standardContour (525/2)).target
  rw [htarget] at hend
  have hline_re : (line ((525:ℝ)/2)).re = 1/2 := by
    simp [line, lineC]
  have hline_im : (line ((525:ℝ)/2)).im = 525/2 := by
    simp [line, lineC]
  have hre := re_pos_at_525o2 (z := line (525/2)) (by rw [hline_re])
    (by rw [hline_re]; norm_num) hline_im
  have hzne : riemannZeta (line (525/2)) ≠ 0 := by
    intro h
    rw [h] at hre
    simp at hre
    linarith
  have harg : |Complex.arg (riemannZeta (line (525/2)))| < Real.pi / 2 := by
...
theorem stripZeroCountMult_525o2 : stripZeroCountMult (525/2) = 115 := by
  have hT := goodHeight_525o2
  have hS := abs_classicalSContour_525o2_lt_half hT
  have hcensus := classicalSContour_eq_Smult_add_offLine hT
  have hsplit := stripZeroCountMult_eq_line_add_offLine (525/2)
  have hsplitR : (stripZeroCountMult (525/2) : ℝ)
      = (zeroEventCountMult (525/2) : ℝ) + (offLineStripZeroCountMult (525/2) : ℝ) := by
    exact_mod_cast hsplit
  have hkey : classicalSContour hT
      = (stripZeroCountMult (525/2) : ℝ) - theta (525/2) / Real.pi - 1 := by
    rw [hcensus]
  ...
  have hinc := theta_increment_enclosure_from (T₁ := (30:ℝ)) (T := ((525:ℝ)/2))
    (by norm_num) (by norm_num)
  have hMain63 : thetaMain ((525:ℝ)/2)
      = (((525:ℝ)/2) * (Real.log ((525:ℝ)/2) - Real.log 2 - Real.log Real.pi)
          - ((525:ℝ)/2)) / 2 := by
  ...
  have hsplit525 : Real.log ((525:ℝ)/2)
      = Real.log (3:ℝ) + 2 * Real.log (5:ℝ) + Real.log (7:ℝ) - Real.log (2:ℝ) := by
    rw [show (525:ℝ)/2 = (3:ℝ) * 5^2 * 7 / 2 by norm_num,
    ...
    push_cast
    ring
  have hl126 : ((55702509/10000000 : ℚ) : ℝ) ≤ Real.log ((525:ℝ)/2)
      ∧ Real.log ((525:ℝ)/2) ≤ ((27851257/5000000 : ℚ) : ℝ) := by
    have h3 := log_br_3
    have h5 := log_br_5
    have h7 := log_br_7
    have h2 := log_br_2
    rw [hsplit525]
    push_cast at h3 h5 h7 h2 ⊢
    constructor <;> linarith [h3.1, h3.2, h5.1, h5.2, h7.1, h7.2, h2.1, h2.2]
  ...
  have hπN : Real.pi * (stripZeroCountMult (525/2) : ℝ)
      = theta (525/2) + Real.pi + Real.pi * classicalSContour hT := by
    have hdiv : theta (525/2) / Real.pi * Real.pi = theta (525/2) :=
      div_mul_cancel₀ _ hπ0.ne'
    have h2 : classicalSContour hT * Real.pi
        = ((stripZeroCountMult (525/2) : ℝ) - theta (525/2) / Real.pi - 1) * Real.pi := by
      rw [hkey]
    rw [sub_mul, sub_mul, hdiv, one_mul] at h2
    linarith [h2]
  have hSb := abs_lt.mp hS
  -- 49 < N < 116 over ℝ
  have hlow : (114:ℝ) < (stripZeroCountMult (525/2) : ℝ) := by
    nlinarith [hπN, hθ30b.1, hθ30b.2, hincb.1, hincb.2, hSb.1,
      hl126.1, hl126.2, hl30.1, hl30.2, hl2.1, hl2.2, hlπ.1, hlπ.2,
      hπl, hπu, hπ0]
  have hhigh : (stripZeroCountMult (525/2) : ℝ) < 116 := by
    nlinarith [hπN, hθ30b.1, hθ30b.2, hincb.1, hincb.2, hSb.2,
      hl126.1, hl126.2, hl30.1, hl30.2, hl2.1, hl2.2, hlπ.1, hlπ.2,
      hπl, hπu, hπ0]
  have hlowN : 114 < stripZeroCountMult (525/2) := by exact_mod_cast hlow
  have hhighN : stripZeroCountMult (525/2) < 116 := by exact_mod_cast hhigh
  omega

end CriticalLinePhasor.DVP

section AxiomAudit
#print axioms CriticalLinePhasor.DVP.goodHeight_525o2
#print axioms CriticalLinePhasor.DVP.contourLogLift525o2_eq_log
#print axioms CriticalLinePhasor.DVP.abs_classicalSContour_525o2_lt_half
#print axioms CriticalLinePhasor.DVP.stripZeroCountMult_525o2
end AxiomAudit
