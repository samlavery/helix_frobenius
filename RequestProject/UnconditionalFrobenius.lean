...
real — a symmetric operator has real eigenvalues.  The zeros we actually derive are on the critical
line **by construction** — and that line is not assumed: the fiber is ridden on `σ = ½`, where `½` is
itself **derived from the geometry**, the unique scale-critical exponent at which the phasor amplitude
...
theorem inner_smul_left (f g : ι →₀ ℂ) (r : ℂ) :
  ...
  have h1 : (inner ℂ (r • f) g : ℂ)
      = ∑ i ∈ f.support, (starRingEnd ℂ) ((r • f) i) * g i :=
    inner_eq_sum_of_subset (r • f) g Finsupp.support_smul
  have h2 : (inner ℂ f g : ℂ) = ∑ i ∈ f.support, (starRingEnd ℂ) (f i) * g i :=
    inner_eq_sum_of_subset f g subset_rfl
  rw [h1, h2, Finset.mul_sum]
  refine Finset.sum_congr rfl fun i _ => ?_
  rw [Finsupp.smul_apply, smul_eq_mul, map_mul]
  ring
...
theorem spectralWave_hasDerivAt (γ : ℝ) (t : ℝ) :
  ...
  have h : HasDerivAt (fun y : ℝ => (γ : ℂ) * (y : ℂ) * Complex.I) ((γ : ℂ) * Complex.I) t := by
    have h0 : HasDerivAt (fun y : ℝ => (γ : ℂ) * (y : ℂ)) (γ : ℂ) t := by
      simpa using (HasDerivAt.ofReal_comp (hasDerivAt_id t)).const_mul (γ : ℂ)
    simpa using h0.mul_const Complex.I
  have hc := (Complex.hasDerivAt_exp ((γ : ℂ) * (t : ℂ) * Complex.I)).comp t h
  have heq : (γ : ℂ) * Complex.I * spectralWave γ t
      = Complex.exp ((γ : ℂ) * (t : ℂ) * Complex.I) * ((γ : ℂ) * Complex.I) := by
    unfold spectralWave; ring
  rw [heq]
  exact hc
