...
/-!
# The weighted-tail primitive identity

Abstract Fubini-plus-Taylor identity used to move a `∫ g·W` pairing onto the second
derivative of `g` against the *tail primitive* of the weight `W`:

  `∫_{u>0} g u · W u  =  ∫_{r>0} g''(r) · (∫_{u>r} (u − r) · W u)`.

The point of the abstract form is that it carries no `Ξ`, no zeros, and no spectral
parameter — it is pure real analysis, and it is stated so that integration by parts
with boundary terms at infinity is never needed.

## Structure

* `taylor2_zero` — Taylor with integral remainder at order 2, base point 0, for
  `g` with `g 0 = 0` and `g' 0 = 0`:  `g u = ∫_0^u (u − r)·g''(r) dr`.
  Proved by building `G v = v·A v − B v` from the two primitives `A, B` of `g''`,
  showing `deriv G = deriv g`, and applying `integral_deriv_eq_sub` to **both**
  `G` and `g`.  No `taylorWithinEval`, no `iteratedDerivWithin`, no constancy lemma.
* `triF` — the triangle integrand, written with an `if` so it vanishes off
  `{0 < r < u}` and the ambient product measure is the plain restricted one.
* `inner_u`, `inner_r` — the two inner integrals, each an indicator computation.
* `weighted_tail_primitive_identity` — the swap, from
  `MeasureTheory.integral_integral_swap`.

The product-integrability hypothesis `hprod` is the only side condition; it is
supplied by the caller, which is the correct granularity for an abstract lemma.

Compiler-audited footprint: `{propext, Classical.choice, Quot.sound}`.
-/
...
/-- **Taylor with integral remainder, order 2, base 0.**  For `g` of class `C²` with
`g 0 = 0` and `g′ 0 = 0`, `g u = ∫_0^u (u − r)·g″(r) dr` for every real `u`. -/
...
/-- The triangle integrand: `(u − r)·g″(r)·W u` on `{r < u}`, zero elsewhere. -/
def triF (W g : ℝ → ℝ) (u r : ℝ) : ℝ :=
...
section AxiomAudit
#print axioms CriticalLinePhasor.WeightedTail.taylor2_zero
#print axioms CriticalLinePhasor.WeightedTail.inner_u
#print axioms CriticalLinePhasor.WeightedTail.inner_r
#print axioms CriticalLinePhasor.WeightedTail.weighted_tail_primitive_identity
end AxiomAudit
