...
at commit 21998bb6196b56789f72a52656a781a75e134eb0, file PrimeNumberTheoremAnd/Consequences.lean.
Copyright the PrimeNumberTheoremAnd contributors; Apache License 2.0
(http://www.apache.org/licenses/LICENSE-2.0).

WHY VENDORED.  `RequestProject.CPSMaassTauberianTransfer3D` consumes `nth_prime_asymp`,
`log_nth_prime_asymp`, `pi_nth_prime`, `tendsto_nth_prime_atTop` and `isLittleO_sqrt_mul_log`
from this file, so it cannot simply be dropped.  It imports `PrimeNumberTheoremAnd.Defs` and
`PrimeNumberTheoremAnd.Wiener`, both of which sit downstream of the `Fourier` breakage under
Lean v4.33.0-rc2 (see `RequestProject/Vendored/Fourier.lean` for the root cause).  Lean resolves
imports by MODULE NAME, so the chain moves together.

LOCAL MODIFICATIONS — exactly three, and nothing else:
  1. this header;
  2. `import PrimeNumberTheoremAnd.Defs` → `import RequestProject.Vendored.Defs`;
  3. `import PrimeNumberTheoremAnd.Wiener` → `import RequestProject.Vendored.Wiener`.
No proof, statement, or definition in this file is altered.  The `Mathlib.*` shim import is left
pointing upstream: it does not depend on `Fourier` and builds.
...
import RequestProject.Vendored.Defs
