/-
VENDORED from https://github.com/AlexKontorovich/PrimeNumberTheoremAnd
at commit 21998bb6196b56789f72a52656a781a75e134eb0, file PrimeNumberTheoremAnd/Wiener.lean.
Copyright the PrimeNumberTheoremAnd contributors; Apache License 2.0
(http://www.apache.org/licenses/LICENSE-2.0).

WHY VENDORED.  This file carries `WienerIkeharaTheorem'` and `WienerIkeharaTheorem''`, the
compiled Tauberian input under the CPS → Sato–Tate program (`CPSWienerIkeharaBridge3D` is the
only thing that inhabits `CPSMaassSatoTate3D.CPSWienerIkeharaOutput3D`; without it every
Sato–Tate theorem downstream stays compiled but becomes conditional on an uninhabited structure).
It imports `PrimeNumberTheoremAnd.Fourier`, which does not build under Lean v4.33.0-rc2 (see
`RequestProject/Vendored/Fourier.lean` for the root cause).  Lean resolves imports by MODULE NAME,
so a vendored `Fourier` alone cannot help — upstream's copy of this file would still import the
broken one.  Hence the chain moves together.

LOCAL MODIFICATIONS — exactly five, and nothing else:
  1. this header;
  2. `import PrimeNumberTheoremAnd.Fourier` → `import RequestProject.Vendored.Fourier`;
  3. **REMOVAL of four `sorry`-carrying declarations** (see the marked block below): the
     declarations at upstream lines 328-401 — `prelim_decay_2` and `prelim_decay_3` (both proved
     `by sorry` upstream), the def `AbsolutelyContinuous` used only by them, and `decay_alt`
     (proved, but consuming `prelim_decay_3` and so transitively carrying `sorryAx`).  Upstream
     PrimeNumberTheoremAnd is a work-in-progress formalization and legitimately contains `sorry`;
     this repository's hard rule is that none may enter our tree.  Deleting rather than importing
     follows Zeta23's own documented practice for this codebase.  **These are NOT on the
     dependency path of anything consumed here** — verified with a positive control, see the
     marked block and the header note below;
  4. `second_fourier_integrable_aux1` — `Equiv.coe_fn_mk` no longer reduces the raw `Equiv.mk`
     left by unfolding `Multiplicative.ofAdd`, so `fun_prop` found no theorems for it; the goal is
     restated with the coercion applied (`show`, defeq) and `fun_prop` succeeds as before;
  5. an interval-integral step — `simp [ae, this]` no longer discharged the filter membership;
     replaced by `mem_ae_iff` + `Measure.restrict_apply' measurableSet_Icc` (the restriction is to
     `Icc`, so the complement of `Ioc` inside it is the null set `{x₀}`).
Items 4 and 5 are ordinary v4.33.0-rc2 port drift, invisible upstream because `Fourier` failed
first and masked everything downstream.  No statement is weakened; no proof is otherwise altered.
The `Mathlib.*` shim and `SmoothExistence` imports are left pointing upstream: they do not depend
on `Fourier` and build.

AXIOM GATE (re-run after every change to this file): `#print axioms WienerIkeharaTheorem''` must
report `[propext, Classical.choice, Quot.sound]`.  Run it with a positive control — before their
deletion, `#print axioms prelim_decay_2` correctly reported `sorryAx`, which is what makes the
clean footprint on `WienerIkeharaTheorem''` a genuine negative rather than a blind spot.

*** REMOVAL TRIGGER ***
Delete this vendored copy and restore the ordinary `PrimeNumberTheoremAnd` dependency as soon as
upstream publishes a release compatible with Lean v4.33.0-rc2 or later.  The only local
modifications are the two `Circle.norm_coe` fixes in `Vendored/Fourier.lean` plus import
repointing; there is no other divergence to preserve.
-/
import Architect
...
import RequestProject.Vendored.Fourier
...
lemma second_fourier_integrable_aux1 (hcont : Measurable ψ) (hsupp : Integrable ψ) (hσ : 1 < σ') :
  ...
  · apply Measurable.aestronglyMeasurable
    ...
    -- LOCAL FIX (v4.33.0-rc2): `Equiv.coe_fn_mk` no longer reduces the raw `Equiv.mk` left by
    -- unfolding `Multiplicative.ofAdd`, so `fun_prop` finds no theorems for it.  Restate the
    -- goal with the coercion applied (defeq) and `fun_prop` succeeds as before.
    show Measurable (Function.uncurry fun (u a : ℝ) =>
      cexp (-(↑u * (↑σ' - 1))) * ((𝐞 (-(a * (u / (2 * π))))) * ψ a))
    fun_prop
...
/- *** REMOVED IN VENDORING — UPSTREAM `sorry` ***
Four declarations were deleted here, at upstream Wiener.lean lines 328-401:
  `prelim_decay_2`      (upstream line 328) — proved `by sorry` upstream
  `AbsolutelyContinuous`(upstream line 332) — a def used only by the two below
  `prelim_decay_3`      (upstream line 347) — proved `by sorry` upstream
  `decay_alt`           (upstream line 363) — proved, but consumes `prelim_decay_3`,
                                              hence transitively carries `sorryAx`
This repository's hard rule is no `sorry` anywhere, and upstream PrimeNumberTheoremAnd is a
work-in-progress formalization that legitimately contains some.  Deleting rather than importing
them follows Zeta23's own practice for this codebase (their `FromPNTPlus` headers document
truncating past unproved regions so that no `sorry` enters).

They are NOT on the dependency path of anything consumed here — verified, not assumed:
`#print axioms WienerIkeharaTheorem''` reports `[propext, Classical.choice, Quot.sound]`, and the
check was run with a POSITIVE CONTROL (`#print axioms prelim_decay_2` correctly reported
`sorryAx` before deletion), so the clean footprint is a genuine negative and not a blind spot.
`decay_alt` had no dependents anywhere in `RequestProject/`.  Nothing else was removed. -/
...
theorem sum_le_integral {x₀ : ℝ} {f : ℝ → ℝ} {n : ℕ} (hf : AntitoneOn f (Ioc x₀ (x₀ + n)))
  ...
  have l1 : f (x₀ + 1) ≤ ∫ x in x₀..x₀ + 1, f x := by
    ...
    -- LOCAL FIX (v4.33.0-rc2): the old `simp [ae, this]` no longer discharges the filter
    -- membership.  The restriction is to `Icc`, so the complement of `Ioc` inside it is the
    -- single point `{x₀}`; `mem_ae_iff` reduces to that null set explicitly.
    refine eventually_of_mem ?_ l5
    rw [mem_ae_iff, Measure.restrict_apply' measurableSet_Icc]
