...
at commit 21998bb6196b56789f72a52656a781a75e134eb0, file PrimeNumberTheoremAnd/Fourier.lean.
Copyright the PrimeNumberTheoremAnd contributors; Apache License 2.0
(http://www.apache.org/licenses/LICENSE-2.0).

WHY VENDORED.  Upstream is pinned to Lean v4.32.2 and does not build under our v4.33.0-rc2 /
Mathlib 51e6992efd06.  `Circle.norm_coe` (Mathlib/Analysis/Complex/Circle.lean) lost its `@[simp]`
attribute in the bump, so the two `simp` calls below no longer close their goals.  We cannot patch
`.lake/packages/**`, and a vendored copy at a *different module path* cannot help unless the whole
Fourier-dependent chain moves with it: Lean resolves imports by MODULE NAME, so upstream's own
`Wiener.lean` would still import the broken `PrimeNumberTheoremAnd.Fourier`.  Hence this file plus
`Defs`, `Wiener`, `Consequences` are vendored together.  Modules in the closure that do NOT depend
on `Fourier` (`Sobolev`, `SmoothExistence`, the `Mathlib.*` shims) are still imported from upstream.

LOCAL MODIFICATIONS — exactly three, and nothing else:
  1. this header;
  2. line ~16, `nnnorm_eq_of_mem_circle`: `by simp` → `by simp [Circle.norm_coe]`;
  3. line ~25, `e.map_bounded'`: `by simp [one_add_one_eq_two]`
     → `by simp [Circle.norm_coe, one_add_one_eq_two]`.
Fixes 2 and 3 are taken VERBATIM from Zeta23/FromPNTPlus/Fourier.lean (anthropics/zeta-23-lean,
commit 3635e74826a4c1fcece7d1cd2b6fa75e43a00510, Apache 2.0), which re-ported this same upstream
file to v4.33.0-rc2 and whose copy is compiled on this toolchain.  Not our own invention.
...
modifications are the two `Circle.norm_coe` fixes; there is no other divergence to preserve.
As of 2026-08-11 upstream `master` and `main` are both still on v4.32.2 with these lines unfixed.
-/
