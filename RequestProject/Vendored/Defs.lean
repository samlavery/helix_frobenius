...
at commit 21998bb6196b56789f72a52656a781a75e134eb0, file PrimeNumberTheoremAnd/Defs.lean.
Copyright the PrimeNumberTheoremAnd contributors; Apache License 2.0
(http://www.apache.org/licenses/LICENSE-2.0).

WHY VENDORED.  This file imports `PrimeNumberTheoremAnd.Fourier`, which does not build under
Lean v4.33.0-rc2 (see `RequestProject/Vendored/Fourier.lean` for the root cause: `Circle.norm_coe`
lost `@[simp]`).  Lean resolves imports by MODULE NAME, so the whole Fourier-dependent chain must
move together; this file is carried only for that reason.

LOCAL MODIFICATIONS — exactly two, and nothing else:
  1. this header;
  2. `import PrimeNumberTheoremAnd.Fourier` → `import RequestProject.Vendored.Fourier`.
No proof, statement, or definition in this file is altered.
