import RequestProject.DVPCos618o2Psum
import RequestProject.DVPSegment126Core

/-!
# The segment core at `t = 618/2` (rung-309; N = 500)

The single Euler–Maclaurin extraction all seven σ-intervals consume:

`Re ζ(σ+(618/2)i) ≥ 1 + ∑_{n=2}^{500} n^{−σ}·cos(618·(log n)/2)
  − 500^{−σ}·(1000/618 + 1/2) − 12130·(500^{−σ−2} + 500^{−σ−1}/(σ+1))`
...
theorem re_zeta618o2_lower {σ : ℝ} (h1 : 1/2 ≤ σ) (h2 : σ ≤ 2) :
    1 + (∑ n ∈ Finset.Icc (2:ℕ) 500, ((n : ℝ) ^ (-σ) * cos618o2c n))
      - (500 : ℝ) ^ (-σ) * (1000/618 + 1/2)
      - 12130 * ((500 : ℝ) ^ (-σ - 2) + (500 : ℝ) ^ (-σ - 1) / (σ + 1))
    ≤ (riemannZeta (↑σ + ↑((((618:ℕ)):ℝ)/2) * Complex.I)).re := by
  set s : ℂ := ↑σ + ↑((((618:ℕ)):ℝ)/2) * Complex.I with hsdef
  have hsre : s.re = σ := by rw [hsdef]; simp
  have hsim : s.im = ((618:ℕ):ℝ)/2 := by rw [hsdef]; simp
  ...
  have hre_ge : EMval.re - ‖riemannZeta s - EMval‖ ≤ (riemannZeta s).re := by
    ...
    have := abs_le.mp habs
    linarith [this.1]
  have hprod : ‖s‖ * ‖s + 1‖ ≤ 388128/4 := by
    have hns : ‖s‖ ≤ 622/2 := by
      calc ‖s‖ ≤ ‖(↑σ : ℂ)‖ + ‖↑((((618:ℕ)):ℝ)/2) * Complex.I‖ := by
        ...
        _ ≤ 622/2 := by
            ...
            linarith
    have hns1 : ‖s + 1‖ ≤ 624/2 := by
      calc ‖s + 1‖ ≤ ‖s‖ + ‖(1 : ℂ)‖ := norm_add_le _ _
        _ ≤ 624/2 := by rw [norm_one]; linarith
    calc ‖s‖ * ‖s + 1‖ ≤ (622/2) * (624/2) :=
          mul_le_mul hns hns1 (norm_nonneg _) (by norm_num)
      _ = 388128/4 := by norm_num
  have herr : ‖s‖ * ‖s + 1‖ * (1/8)
      * ((((500 : ℕ) : ℝ)) ^ (-s.re - 2)
        + (((500 : ℕ) : ℝ)) ^ (-s.re - 1) / (s.re + 1))
      ≤ 12130 * ((500 : ℝ) ^ (-σ - 2) + (500 : ℝ) ^ (-σ - 1) / (σ + 1)) := by
  ...
  have hhead_re : (∑ n ∈ Finset.range (500 + 1), (((n : ℝ)) : ℂ) ^ (-s)).re
      = 1 + ∑ n ∈ Finset.Icc (2:ℕ) 500, ((n : ℝ) ^ (-σ) * cos618o2c n) := by
    rw [Complex.re_sum]
    ...
    have hcong : ∑ n ∈ Finset.Icc (2:ℕ) 500, ((((n : ℝ)) : ℂ) ^ (-s)).re
        = ∑ n ∈ Finset.Icc (2:ℕ) 500, ((n : ℝ) ^ (-σ) * cos618o2c n) := by
      refine Finset.sum_congr rfl ?_
      intro n hn
      have hn2 : 2 ≤ n := (Finset.mem_Icc.mp hn).1
      have h618 : -s = -(↑σ + ↑((((618:ℕ)):ℝ)/2) * Complex.I) := by
        rw [hsdef]
      rw [h618, (nat_cpow_neg_split (n := n) (by omega) σ ((((618:ℕ)):ℝ)/2)).1]
      rw [show ((((618:ℕ)):ℝ)/2) * Real.log ((n:ℕ))
          = (((618:ℕ)):ℝ) * (Real.log ((n:ℕ)) / 2) by ring]
      simp only [cos618o2c]
    rw [hcong]
    ring
  have hbdy : ‖(((500 : ℕ) : ℝ) : ℂ) ^ (1 - s) / (s - 1)‖
    ...
    have hden : (618/2 : ℝ) ≤ ‖s - 1‖ := by
      have habsim := Complex.abs_im_le_norm (s - 1)
      have him : (s - 1).im = ((618:ℕ):ℝ)/2 := by simp [hsim]
      rw [him] at habsim
      calc (618/2 : ℝ) = |((618:ℕ):ℝ)/2| := by norm_num
        _ ≤ ‖s - 1‖ := habsim
  ...
  have hEMre : (1 + ∑ n ∈ Finset.Icc (2:ℕ) 500, ((n : ℝ) ^ (-σ) * cos618o2c n))
      - (500 : ℝ) ^ (-σ) * (1000/618) - (500 : ℝ) ^ (-σ) * (1/2)
      ≤ EMval.re := by
    rw [hEMdef]
...
section AxiomAudit
#print axioms CriticalLinePhasor.DVP.re_zeta618o2_lower
end AxiomAudit
