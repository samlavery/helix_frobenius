import RequestProject.DVPPhaseStationQuarter

/-!
# The quarter-integer evaluated-boundary station core (rung-199.5)

`phase_station_lower_quarter` books the boundary crudely (`√N/(M/2)`); at
`t = 111.5` that cost exceeds the station margin.  This file evaluates the
boundary term exactly at `s = ½ + (M/4)i` — the quarter-integer analogue of
`boundary_re_eval`/`phase_station_lower_eval` — so the boundary can HELP.
-/
...
/-- **The exact boundary real part** at `s = ½ + (M/4)i`. -/
theorem boundary_re_eval_quarter (M N : ℕ) (hM : 1 ≤ M) (hN : 1 ≤ N) (ψ : ℝ) :
    (Complex.exp ((ψ : ℝ) * Complex.I)
        * ((((N : ℕ) : ℝ) : ℂ) ^ (1 - (↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/4) * Complex.I))
          / ((↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/4) * Complex.I) - 1))).re
      = (N:ℝ) ^ ((1/2 : ℝ))
        * (-(Real.cos ((M:ℝ) * (Real.log N / 4) - ψ)) / 2
          - ((M:ℝ)/4) * Real.sin ((M:ℝ) * (Real.log N / 4) - ψ))
        / ((1/4 : ℝ) + ((M:ℝ)/4) ^ 2) := by
  have hN0 : (0:ℝ) < (N:ℝ) := by exact_mod_cast hN
  set α : ℝ := (M:ℝ) * (Real.log N / 4) with hα
  have hcpow : (((N : ℕ) : ℝ) : ℂ) ^ (1 - (↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/4) * Complex.I))
      = (((N:ℝ) ^ ((1/2 : ℝ)) : ℝ) : ℂ)
  ...
  have hform : Complex.exp ((ψ : ℝ) * Complex.I)
      * ((((N:ℝ) ^ ((1/2 : ℝ)) : ℝ) : ℂ)
        ...
        / ((↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/4) * Complex.I) - 1))
      = (((N:ℝ) ^ ((1/2 : ℝ)) : ℝ) : ℂ)
        * (Complex.exp ((((ψ - α : ℝ))) * Complex.I)
          * ((((-(1/2) : ℝ)) : ℂ) + (((M:ℝ)/4 : ℝ) : ℂ) * Complex.I)⁻¹) := by
    rw [div_eq_mul_inv,
      show ((((ψ - α : ℝ))) : ℂ) * Complex.I
        = ((ψ : ℝ) : ℂ) * Complex.I + ((-α : ℝ) : ℂ) * Complex.I by
          push_cast; ring,
      Complex.exp_add,
      show ((↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/4) * Complex.I) - 1)
        = (((-(1/2) : ℝ)) : ℂ) + (((M:ℝ)/4 : ℝ) : ℂ) * Complex.I by push_cast; ring]
    ring
  rw [hform]
  rw [Complex.mul_re]
  ...
  have hD0 : (-(1/2) : ℝ) * -(1/2) + (M:ℝ)/4 * ((M:ℝ)/4) ≠ 0 := by positivity
  field_simp
  ring

/-- **The half-integer evaluated-boundary station lower bound** (height `M/2`). -/
theorem phase_station_lower_quarter_eval (M N : ℕ) (hM : 1 ≤ M) (hN : 1 ≤ N)
    (ψ : ℝ) :
    (∑ i ∈ Finset.range N,
        (((i+1 : ℕ)) : ℝ) ^ (-(1/2 : ℝ))
          * Real.cos ((M:ℝ) * (Real.log ((i+1 : ℕ)) / 4) - ψ))
      + (N:ℝ) ^ ((1/2 : ℝ))
        * (-(Real.cos ((M:ℝ) * (Real.log N / 4) - ψ)) / 2
          - ((M:ℝ)/4) * Real.sin ((M:ℝ) * (Real.log N / 4) - ψ))
        / ((1/4 : ℝ) + ((M:ℝ)/4) ^ 2)
    ...
    ≤ (Complex.exp ((ψ : ℝ) * Complex.I)
        * riemannZeta (line (((M:ℕ) : ℝ)/4))).re := by
  have hMR : (1:ℝ) ≤ (M:ℝ) := by exact_mod_cast hM
  have hNR : (1:ℝ) ≤ (N:ℝ) := by exact_mod_cast hN
  have hsform : line (((M:ℕ) : ℝ)/4) = ↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/4) * Complex.I := by
    apply Complex.ext
    · rw [line_re]
      simp
    · rw [lineIm]
      simp
  set s : ℂ := ↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/4) * Complex.I with hsdef
  have hsre : s.re = 1/2 := by rw [hsdef]; simp
  have hsim : s.im = (M:ℝ)/4 := by rw [hsdef]; simp
  ...
  have herr : ‖s‖ * ‖s + 1‖ * (1/8)
    ...
    have hns : ‖s‖ ≤ (M:ℝ)/4 + 1 := by
      calc ‖s‖ ≤ ‖(↑(1/2 : ℝ) : ℂ)‖ + ‖↑(((M:ℕ) : ℝ)/4) * Complex.I‖ := by
        ...
        _ ≤ (M:ℝ)/4 + 1 := by linarith
    have hns1 : ‖s + 1‖ ≤ (M:ℝ)/4 + 2 := by
      calc ‖s + 1‖ ≤ ‖s‖ + ‖(1 : ℂ)‖ := norm_add_le _ _
        _ ≤ (M:ℝ)/4 + 2 := by rw [norm_one]; linarith
    ...
    have hX : (0:ℝ) ≤ (N:ℝ) ^ (-(5/2 : ℝ)) + (N:ℝ) ^ (-(3/2 : ℝ)) / (3/2) := by
      ...
      positivity
    have hprod : ‖s‖ * ‖s + 1‖ ≤ ((M:ℝ)/4+1) * ((M:ℝ)/4+2) :=
  ...
  have hhead : (Complex.exp ((ψ : ℝ) * Complex.I)
      ...
      = ∑ i ∈ Finset.range N,
          (((i+1 : ℕ)) : ℝ) ^ (-(1/2 : ℝ))
            * Real.cos ((M:ℝ) * (Real.log ((i+1 : ℕ)) / 4) - ψ) := by
    ...
    intro i _
    have h0 := re_phase_term (n := i + 1) (by omega) (1/2) (((M:ℕ) : ℝ)/4) ψ
    rw [show (M:ℝ) * (Real.log ((i+1 : ℕ)) / 4) - ψ
        = ((M:ℕ) : ℝ)/4 * Real.log ((i+1 : ℕ)) - ψ by ring]
    rw [h0]
  have hbdyre := boundary_re_eval_quarter M N hM hN ψ
  ...
  have hEMre : (∑ i ∈ Finset.range N,
    ...
    rw [hbdy_eq]
    linarith [hh'.2, hhalf]
  have hζform : riemannZeta (line (((M:ℕ) : ℝ)/4)) = riemannZeta s := by
    rw [hsform]
  rw [hζform]
...
/-- **The half-integer evaluated-boundary station lower bound** (height `M/2`). (SECOND-ORDER: consumes `zeta_EM_enclosure_second_numeric`.) -/
theorem phase_station_lower_quarter_eval_second (M N : ℕ) (hM : 1 ≤ M) (hN : 1 ≤ N)
    (ψ : ℝ) :
    (∑ i ∈ Finset.range N,
        (((i+1 : ℕ)) : ℝ) ^ (-(1/2 : ℝ))
          * Real.cos ((M:ℝ) * (Real.log ((i+1 : ℕ)) / 4) - ψ))
      + (N:ℝ) ^ ((1/2 : ℝ))
        * (-(Real.cos ((M:ℝ) * (Real.log N / 4) - ψ)) / 2
        ...
        / ((1/4 : ℝ) + ((M:ℝ)/4) ^ 2)
      - (N:ℝ) ^ (-(1/2 : ℝ)) / 2
      - ((M:ℝ)/4+1) * (N:ℝ) ^ (-(3/2 : ℝ)) / 12
      - ((M:ℝ)/4+1) * ((M:ℝ)/4+2) * ((M:ℝ)/4+3) / 48
  ...
  have herr : ‖s‖ * ‖s + 1‖ * ‖s + 2‖ * (1/48)
    ...
    have hX : (0:ℝ) ≤ (N:ℝ) ^ (-(7/2 : ℝ)) + (N:ℝ) ^ (-(5/2 : ℝ)) / (5/2) := by
      ...
      positivity
    have hprod : ‖s‖ * ‖s + 1‖ * ‖s + 2‖ ≤ ((M:ℝ)/4+1) * ((M:ℝ)/4+2) * ((M:ℝ)/4+3) := by
      have h12 : ‖s‖ * ‖s + 1‖ ≤ ((M:ℝ)/4+1) * ((M:ℝ)/4+2) :=
        mul_le_mul hns hns1 (norm_nonneg _) (by positivity)
      calc ‖s‖ * ‖s + 1‖ * ‖s + 2‖
          ≤ (((M:ℝ)/4+1) * ((M:ℝ)/4+2)) * ((M:ℝ)/4+3) :=
            mul_le_mul h12 hns2 (norm_nonneg _) (by positivity)
        _ = ((M:ℝ)/4+1) * ((M:ℝ)/4+2) * ((M:ℝ)/4+3) := by ring
  ...
  have hEMre : (∑ i ∈ Finset.range N,
    ...
    have hB2n : ‖s * (((N : ℕ) : ℝ) : ℂ) ^ (-s - 1) / 12‖
      ...
      have : ‖s‖ * (N:ℝ) ^ (-(3/2 : ℝ)) ≤ ((M:ℝ)/4+1) * (N:ℝ) ^ (-(3/2 : ℝ)) :=
...
section AxiomAudit
#print axioms CriticalLinePhasor.DVP.boundary_re_eval_quarter
#print axioms CriticalLinePhasor.DVP.phase_station_lower_quarter_eval
#print axioms CriticalLinePhasor.DVP.phase_station_lower_quarter_eval_second
end AxiomAudit
