import RequestProject.DVPPhaseStationEighth

/-!
# The eighth-integer evaluated-boundary station core (rung-199.5)

`phase_station_lower_eighth` books the boundary crudely (`√N/(M/2)`); at
`t = 111.5` that cost exceeds the station margin.  This file evaluates the
boundary term exactly at `s = ½ + (M/8)i` — the eighth-integer analogue of
...
/-- **The exact boundary real part** at `s = ½ + (M/8)i`. -/
theorem boundary_re_eval_eighth (M N : ℕ) (hM : 1 ≤ M) (hN : 1 ≤ N) (ψ : ℝ) :
    (Complex.exp ((ψ : ℝ) * Complex.I)
        * ((((N : ℕ) : ℝ) : ℂ) ^ (1 - (↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/8) * Complex.I))
          / ((↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/8) * Complex.I) - 1))).re
      = (N:ℝ) ^ ((1/2 : ℝ))
        * (-(Real.cos ((M:ℝ) * (Real.log N / 8) - ψ)) / 2
          - ((M:ℝ)/8) * Real.sin ((M:ℝ) * (Real.log N / 8) - ψ))
        / ((1/4 : ℝ) + ((M:ℝ)/8) ^ 2) := by
  have hN0 : (0:ℝ) < (N:ℝ) := by exact_mod_cast hN
  set α : ℝ := (M:ℝ) * (Real.log N / 8) with hα
  have hcpow : (((N : ℕ) : ℝ) : ℂ) ^ (1 - (↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/8) * Complex.I))
      = (((N:ℝ) ^ ((1/2 : ℝ)) : ℝ) : ℂ)
  ...
  have hform : Complex.exp ((ψ : ℝ) * Complex.I)
      * ((((N:ℝ) ^ ((1/2 : ℝ)) : ℝ) : ℂ)
        ...
        / ((↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/8) * Complex.I) - 1))
      = (((N:ℝ) ^ ((1/2 : ℝ)) : ℝ) : ℂ)
        * (Complex.exp ((((ψ - α : ℝ))) * Complex.I)
          * ((((-(1/2) : ℝ)) : ℂ) + (((M:ℝ)/8 : ℝ) : ℂ) * Complex.I)⁻¹) := by
    rw [div_eq_mul_inv,
      show ((((ψ - α : ℝ))) : ℂ) * Complex.I
        = ((ψ : ℝ) : ℂ) * Complex.I + ((-α : ℝ) : ℂ) * Complex.I by
          push_cast; ring,
      Complex.exp_add,
      show ((↑(1/2 : ℝ) + ↑(((M:ℕ) : ℝ)/8) * Complex.I) - 1)
        = (((-(1/2) : ℝ)) : ℂ) + (((M:ℝ)/8 : ℝ) : ℂ) * Complex.I by push_cast; ring]
    ring
  rw [hform]
  rw [Complex.mul_re]
  ...
  have hD0 : (-(1/2) : ℝ) * -(1/2) + (M:ℝ)/8 * ((M:ℝ)/8) ≠ 0 := by positivity
  field_simp
  ring

/-- **The half-integer evaluated-boundary station lower bound** (height `M/2`). -/
theorem phase_station_lower_eighth_eval (M N : ℕ) (hM : 1 ≤ M) (hN : 1 ≤ N)
    (ψ : ℝ) :
    (∑ i ∈ Finset.range N,
        (((i+1 : ℕ)) : ℝ) ^ (-(1/2 : ℝ))
  ...
  have hhead : (Complex.exp ((ψ : ℝ) * Complex.I)
    ...
    rw [h0]
  have hbdyre := boundary_re_eval_eighth M N hM hN ψ
...
theorem phase_station_lower_eighth_eval_second (M N : ℕ) (hM : 1 ≤ M) (hN : 1 ≤ N)
    (ψ : ℝ) :
    (∑ i ∈ Finset.range N,
        (((i+1 : ℕ)) : ℝ) ^ (-(1/2 : ℝ))
          * Real.cos ((M:ℝ) * (Real.log ((i+1 : ℕ)) / 8) - ψ))
      + (N:ℝ) ^ ((1/2 : ℝ))
        * (-(Real.cos ((M:ℝ) * (Real.log N / 8) - ψ)) / 2
        ...
        / ((1/4 : ℝ) + ((M:ℝ)/8) ^ 2)
      - (N:ℝ) ^ (-(1/2 : ℝ)) / 2
      - ((M:ℝ)/8+1) * (N:ℝ) ^ (-(3/2 : ℝ)) / 12
      - ((M:ℝ)/8+1) * ((M:ℝ)/8+2) * ((M:ℝ)/8+3) / 48
  ...
  have herr : ‖s‖ * ‖s + 1‖ * ‖s + 2‖ * (1/48)
    ...
    have hX : (0:ℝ) ≤ (N:ℝ) ^ (-(7/2 : ℝ)) + (N:ℝ) ^ (-(5/2 : ℝ)) / (5/2) := by
      ...
      positivity
    have hprod : ‖s‖ * ‖s + 1‖ * ‖s + 2‖ ≤ ((M:ℝ)/8+1) * ((M:ℝ)/8+2) * ((M:ℝ)/8+3) := by
      have h12 : ‖s‖ * ‖s + 1‖ ≤ ((M:ℝ)/8+1) * ((M:ℝ)/8+2) :=
        mul_le_mul hns hns1 (norm_nonneg _) (by positivity)
      calc ‖s‖ * ‖s + 1‖ * ‖s + 2‖
          ≤ (((M:ℝ)/8+1) * ((M:ℝ)/8+2)) * ((M:ℝ)/8+3) :=
            mul_le_mul h12 hns2 (norm_nonneg _) (by positivity)
        _ = ((M:ℝ)/8+1) * ((M:ℝ)/8+2) * ((M:ℝ)/8+3) := by ring
  ...
  have hEMre : (∑ i ∈ Finset.range N,
    ...
    have hB2n : ‖s * (((N : ℕ) : ℝ) : ℂ) ^ (-s - 1) / 12‖
      ...
      have : ‖s‖ * (N:ℝ) ^ (-(3/2 : ℝ)) ≤ ((M:ℝ)/8+1) * (N:ℝ) ^ (-(3/2 : ℝ)) :=
...
section AxiomAudit
#print axioms CriticalLinePhasor.DVP.boundary_re_eval_eighth
#print axioms CriticalLinePhasor.DVP.phase_station_lower_eighth_eval
#print axioms CriticalLinePhasor.DVP.phase_station_lower_eighth_eval_second
end AxiomAudit
